...
<div id="{if $banner}page_banner_container_{$banner.id_st_page_banner}{/if}" class="breadcrumb_wrapper {if !$sttheme.fullwidth && $breadcrumb_width} wide_container {/if}" {if $banner['image_multi_lang']} style="background-image:url({$banner['image_multi_lang']});" {/if}>
{if !$banner.hide_breadcrumb || $banner.description}
  <div class="{if !$sttheme.fullwidth}container{else}container-fluid{/if}"><div class="row">
