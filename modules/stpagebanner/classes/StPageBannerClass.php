...
class StPageBannerClass extends ObjectModel
{
	...
	public static $definition = array(
		...
		'fields'    => array(
            ...
            'show_subcate'        => array('type' => self::TYPE_BOOL, 'validate' => 'isBool'),
	...
	public static function getAll($identify=0, $type=0, $id_lang, $active=0, $limit=0)
	{
	   Shop::addTableAssociation('st_page_banner', array('type' => 'shop'));
        ...
        if ($type == 1) {
            $categories = array();
            $category = new Category((int)$identify);
            if($category->id)
            {
            ...
            $where = ' AND (sms.`show_subcate` = 0 AND sms.`item_v`="'.$identify.'" OR sms.`show_subcate` = 1 AND sms.`item_v` IN('.implode(',', $categories) .'))';
            $limit = 1;
        }
