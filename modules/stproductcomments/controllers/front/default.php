...
class StProductCommentsDefaultModuleFrontController extends ModuleFrontController
{
	...
	protected function ajaxProcessAddComment()
	{
		...
		if (!count($errors))
		{
			$customer_comment = StProductCommentClass::getByCustomerId((int)(Tools::getValue('id_product','0')), $id_customer, $this->context->shop->id, (int)$id_guest);
			if (!$customer_comment || ($customer_comment && (strtotime($customer_comment['date_add']) + Configuration::get($this->_prefix_st.'MINIMAL_TIME')) < time()))
			{
                ...
                $comment->validate = !Configuration::get($this->_prefix_st.'MODERATE');
				$comment->save();
                ...
                if ($comment->id && Configuration::get($this->_prefix_st.'NOTICE_WEBMASTER')) {
                    ...
                    Mail::Send(
                        ...
                        'review_admin',
                        $this->trans('New comment for product', array(), 'Shop.Theme.Transformer').'[#'.$comment->id_proudct.']',
                        $param,
                        $email,
                        null,
                        null,
