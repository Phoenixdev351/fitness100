...
class StProductCommentClass extends ObjectModel
{
    ...
    )
    {
        ...
        $comments = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            ...
            AND pc.`id_parent` = 0'.
            ($image_only ? ' GROUP BY pci.`id_st_product_comment` ' : '').
    		'
    ...
    public static function statByProduct($id_product = 0)
    {
        ...
        $result = Db::getInstance()->getRow('
            ...
            AND pc.`id_parent` = 0   
        ');
        $result['image_total'] = Db::getInstance()->getValue('select count(0) from (
            SELECT COUNT(0)
            ...
            ($validate == '1' ? ' AND pc.`validate` = 1' : '').
            ($id_product ? ' AND pc.`id_product` = '.(int)$id_product : '').'
            AND pc.`id_parent` = 0 group by pci.id_st_product_comment) as c
        ');
        return $result;
    }
	...
	public static function getGradeByProduct($id_product, $id_lang)
	{
		...
		LEFT JOIN `'._DB_PREFIX_.'st_product_comment_criterion_lang` pccl ON (pccl.`id_st_product_comment_criterion` = pcg.`id_st_product_comment_criterion` AND pccl.`id_lang` = '.(int)$id_lang.')
	...
	public static function getAveragesByProduct($id_product, $id_lang)
	{
		...
		for ($i = 0; $i < $count_grades; ++$i)
			if (array_key_exists((int)$grades[$i]['id_st_product_comment_criterion'], $criterionsGradeTotal) === false)
				$criterionsGradeTotal[(int)$grades[$i]['id_st_product_comment_criterion']] = (int)($grades[$i]['grade']);
			else
				$criterionsGradeTotal[(int)$grades[$i]['id_st_product_comment_criterion']] += (int)($grades[$i]['grade']);
    ...
    )
	{
		...
		$sql  = '
			...
			WHERE pc.`id_shop` = '.(int)Context::getContext()->shop->id.'
            AND pc.`id_parent`='.(int)$id_parent.
            (!$all ? ' AND pc.`validate` = '.(int)$validate : '').self::getFilter();
    ...
    public static function getFilter()
    {
        ...
        if (count($_POST)) {
            foreach($_POST as $key => $val) {
                if (strpos($key, 'stproductcommentsFilter_') !== false) {
                    $field = str_replace('stproductcommentsFilter_', '', $key);
                    if (($field == 'id_st_product_comment') && $val) {
                        $where .= ' AND pc.'.$field.'="'.$val.'"';
                    } elseif($field == 'home_featured' && $val !== '') {
                        $where .= ' AND pc.'.$field.'="'.(int)$val.'"';
                    } elseif ($field == 'date_add' && $val) {
                        if (isset($val[0]) && $val[0]) {
                            $where .= ' AND pc.'.$field.'>="'.$val[0].'"';
                        }
                        if (isset($val[1]) && $val[1]) {
                            $where .= ' AND pc.'.$field.'<="'.$val[1].'"';    
                        }
                    } elseif ($field == 'name' && $val) {
                        $result = Db::getInstance()->executeS('SELECT id_product FROM '._DB_PREFIX_.'product_lang WHERE name LIKE "%'.$val.'%" AND id_lang='.(int)Context::getContext()->language->id.' AND id_shop='.(int)Context::getContext()->shop->id);
                        $ids = '';
                        foreach($result as $row) {
                            $ids .= $row['id_product'].',';
                        }
                        $where .= ' AND pc.id_product IN ('.trim($ids, ',').')';
                    } elseif ($field == 'content' && $val) {
    ...
    public static function getByCustomerId($id_product, $id_customer, $id_shop, $id_guest)
    {
        return Db::getInstance()->getRow('
            SELECT * FROM `'._DB_PREFIX_.'st_product_comment`
            WHERE id_product='.(int)$id_product.'
            AND id_customer='.(int)$id_customer.'
            AND id_shop='.(int)$id_shop.'
            AND id_guest='.$id_guest.'
            ORDER BY id_st_product_comment DESC'
        );
    }
}
