...
class StProductComments extends Module implements WidgetInterface
{
        ...
        $this->_hooks = array(
            'Product' => array(
                ...
                array(
                    ...
                    'val' => '1',
                    'name' => $this->getTranslator()->trans('displayProductDescRightColumn', array(), 'Admin.Theme.Transformer'),
                ),
                array(
    ...
    private function putCSVFile($data = array(), $name)
    {
        if (!$data || !$name) {
            return false;
        }
        $csv = _PS_MODULE_DIR_.$this->name.'/'.$name.'.csv';
        @unlink($csv);
        if ($fp = fopen($csv, 'w')) {
            $i = 0;
            foreach ($data as &$cur)
    ...
    public function getContent()
	{
        ...
        if (Tools::isSubmit('add'.$this->name) || (Tools::isSubmit('update'.$this->name) && $id_st_product_comment))
        {
            ...
            $tabs = array();
            $tabs = array(
                array('id'  => '0,1,5,6,12,11', 'name' => $this->getTranslator()->trans('General settings', array(), 'Admin.Theme.Transformer')),
    ...
    private function _postProcess()
    {
        ...
        if (isset($_POST['save'.$this->name])) {
            ...
            if (!count($error) && $validate === true)
            {
                ...
                if($comment->save(false, false))
                {
    				if ($grade = (int)Tools::getValue('grade'))
    				{
    				    StProductCommentCriterionClass::deleteByComment($comment->id);
    				    foreach(StProductCommentCriterionClass::getCriterions($this->context->language->id) as $value) {
    				        $product_comment_criterion = new StProductCommentCriterionClass($value['id_st_product_comment_criterion']);
           ...
           if ($criterion->validateFields(false) && $criterion->save()) {
            ...
            if (Tools::getValue('Product_displayProductDescRightColumn') && Tools::getValue('Product_displayProductExtraContent')) {
                unset($_POST['Product_displayProductDescRightColumn']);
            }
            $this->updateConfigurLang();
            $this->saveHook();
            foreach($this->fields_form AS $form) {
                foreach($form['form']['input'] AS $field) {
                    if ($field['type'] == 'file') {
            ...
            if (isset($_FILES['csv_file']) && $_FILES['csv_file'] && !empty($_FILES['csv_file']['tmp_name']) && $_FILES['csv_file']['error'] == 0) {
                $ext = strtolower(substr(strrchr($_FILES['csv_file']['name'], '.'), 1));
                if ($ext != 'csv') {
                    $this->validation_errors[] = $this->getTranslator()->trans('Please upload csv file.', array(), 'Admin.Theme.Transformer');
                }
                $file = _PS_MODULE_DIR_.$this->name.'/import/'.date('YmdH').'.csv';
                if (!move_uploaded_file($_FILES['csv_file']['tmp_name'], $file)) {
                    $this->validation_errors[] = $this->getTranslator()->trans('Move upload file failed.', array(), 'Admin.Theme.Transformer');
                }
                if (file_exists($file)) {
                    if ($fp = fopen($file, 'r')) {
                        set_time_limit(6*60);
                        ...
                        while (!feof($fp)) {
                            ...
                            // [0]Comment ID;[1]Customer;[2]Rating;[3]Product ID;[4]Status;[5]comment;[6]Add time
                            $comment_id = (int)$row[0];
                            if ($comment_id) {
                                $comment = new StProductCommentClass($comment_id);
                            } else {
                                $comment = new StProductCommentClass();
                            }
                            $comment->id_parent = 0;
                            $comment->id_shop = (int)$this->context->shop->id;
                            $comment->id_order_detail = 0;
                            if ((int)$row[1] > 0) {
                                $comment->id_customer = (int)$row[1];
                                $customer = new Customer($comment->id_customer);
                                if ($customer->id) {
                                    $comment->customer_name = $customer->firstname.' '.$customer->lastname;
                                }
                            } else {
                                $comment->customer_name = pSQL($row[1]);
                            }
                            $comment->id_product = (int)$row[3];
                            $comment->validate = (int)$row[4];
                            $comment->content = pSQL($row[5]);
                            $comment->date_add = pSQL($row[6]);
                            $comment->is_admin = 1;
                    ...
                    $this->validation_errors[] = $this->getTranslator()->trans('Upload file failed, plase ensure the folder %1% is writable.', array('%1%' => dirname($file)), 'Admin.Theme.Transformer');
                }
            }
    ...
    public function updateCatePerRow() {
        foreach ($this->fields_default_stsn as $k => $v) {
            Configuration::updateValue($this->_prefix_stsn.Tools::strtoupper($k), (int)Tools::getValue($k));
        }
    }
    ...
    protected function initConfigFormFields()
    {
        ...
        $this->fields_form[11]['form'] = array(
            'legend' => array(
                'title' => $this->getTranslator()->trans('Comment list page', array(), 'Admin.Theme.Transformer'),
                'icon' => 'icon-cogs'
            ),
            'input' => $fields['list'],
            'submit' => array(
        ...
        $this->fields_form[12]['form'] = array(
            'legend' => array(
                'title' => $this->getTranslator()->trans('Import comments from CSV file', array(), 'Admin.Theme.Transformer'),
                'icon' => 'icon-cogs'
            ),
            'input' => array(
                array(
                    'type' => 'file',
                    'label' => $this->getTranslator()->trans('Upload CSV file:', array(), 'Admin.Theme.Transformer'),
                    'name' => 'csv_file',
                    'desc' => array(
                        $this->getTranslator()->trans('Download CSV template %1% here %2%', array('%1%' => '<a href="'._MODULE_DIR_.$this->name.'/sample_comment.csv">', '%2%' => '</a>'), 'Admin.Theme.Transformer'),
                    ),
                ),
            ),
            'submit' => array(
    ...
    public function renderCommentForm()
	{
		...
		$this->fields_form[0]['form'] = array(
			...
			'input' => array(
                ...
                'pc_images' => array(
                    'type' => 'pc_images',
                    'label' => $this->trans('Images:', array(), 'Admin.Theme.Transformer'),
                    'name' => 'image',
                    'values' => $pc_images,
                 ),
                 array(
                ...
                'date_add' => array(
                    'type' => 'date',
                    'label' => $this->trans('Created on:', array(), 'Admin.Theme.Transformer'),
                    'name' => 'date_add',
        ...
        if (!$comment->id) {
            ...
            $comment->date_add = date('Y-m-d', strtotime($comment->date_add));
        }
        if ($comment->id_parent) {
            unset($this->fields_form[0]['form']['input']['id_product']);
            unset($this->fields_form[0]['form']['input']['grade']);
            ...
            unset($this->fields_form[0]['form']['input']['date_add']);
            $this->_html .= $this->getNavigate($comment->id ? $comment->id : $id_st_product_comment);
        }
        if (!$pc_images['str']) {
            unset($this->fields_form[0]['form']['input']['pc_images']);
        }
		$helper = new HelperForm();
        $helper->module = $this;
        $helper->show_toolbar = false;
    ...
    public function renderCommentExtra($hookName, $params)
    {
        ...
        if (!$this->isCached('module:stproductcomments/views/templates/hook/stproductcomments_reviews.tpl', $this->stGetCacheId($hookName.$id_product))) {
    ...
    public function hookDisplayAdminOrder($params)
    {
        ...
        $this->smarty->assign(array(
            'order' => $order,
            'products' => $products,
            'secure_key' => Tools::encrypt('StProductCommentsMyCommentsModuleFrontController'),
        ));
        return $this->display(__FILE__, 'bo_order.tpl');
    }
    ...
    private function _prepare_comments($id_product){
        ...
        $allow_guest = Configuration::get($this->_prefix_st.'ALLOW_GUESTS');
        // if ($allow_guest) {
        // }
        
        $comment_arr =  StProductCommentClass::getListComments(1, 1, $id_product, $this->context->cookie->id_customer);
        $one_comment = false;
        if(is_array($comment_arr))
            $one_comment = array_shift($comment_arr);
        
        $this->smarty->assign(array(
            'allow_guest' => $allow_guest,
            'logged' => $this->context->customer->isLogged(true),
            'customer_name' => $this->context->customer->isLogged(true) ? $this->context->customer->firstname.' '.$this->context->customer->lastname : '',
            'id_module' => $this->id,
            ...
            'criterions2' => StProductCommentCriterionClass::getCriterions($this->context->language->id, true),
            'averages' => $averages,
            'averageTotal' => $averageTotal,
            'biaoqian' => $biaoqian,
            'can_comment' => StProductCommentClass::canComment($this->context->cookie->id_customer, $id_product),
            'g_rich_snippets' => Configuration::get($this->_prefix_st.'GOOGLE_RICH_SNIPPETS'),
            'aw_display' => Configuration::get($this->_prefix_st.'PRO_AW_DISPLAY'),
    ...
    public function hookDisplayHeader($params)
    {
        ...
        Media::addJsDef(array(
            'stproductcomments_url' => $this->context->link->getModuleLink('stproductcomments','default'),
        ...
        if (isset($this->context->controller->module) && $this->context->controller->module->name == $this->name || Configuration::get($this->_prefix_st.'ALLOW_GUESTS') && Dispatcher::getInstance()->getController() == 'product' || Dispatcher::getInstance()->getController() == 'article') {
        ...
        if (Configuration::get($this->_prefix_st.'UPLOAD_IMAGE') && (isset($this->context->controller->module) && $this->context->controller->module->name == $this->name || Configuration::get($this->_prefix_st.'ALLOW_GUESTS') && Dispatcher::getInstance()->getController() == 'product')) {
        ...
        if (!$this->isCached($template, $this->stGetCacheId('header')))
        {
            ...
            /*else
                $custom_css .= '.'.$classname.' .products_section .swiper-button.swiper-button-disabled, .'.$classname.' .products_section .swiper-button.swiper-button-disabled:hover{background-color:transparent;}';*/
    ...
    {
        if(Dispatcher::getInstance()->getController()!='index' && !isset($params['st_time']))
            return false;
    ...
    public function getComments($ext = '')
    {
        ...
        if ($comments) {
            ...
            foreach ($comments as &$value) {
                ...
                $array = explode(' ', $value['customer_name']);
                $custom_name = '';
                ...
                if (!$custom_name_ds) {
                    ...
                    $value['customer_name'] = trim($custom_name);
    ...
    {
        $fields_value = array();
        $lang_array = array('list_page_title','list_page_keyword','list_page_description');
        foreach($this->getFormFieldsDefault() AS $k => $v) {
            if (!in_array($k, $lang_array)) {
                $fields_value[$k] = Configuration::get($this->_prefix_st.strtoupper($k));
            }
        }
        $languages = Language::getLanguages(false);
        $fields_value['list_page_title'] = $fields_value['list_page_keyword'] = $fields_value['list_page_description'] = array();
        foreach ($languages as $language)
        {
            $fields_value['list_page_title'][$language['id_lang']] = Configuration::get($this->_prefix_st.'LIST_PAGE_TITLE', $language['id_lang']);
            $fields_value['list_page_keyword'][$language['id_lang']] = Configuration::get($this->_prefix_st.'LIST_PAGE_KEYWORD', $language['id_lang']);
            $fields_value['list_page_description'][$language['id_lang']] = Configuration::get($this->_prefix_st.'LIST_PAGE_DESCRIPTION', $language['id_lang']);
        }
        foreach($this->fields_default_stsn AS $k=> $v) {
    ...
    public function updateConfigurLang() {
        $languages = Language::getLanguages(false);
        $list_page_title = $list_page_keyword  = $list_page_description = array();
        $defaultLanguage = new Language((int)(Configuration::get('PS_LANG_DEFAULT')));
        foreach ($languages as $language)
        {
            $list_page_title[$language['id_lang']] = Tools::getValue('list_page_title_'.$language['id_lang']) ? Tools::getValue('list_page_title_'.$language['id_lang']) : Tools::getValue('list_page_title_'.$defaultLanguage->id);
            $list_page_keyword[$language['id_lang']] = Tools::getValue('list_page_keyword_'.$language['id_lang']) ? Tools::getValue('list_page_keyword_'.$language['id_lang']) : Tools::getValue('list_page_keyword_'.$defaultLanguage->id);
            $list_page_description[$language['id_lang']] = Tools::getValue('list_page_description_'.$language['id_lang']) ? Tools::getValue('list_page_description_'.$language['id_lang']) : Tools::getValue('list_page_description_'.$defaultLanguage->id);
        }
        Configuration::updateValue($this->_prefix_st.'LIST_PAGE_TITLE', $list_page_title);
        Configuration::updateValue($this->_prefix_st.'LIST_PAGE_KEYWORD', $list_page_keyword);
        Configuration::updateValue($this->_prefix_st.'LIST_PAGE_DESCRIPTION', $list_page_description);
    }
    ...
    public function hookActionDeleteGDPRCustomer($customer)
    {
        if (!empty($customer['id'])) {
            $sql = "SELECT id_st_product_comment FROM "._DB_PREFIX_."st_product_comment WHERE id_customer = '".(int)$customer['id']."'";
            if ($rs = Db::getInstance()->executeS($sql)) {
                foreach($rs AS $val) {
                    $object = new StProductCommentClass($val['id_st_product_comment']);
                    $object->delete();
                }
            }
    ...
    {
        if (!Tools::isEmpty($customer['id'])) {
            $sql = "SELECT * FROM "._DB_PREFIX_."st_product_comment WHERE id_customer = '".(int)$customer['id']."'";
            if ($res = Db::getInstance()->ExecuteS($sql)) {
                return json_encode($res);
            }
            return json_encode($this->getTranslator()->trans('Product comments: No data found.', array(), 'Shop.Theme.Transformer'));
       }
   }
   public function hookDashboardZoneOne($params)
   {
        $this->context->smarty->assign(
            array(
                'nbr_all' => StProductCommentClass::getByValidate(0, 0, true, true),
                'nbr_pendding' => StProductCommentClass::getByValidate(0, 0, false, true),
                'nbr_reported' => count(StProductCommentClass::getReportedComments()),
