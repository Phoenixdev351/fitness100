...
<div class="flex_container flex_left general_top_border general_bottom_border p-2 mb-2 general_bg">
  <a href="{$pcomments.clear_all_link}" class="{if $pcomments.id_product} pr-search-link {/if} mr-3 {if !$pcomments.does_filter_by_star && !$pcomments.does_filter_by_pic} theme_color {/if}" rel="nofollow" title="{l s='All reviews' d='Shop.Theme.Transformer'}">{l s='All reviews' d='Shop.Theme.Transformer'}<span class="pcomment_stat">({$pcomments.stat.total})</span></a>
  <div class="dropdown_wrap mr-3">{strip}
        ...
        <div class="dropdown_list" aria-labelledby="">
            <ul class="dropdown_list_ul dropdown_box">
              {foreach $pcomments.filter_star as $filter}
              {if !$filter.current}
              <li><a href="{$filter.url}" class="{if $pcomments.id_product} pr-search-link {/if} dropdown_list_item" title="{$filter.label}" rel="nofollow">{$filter.label}<span class="pcomment_stat">({$pcomments.stat.{$filter.val}})</span></a></li>
              {/if}
              {/foreach}
            </ul>
        </div>
  </div>
  <a href="{$pcomments.filter_pic}" class="{if $pcomments.id_product} pr-search-link {/if} mr-3 {if $pcomments.does_filter_by_pic} theme_color {/if}" rel="nofollow" title="{l s='With pictures' d='Shop.Theme.Transformer'}" >{l s='With pictures' d='Shop.Theme.Transformer'}<span class="pcomment_stat">({$pcomments.stat.image_total})</span></a>
  ...
  <div class="dropdown_wrap flex_float_right">{strip}
        ...
        <div class="dropdown_list" aria-labelledby="">
            <ul class="dropdown_list_ul dropdown_box">
              {foreach $pcomments.filter_order as $filter}
              {if !$filter.current}
              <li><a href="{$filter.url}" class="{if $pcomments.id_product} pr-search-link {/if} dropdown_list_item" title="{$filter.label}" rel="nofollow">{$filter.label}</a></li>
              {/if}
              {/foreach}
            </ul>
        </div>
