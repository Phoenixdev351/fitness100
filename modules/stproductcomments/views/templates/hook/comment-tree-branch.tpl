...
<div id="pcomment-{$node.id_st_product_comment}" class="line_item">
    <div class="flex_container flex_column_md flex_start px-2 pcomment-for-reply" {if $pcomments.g_rich_snippets} itemprop="review" itemscope itemtype="https://schema.org/Review"{/if}>
        <div class="pcomment_left">
            <div class="pcomment_author font-weight-bold pcomment_left_item" {if $pcomments.g_rich_snippets} itemprop="author" itemscope itemtype="https://schema.org/Person"{/if}><span{if $g_rich_snippets} itemprop="name"{/if}>{$node.customer_name}</span></div>
            ...
            <div class="flex_box">
                <div class="pcomment_rbl flex_child mb-1">
                    ...
                    {if isset($pcomments.id_product) && $pcomments.id_product && $node.product_attr_name}
                        <span{if $pcomments.g_rich_snippets} itemprop="itemReviewed"{/if}  class="pcomment_attr_name mr-2">
                            {$node.product_attr_name}
                        </span>
                    {else}
                        <a class="pcomment_product_name mr-2" href="{$node.product_link}" title="{$node.product_name_full}">{if $pcomments.g_rich_snippets}<span itemprop="itemReviewed">{/if}{$node.product_name_full}{if $pcomments.g_rich_snippets}</span>{/if}</a>
                    {/if}
                    {/if}
