...
class StblogArticleModuleFrontController extends StblogModuleFrontController
{
        ...
        if(!$this->errors)
		{
            ...
            if (Module::isInstalled('jscomposer') && (bool) Module::isEnabled('jscomposer')) {
                if ($instance = Module::getInstanceByName('jscomposer')) {
                    $instance->init();
                    $this->blog->content = $instance->do_shortcode($this->blog->content);    
                }
            }
    ...
    {
        return $this->context->link->getModuleLink('stblog', 'article', array('id_st_blog'=>$this->blog->id, 'rewrite'=>$this->blog->link_rewrite));
    }
}