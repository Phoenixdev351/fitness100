...
class AdminStBlogController extends ModuleAdminController
{
    ...
    public function __construct()
	{
        ...
        $this->_join = 'INNER JOIN '._DB_PREFIX_.'st_blog_shop sa ON a.id_st_blog=sa.id_st_blog && sa.id_shop IN('.implode(',',Shop::getContextListShopID()).')';
        $this->_join .= $id_category ? 'INNER JOIN '._DB_PREFIX_.'st_blog_category_blog cb ON a.id_st_blog = cb.id_st_blog' : '';

        $this->_select = 'sa.id_shop'.($id_category ? ',cb.position' : '');
    ...
    public function displayCover($id)
    {
        ...
        $url = $object->getCoverUrl('thumb', Context::getContext()->language->id);
    ...
    {
        $blog = new StBlogClass((int)$id, $this->context->language->id);
        return '<a href="'.$this->context->link->getModuleLink('stblog', 'article', array('id_st_blog'=>(int)$id,'rewrite'=>$blog->link_rewrite,'preview'=>1)).'" target="_blank" class="view">
	...
	public function postProcess()
	{
		...
		if (Tools::isSubmit('submitAdd'.$this->table) || Tools::isSubmit('submitAdd'.$this->table.'AndStay'))
		{
            ...
            // Use extact time to calc timeago
            $diff_time = strtotime('Now') - strtotime(date('Y-m-d 00:00:00'));
            $_POST['date_add'] = date('Y-m-d H:i:s', strtotime($_POST['date_add']) + $diff_time);
            ...
            {
                $this->redirect_after = '';
                $this->errors[] = $this->l('No permission to write for folder['._PS_ST_BLOG_IMG_DIR_.'].');
            }
            else
                return $object;
		}
        ...
        elseif(Tools::isSubmit('deletecoverimage'))
        {
            if ($is_st_blog = (int)Tools::getValue('id_st_blog'))
            {
        		...
        		if ($image->setShopList($image->getAssociatedShops())->delete())
        			...
        			$this->errors[] = $this->l('An error occurred while attempting to delete the cover.');    
            }else
        			$this->errors[] = $this->l('An error occurred while attempting to delete the cover[parameter error].');
				...
				if (Validate::isLoadedObject($object = $this->loadObject(true)))
				{
					...
					if (isset($object->noZeroObject) && count($taxes = call_user_func(array($this->className, $object->noZeroObject))) <= 1)
						$this->errors[] = $this->l('You need at least one object.')
							.' <b>'.$this->table.'</b><br />'.$this->l('You cannot delete all of the items.');
					else
					{
                        $identifier = '';
						if ($this->deleted)
		...
		elseif (Tools::getValue('submitDel'.$this->table) || Tools::isSubmit('submitBulkdelete'.$this->table))
		{
            ...
            Tools::redirectAdmin(self::$currentIndex.'&updatest_blog&id_st_blog='.(int)Tools::getValue('id_st_blog').'&token='.Tools::getValue('token'));
        }
		parent::postProcess();
	}
    ...
    public function postCover($obj)
    {
        if (isset($_FILES) && count($_FILES)) {
            foreach($_FILES as $name => $value) {
                ...
                list($_name, $id_lang) = explode('_', $name);
                if (isset($_FILES[$name]['tmp_name']) && !empty($_FILES[$name]['tmp_name']))
                {
                    $old_images = StBlogImageClass::getImageIdByType($obj->id, 1, (int)$id_lang);
                    foreach($old_images AS $old)
                    {
                        $img = new StBlogImageClass('1', $old);
                        $img->setShopList($img->getAssociatedShops())->delete();
                    }
                    $image = new StBlogImageClass('1', null, (int)$id_lang);
                    $image->setShopList($obj->getAssociatedShops())->save($name);    
                }
            }
        }
        return true;
	...
	public function renderForm()
	{
        ...
        // Add possibility to upload cover for each langauge.
        ...
        foreach ($languages as $lang)
        {
            $desc = '';
            if ($this->object->id) {
                $img = $this->object->getCoverUrl('thumb', $lang['id_lang']);
                $desc = $img ? '<img src="'.$img.'" width="200"/>'.
                    '<p><a class="btn btn-default st_blog_delete_cover" data-id="'.$this->object->id.'" data-lang="'.(int)$lang['id_lang'].'" data-token="'.Tools::getValue('token').'" href="javascript:;"><i class="icon-trash"></i> '.$this->trans(' Delete', array(), 'Admin.Theme.Transformer').'</a></p>' : '';
            }
            $form_cover['image_'.$lang['id_lang']] = array(
                    'type' => 'file',
                    'label' => $this->trans('Cover image:', array(), 'Modules.Stblog.Admin').' - '.$lang['name'].($default_lang == $lang['id_lang'] ? '('.$this->trans('default language', array(), 'Modules.Stblog.Admin').')' : '').':',
                    'name' => 'image_'.$lang['id_lang'],
                    'desc' => $desc
                );
        }
        $cover_key = array_search('cover', $this->fields_form[0]['form']['input']);
        $array = array_slice($this->fields_form[0]['form']['input'], 0, $cover_key);
        $array = $array + $form_cover;
        array_splice($this->fields_form[0]['form']['input'], 0, $cover_key+1, $array);
   	...
   	public function ajaxProcessDeleteGalleryImage()
	{
        ...
        if (!($id_image = (int)Tools::getValue('id_image')))
            die(Tools::jsonEncode(array('error'=>$this->l('An error occurred (the image not exists).'),'id'=>$id_image)));
        
        $image = new StBlogImageClass('2', $id_image);
        
        if (!$image->id)
            die(Tools::jsonEncode(array('error'=>$this->l('An error occurred (unknown object).'),'id'=>$id_image)));
        
		if ($image->setShopList($image->getAssociatedShops())->delete())
        {
            $image->refreshPositions();
            die(Tools::jsonEncode(array('success'=>$this->l('The picture was deleted successful.'),'id'=>$id_image)));
        }
		else
            die(Tools::jsonEncode(array('error'=>$this->l('The picture was deleted successful.'),'id'=>$id_image)));
	}
    ...
    public function ajaxProcessUpdateImagePosition()
	{
		...
		if ($res)
			...
			$this->jsonError($this->l('An error occurred while attempting to move this picture.'));
	}
    ...
    public function ajaxProcessUpdateGalleryImageShopAsso()
	{
        ...
        if (empty($images))
             $this->jsonError($this->l('An error occurred while attempting to associate this image with your shop(Image not Found). '));
		...
		if ($res)
			...
			$this->jsonError($this->l('An error occurred while attempting to associate this image with your shop. '));
	}
    ...
    public function ajaxProcessAddImage()
	{
		...
		{
			$files = array();
			$files[0]['error'] = $this->l('Cannot add image because blog creation failed.');
		}
    ...
    public function ajaxProcessDeleteCoverImage()
	{
		...
		$image = StBlogImageClass::getCoverImage((int)Tools::getValue('id_st_blog'), (int)$id_lang);
        if ($image)
        {
