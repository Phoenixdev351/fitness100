...
class StblogModuleFrontController extends ModuleFrontController
{
    ...
    public function getTemplateVarPage()
    {
    	...
    	if ($page_name == 'module-stblog-category' && ($id_st_blog_category = Tools::getValue('id_st_blog_category')))
        {
    		...
    		{
    			$ret['title'] = $row['meta_title'] ? $row['meta_title'] : $row['name'];
                $ret['description'] = $row['meta_description'];
                $ret['keywords'] = $row['meta_keywords'];
    		}
        }
		elseif ($page_name == 'module-stblog-article' && ($id_st_blog = Tools::getValue('id_st_blog')))
        {
            $sql = 'SELECT `name`,`meta_title`, `meta_description`, `meta_keywords`
            ...
            if ($m = Tools::getValue('m')) {
                $ret['title'] = $m.' - '.Configuration::get('ST_BLOG_META_TITLE',$this->context->language->id);
                $ret['description'] = $m.' - '.Configuration::get('ST_BLOG_META_DESCRIPTION', $this->context->language->id);
            }
        }
        
        $ret['title'] =='' && $ret['title'] = Configuration::get('ST_BLOG_META_TITLE', $this->context->language->id);
        $ret['description'] =='' && $ret['description'] = Configuration::get('ST_BLOG_META_DESCRIPTION', $this->context->language->id);
        $ret['keywords'] =='' && $ret['keywords'] = Configuration::get('ST_BLOG_META_KEYWORDS', $this->context->language->id);
