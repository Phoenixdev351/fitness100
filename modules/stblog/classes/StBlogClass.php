...
class StBlogClass extends ObjectModel
{
    ...
    public static function getBlogDetials($id_lang, $row, Context $context = null)
    {
        ...
        if($row['accept_comment'] && Module::isInstalled('stblogcomments') && Module::isEnabled('stblogcomments')) {
            if (!class_exists('StBlogCommentClass')) {
                require_once(_PS_MODULE_DIR_.'stblogcomments/classes/StBlogCommentClass.php');
            }
            $row['comment_counter'] = StBlogCommentClass::countComments($id_st_blog,$context->shop->id,Configuration::get('ST_BLOG_C_MODERATE'));
        }
        if($row['type']==1)
        {
            if (!$cover = StBlogImageClass::getCoverImage($id_st_blog,$id_lang,1)) {
                $cover = StBlogImageClass::getCoverImage($id_st_blog,(int)Configuration::get('PS_LANG_DEFAULT'),1);
            }
        ...
        if (Module::isInstalled('jscomposer') && (bool) Module::isEnabled('jscomposer')) {
            if ($instance = Module::getInstanceByName('jscomposer')) {
                $instance->init();
                $row['content'] = $instance->do_shortcode($row['content']);    
            }
        }
