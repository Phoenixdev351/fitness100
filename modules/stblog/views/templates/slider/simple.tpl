...
<div class="pro_column_box clearfix line_item">
  ...
  <a href="{$blog.link}" title="{$blog.name}" class="pro_column_left">
    ...
    <!--[if IE 9]><video style="display: none;"><![endif]-->
      <source
        {if $is_lazy}data-{/if}srcset="{$blog.cover.links.thumb.image|regex_replace:'/\.jpg$/':'.webp'}"
        title="{$blog.name}"
        type="image/webp"
        >
    <!--[if IE 9]></video><![endif]-->
    {/if}
    <img {if $is_lazy}data-{/if}src="{$blog.cover.links.thumb.image}" alt="{$blog.name}" width="{$blog.cover.links.thumb.width}" height="{$blog.cover.links.thumb.height}" class="{if isset($from_slider)}swiper-lazy{else}cate_pro_lazy{/if}" />
    </picture>
  </a>
  {/if}
