...
class StBlog extends Module
{
    ...
    private static $_hooks = array(
        ...
        array('displayStBlogContent','displayStBlogContent','Put content to blog content',1),      
    ...
    public function hookGSitemapAppendUrls($params)
    {
        ...
        $links[]  = array(
            ...
            'link' => $this->context->link->getModuleLink('stblog', 'default'), 
            'image' => false
        );

        Shop::addTableAssociation('st_blog', array('type' => 'shop'));
        $result = Db::getInstance()->executeS('
            SELECT cl.`link_rewrite`, b.`date_upd`,b.`id_st_blog`
            FROM `'._DB_PREFIX_.'st_blog` b
            LEFT JOIN `'._DB_PREFIX_.'st_blog_lang` cl ON (b.`id_st_blog` = cl.`id_st_blog` AND cl.`id_lang` = '.(int)$id_lang.')
            '.Shop::addSqlAssociation('st_blog','b').'
            WHERE b.`active` = 1'
        );
        foreach($result AS $row)
        {
            $url = $this->context->link->getModuleLink('stblog', 'article', array('id_st_blog'=>$row['id_st_blog'],'rewrite'=>$row['link_rewrite']), null, (int)$id_lang);
            $links[]  = array(
                'type' => 'module', 
                'page' => 'blog', 
        ...
        $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            SELECT c.`id_st_blog_category`, cl.`name`, cl.`link_rewrite`, c.`date_upd` 
            FROM `'._DB_PREFIX_.'st_blog_category` c
            LEFT JOIN `'._DB_PREFIX_.'st_blog_category_lang` cl
                ON (cl.`id_st_blog_category` = c.`id_st_blog_category` AND cl.`id_lang` = '.(int)$id_lang.')
            '.Shop::addSqlAssociation('st_blog_category','c').'
            WHERE c.`active` = 1' 
        );
        foreach($result AS $row)
        {
            $url = $this->context->link->getModuleLink('stblog', 'category', array('id_st_blog_category'=>$row['id_st_blog_category'],'rewrite'=>$row['link_rewrite']), null, (int)$id_lang);
            $links[]  = array(
                'type' => 'module', 
                'page' => 'stblogcategory', 
                'lastmod' => $row['date_upd'], 
