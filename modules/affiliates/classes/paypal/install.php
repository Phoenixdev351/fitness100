...
function getTag($inputTag, $tagArray, $branchArray)
{
    ...
    if (strpos($inputTag, '*') === 0) {
        return end($tagArray);
    } elseif (((strpos($inputTag, '>')) !== false) && (strpos($inputTag, '>') >= 0)) {
        ...
        if (strpos($inputTag, '<=') === 0) {
            ...
            foreach ($tagArray as $version) {
                if (version_compare($strippedTag, Tools::strtolower($version)) == 1 || version_compare($strippedTag, Tools::strtolower($version)) == 0) {
                    $tag = $version;
                }
            }
            ...
            $strippedTag = 'v' . str_replace('<', '', $inputTag);
            foreach ($tagArray as $version) {
                if (version_compare($strippedTag, Tools::strtolower($version)) == 1) {
                    $tag = $version;
                }
            }
            if (!in_array($tag, $tagArray)) {
...
{
    $array = array();
    $reference = Tools::jsonDecode(curlExec($url));
...
function customInstall($dependency, $installDir, &$processed)
{
    ...
    $branchUrl = sprintf(
        'https://api.github.com/repos/%s/%s/git/refs/heads/',
        $dependency['group'],
        $dependency['artifact']
    );
    ...
    if (!in_array($downloadUrl, $processed)) {
        ...
        foreach (glob('$dest/**/composer.json') as $composer) {
            $json = Tools::file_get_contents($composer);
            $json_a = Tools::jsonDecode($json, true);
...
function createAutoload()
{
    ...
    foreach ($fileList as $file) {
        ...
        $found = get_classes_defined(Tools::file_get_contents($file));

        foreach ($found as $class) {
            $class = Tools::strtolower($class);
...
<?php
    ...
    */
    class {$loaderClass}
    {
        ...
        {
            \$class = Tools::strtolower(trim(\$class, '\\\\'));
