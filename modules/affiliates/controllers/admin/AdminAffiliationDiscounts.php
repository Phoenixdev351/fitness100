...
class AdminAffiliationDiscountsController extends ModuleAdminController
{
    public function __construct()
    {
        ...
        $affiliateVouchers = Affiliation::getAffiliateVuchers();

        $this->_where .= ' AND a.`id_cart_rule` IN ('.pSQL(implode(',', $affiliateVouchers)).')';
    ...
    public function ajaxProcessLoadCartRules()
    {
        ...
        if (($page * $limit) + 1 == $count || $count == 0) {
            ...
            if ($type == 'selected') {
                ...
                if ($i == $limit) {
                    $next_link = Context::getContext()->link->getAdminLink('AdminAffiliationDiscounts').'&ajaxMode=1&ajax=1&id_cart_rule='.(int)$id_cart_rule.'&action=loadCartRules&limit='.(int)$limit.'&type=selected&count='.($count - 1 + count($cart_rules['selected']).'&search='.urlencode($search));
                }
            } else {
                $i = 1;
                ...
                if ($i == $limit) {
                    $next_link = Context::getContext()->link->getAdminLink('AdminAffiliationDiscounts').'&ajaxMode=1&ajax=1&id_cart_rule='.(int)$id_cart_rule.'&action=loadCartRules&limit='.(int)$limit.'&type=unselected&count='.($count - 1 + count($cart_rules['unselected']).'&search='.urlencode($search));
                }
            }
        }
    ...
    public function renderForm()
    {
        ...
        Media::addJsDef(array('baseHref' => $this->context->link->getAdminLink('AdminAffiliationDiscounts').'&ajaxMode=1&ajax=1&id_cart_rule='. (int)Tools::getValue('id_cart_rule').'&action=loadCartRules&limit='.(int)$limit.'&count=0'));
        $this->content .= $this->getMenu();
        $this->content .= $this->context->smarty->fetch($this->module->getLocalPath().'views/templates/admin/affiliation_discounts/form.tpl');
    ...
    protected function getMenu()
    {
        ...
        $this->context->smarty->assign('currentMenuTab', 'discounts');
