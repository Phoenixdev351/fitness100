...
{elseif $result == 1}
{assign var='signup_link' value=$link->getPageLink('authentication', true, null, ['create_account' => '1'])}
<script type="text/javascript" src="{$jQuery_path}"></script>
<script type="text/javascript">
//<![CDATA[
var discount = '';
...
var signup_link = "{$signup_link}";
var discount_type = "{$discount_type|escape:'htmlall':'UTF-8'}";
if (discount_type == 'percent') {
	...
	discount = "<span style='font-weight: bold; color: rgb(207, 25, 24); vertical-align: middle; font-size: 14px; font-style: italic;'>{$affcurrency->iso_code}{$discount|floatval|round:2}</span>";
}
...
$(document).ready(function()
{
	$.fancybox('{if isset($code) AND $code}<div id="affiliation-program" class="clearfix"></div>'+
		...
		'{if isset($welcom_message) && $welcom_message}<div class="message_content">{$welcom_message|regex_replace:"/[\r\t\n]/":" " nofilter}{*html content, cannot escaped*}{/if}<center><p class="referral_signup clearfix">'+
		'<br/><a class="button btn btn-success btn-lg" href="'+ signup_link +'">'+
