...
<div class="form-group">
	<label class="control-label  col-lg-3">{l s='Free shipping' mod='affiliates'}</label>
	<div class="col-lg-9">
...
<div class="form-group">
	<label class="control-label col-lg-3">{l s='Apply a discount' mod='affiliates'}</label>
	<div class="col-lg-9">
		<div class="radio">
		...
		<div class="radio">
			<label for="apply_discount_off">
				...
				<i class="icon-remove color_danger"></i> {l s='None' mod='affiliates'}
			</label>
		</div>
	</div>
</div>
...
<div id="apply_discount_percent_div" class="form-group">
	<label class="control-label col-lg-3">{l s='Value' mod='affiliates'}</label>
	<div class="col-lg-9">
		<div class="input-group col-lg-2">
			<span class="input-group-addon">%</span>
			<input type="text" id="reduction_percent" class="input-mini" name="reduction_percent" value="{$currentTab->getFieldValue($currentObject, 'reduction_percent')|floatval|escape:'htmlall':'UTF-8'}" />
		</div>
		<span class="help-block"><i class="icon-warning-sign"></i> {l s='Does not apply to the shipping costs' mod='affiliates'}</span>
	</div>
</div>

<div id="apply_discount_amount_div" class="form-group">
	<label class="control-label col-lg-3">{l s='Amount' mod='affiliates'}</label>
	<div class="col-lg-7">
		<div class="row">
			<div class="col-lg-4">
				<input type="text" id="reduction_amount" name="reduction_amount" value="{$currentTab->getFieldValue($currentObject, 'reduction_amount')|floatval|escape:'htmlall':'UTF-8'}" onchange="this.value = this.value.replace(/,/g, '.');" />
			</div>
			<div class="col-lg-4">
				<select name="reduction_currency" >
				{foreach from=$currencies item='currency'}
					<option value="{$currency.id_currency|intval|escape:'htmlall':'UTF-8'}" {if $currentTab->getFieldValue($currentObject, 'reduction_currency') == $currency.id_currency || (!$currentTab->getFieldValue($currentObject, 'reduction_currency') && $currency.id_currency == $defaultCurrency)}selected="selected"{/if}>{$currency.iso_code|escape:'htmlall':'UTF-8'}</option>
				{/foreach}
				</select>
			</div>
			<div class="col-lg-4">
				<select name="reduction_tax" >
					<option value="0" {if $currentTab->getFieldValue($currentObject, 'reduction_tax') == 0}selected="selected"{/if}>{l s='Tax excluded' mod='affiliates'}</option>
					<option value="1" {if $currentTab->getFieldValue($currentObject, 'reduction_tax') == 1}selected="selected"{/if}>{l s='Tax included' mod='affiliates'}</option>
				</select>
			</div>
		</div>
	</div>
...
<div id="apply_discount_to_div" class="form-group">
	<label class="control-label col-lg-3">{l s='Apply a discount to' mod='affiliates'}</label>
	<div class="col-lg-7">
		<p class="radio">
			<label for="apply_discount_to_order">
				<input type="radio" name="apply_discount_to" id="apply_discount_to_order" value="order"{if $currentTab->getFieldValue($currentObject, 'reduction_product')|intval == 0} checked="checked"{/if} />
				 {l s='Order (without shipping)' mod='affiliates'}
			</label>
		</p>
		<p class="radio">
			<label for="apply_discount_to_product">
				...
				{l s='Specific product' mod='affiliates'}
			</label>
		</p>
		<p class="radio">
			<label for="apply_discount_to_cheapest">
				<input type="radio" name="apply_discount_to" id="apply_discount_to_cheapest" value="cheapest"{if $currentTab->getFieldValue($currentObject, 'reduction_product')|intval == -1} checked="checked"{/if} />
				 {l s='Cheapest product' mod='affiliates'}
			</label>
		</p>
		<p class="radio">
			<label for="apply_discount_to_selection">
				...
				{l s='Selected product(s)' mod='affiliates'}{if $product_rule_groups|@count == 0}&nbsp;<span id="apply_discount_to_selection_warning" class="text-muted clearfix"><i class="icon-warning-sign"></i> <a href="#" id="apply_discount_to_selection_shortcut">{l s='You must select some products before' mod='affiliates'}</a></span>{/if}
			</label>
		</p>
	</div>
</div>
...
<div id="apply_discount_to_product_div" class="form-group">
	<label class="control-label col-lg-3">{l s='Product' mod='affiliates'}</label>
	<div class="col-lg-9">
		<div class="input-group col-lg-5">
			...
			<input type="hidden" id="reduction_product" name="reduction_product" value="{$currentTab->getFieldValue($currentObject, 'reduction_product')|intval|escape:'htmlall':'UTF-8'}" />
...
<div id="apply_discount_to_product_special" class="form-group">
    ...
    <span class="label-tooltip" data-toggle="tooltip"
        title="{l s='If enabled, the voucher will not apply to products already on sale.' mod='affiliates'}">
    {l s='Exclude discounted products' mod='affiliates'}
    </span>
  </label>
 	<div class="col-lg-9">
 		<span class="switch prestashop-switch fixed-width-lg">
...
<div class="form-group">
	<label class="control-label col-lg-3">{l s='Send a free gift' mod='affiliates'}</label>
	<div class="col-lg-9">
		<span class="switch prestashop-switch fixed-width-lg">
...
<div id="free_gift_div" class="form-group">
	<label class="control-label col-lg-3">{l s='Search a product' mod='affiliates'}</label>
	<div class="col-lg-9">
		<div class="input-group col-lg-5">
			<input type="text" id="giftProductFilter" value="{$giftProductFilter|escape:'htmlall':'UTF-8'}" />
			<span class="input-group-addon"><i class="icon-search"></i></span>
		</div>
	</div>
</div>
...
<div id="gift_products_found" {if $gift_product_select == ''}style="display:none"{/if}>
	<div id="gift_product_list" class="form-group">
		<label class="control-label col-lg-3">{l s='Matching products' mod='affiliates'}</label>
		<div class="col-lg-5">
			<select name="gift_product" id="gift_product" onclick="displayProductAttributes();" class="control-form">
				{$gift_product_select nofilter} {* html content *}
			</select>
		</div>
	</div>
	<div id="gift_attributes_list" class="form-group" {if !$hasAttribute}style="display:none"{/if}>
		<label class="control-label col-lg-3">{l s='Available combinations' mod='affiliates'}</label>
		<div class="col-lg-5" id="gift_attributes_list_select">
			{$gift_product_attribute_select} {* html content *}
		</div>
	</div>
</div>
