...
var copied_msg = "{l s='Copied' mod='affiliates' js=1}";
...
function copyToClipboard(element, trig) {
  var $temp = $("<input>");
  $("body").append($temp);
  $temp.val($(element).text()).select();
  document.execCommand("copy");
  $temp.remove();
  $(trig).text(copied_msg);
  $(trig).addClass('copied_color');
}
//]]>
</script>
<div id="referrals">
	...
	<div class="text form-group">
		<ul class="well" style="list-style:inside none disc;">
			<li>{l s='Share this URL to invite your referrals.' mod='affiliates'}</li>
			<li>{l s='URL' mod='affiliates'} : <strong style="word-wrap: break-word;" id="fmm_aff_link_share">{$ref_link|escape:'htmlall':'UTF-8'}</strong> <span id="fmm_aff_copier" class="" onclick="copyToClipboard('#fmm_aff_link_share', this)">{l s='Copy' mod='affiliates'}</span></li>
		</ul>
		<br/>
		<span>{include file="module:affiliates/views/templates/front/social_share.tpl"}</span>
		<br/>
		{if $refferal_coupon_state > 0}
		<div id="fmm_aff_quick_code">
			<h4 class="page-heading title_block panel">{l s='Discount code for refferals' mod='affiliates'}</h4>
			<p>{l s='You can share this Discount code with refferals to let them enjoy the discounts.' mod='affiliates'}</p>
			<div class="fmm_code_blk">
				<div class="fmm_code_blk_sub">
					<p>{l s='Refferals will get' mod='affiliates'} <strong>{if $refferal_coupon_type == 'amount'}{$refferal_coupon_curr|escape:'htmlall':'UTF-8'}{/if}{$refferal_coupon_disc|escape:'htmlall':'UTF-8'}{if $refferal_coupon_type == 'percent'}%{/if}</strong> {l s='discount. Using below code:' mod='affiliates'}</p>
					<span class="fmm_code_blk_code">{$refferal_coupon_code|escape:'htmlall':'UTF-8'}</span>
					<span class="fmm_code_blk_redflag">{l s='Do use your special link to share this voucher.' mod='affiliates'}</span>
				</div>
			</div>
		</div>
		{/if}
		...
		<div class="table-responsive">
			...
			<tbody>
				...
				<tr class="{if $smarty.section.referral.index % 2}item{else}alternate_item{/if}">
					...
					<td>
						<input type="text" class="text form-control" name="referralFirstName[{$smarty.section.referral.index|escape:'htmlall':'UTF-8'}]" size="14" value="{if isset($smarty.post.referralFirstName) AND isset($smarty.post.referralFirstName[$smarty.section.referral.index]) AND $smarty.post.referralFirstName[$smarty.section.referral.index]}{$smarty.post.referralFirstName[$smarty.section.referral.index]|escape:'htmlall':'UTF-8'}{/if}" />
					</td>
					<td>
						<input type="text" class="text form-control" name="referralLastName[{$smarty.section.referral.index|escape:'htmlall':'UTF-8'}]" size="14" value="{if isset($smarty.post.referralLastName) AND isset($smarty.post.referralLastName[$smarty.section.referral.index]) AND $smarty.post.referralLastName[$smarty.section.referral.index]}{$smarty.post.referralLastName[$smarty.section.referral.index]|escape:'htmlall':'UTF-8'}{/if}" />
					</td>
					<td>
						<input type="text" class="text form-control" name="referralEmail[{$smarty.section.referral.index|escape:'htmlall':'UTF-8'}]" size="20" value="{if isset($smarty.post.referralEmail) AND isset($smarty.post.referralEmail[$smarty.section.referral.index]) AND $smarty.post.referralEmail[$smarty.section.referral.index]}{$smarty.post.referralEmail[$smarty.section.referral.index]|escape:'htmlall':'UTF-8'}{/if}" />
					</td>
				</tr>
				{/section}
			</tbody>
	...
	{if isset($levels) AND $levels}
		{include file="module:affiliates/views/templates/front/reward-details.tpl"}
	{/if}
</div>