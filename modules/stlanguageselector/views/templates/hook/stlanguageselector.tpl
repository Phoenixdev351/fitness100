		...
		<div class="dropdown_list" aria-labelledby="{l s='Language selector' d='Shop.Theme.Transformer'}">
			<ul class="dropdown_box dropdown_list_ul">
				{foreach from=$languages key=k item=language}
					...
					<li>
						<a class="dropdown_list_item" href="{url entity='language' id=$language.id_lang}" title="{$language.name_simple}">
...
{else}
	{foreach from=$languages key=k item=language}
		{if $language.id_lang != $current_language.id_lang}
			<a href="{url entity='language' id=$language.id_lang}" title="{$language.name_simple}" class="top_bar_item language_selector">
