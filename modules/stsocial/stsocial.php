...
class StSocial extends Module implements WidgetInterface
{
    ...
    private function initButtons()
    {
        $this->social_buttons = array(
            ...
            array(
                'id' =>'reddit', 
                'name' => $this->getTranslator()->trans('Reddit', array(), 'Modules.Stsocial.Admin'),
                'title' => $this->getTranslator()->trans('Share on Reddit', array(), 'Modules.Stsocial.Admin'),
                'icon_class' => 'fto-reddit',
            ...
            array(
                'id' =>'vk', 
                'name' => $this->getTranslator()->trans('Vk', array(), 'Modules.Stsocial.Admin'),
                'title' => $this->getTranslator()->trans('Share on Vk', array(), 'Modules.Stsocial.Admin'),
                'icon_class' => 'fto-vkontakte',
            ...
            array(
                'id' =>'email', 
                'name' => $this->getTranslator()->trans('Email', array(), 'Modules.Stsocial.Admin'),
                'title' => $this->getTranslator()->trans('Email', array(), 'Modules.Stsocial.Admin'),
                'icon_class' => 'fto-mail-alt',
                'url' => 'mailto:',
                'url_key' => 'body',
                'name_key' => 'subject',
                'image_key' => '',
	...
	public function installDB()
	{
		$return = (bool)Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'st_social` (
                ...
                `new_window` tinyint(1) unsigned NOT NULL DEFAULT 1, 
	...
	{
		return Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'st_social`,`'._DB_PREFIX_.'st_social_lang`,`'._DB_PREFIX_.'st_social_shop`');
	}
	...
	public function getContent()
	{
		...
		else
		{
            ...
            $this->_html .= $helper->generateList(StSocialClass::getAll($this->context->language->id, 0), $this->fields_list);
            $helper = $this->initSettingForm();
            return $this->_html.$helper->generateForm($this->fields_form);
		}
	}
    protected function initForm()
    {
        ...
        $this->fields_form[0]['form'] = array(
            ...
            'input' => array(
                ...
                array(
                    'type' => 'text',
                    'label' => $this->getTranslator()->trans('Description:', array(), 'Admin.Theme.Transformer'),
                    'name' => 'description',
                    'lang' => true,
                    'desc' => $this->getTranslator()->trans('Description text, leave empty to use product name.', array(), 'Modules.Stsocial.Admin'),
                ),
                array(
                    'type' => 'text',
                ...
                array(
                    'type' => 'switch',
                    'label' => $this->getTranslator()->trans('New window:', array(), 'Admin.Theme.Transformer'),
                    'name' => 'new_window',
                    'is_bool' => true,
                    'default_value' => 1,
                    'values' => array(
        ...
        $this->fields_form[0]['form'] = array(
            ...
            'input' => array(
                'logo_img_field' => array(
                    'type' => 'file',
                    'label' => $this->getTranslator()->trans('Open Graph Image:', array(), 'Modules.Stsocial.Admin'),
                    'name' => 'logo_img',
                ...
                array(
                    ...
                    'values' => array(
                        array(
                            'id' => 'links_size_0',
                            'value' => 0,
                            'label' => $this->getTranslator()->trans('Small', array(), 'Admin.Theme.Transformer')),
                        array(
                            'id' => 'links_size_1',
                            'value' => 1,
    ...
    public function hookDisplayHeader($params)
    {
        ...
        if ($has_facebook) {
            ...
            if ($page_name == 'category' && $id_category = Tools::getValue('id_category'))
            {
                ...
                $image_link = $this->context->link->getCatImageLink($category->link_rewrite, $category->id_image);
                $this->smarty->assign(array('image_link'=>$image_link));
                $img = _PS_CAT_IMG_DIR_.$id_category.'.jpg';
                $this->assignImageDimension($img);
            }
            elseif ($page_name == 'manufacturer' && $id_manufacturer = Tools::getValue('id_manufacturer'))
            {
                $img = _PS_MANU_IMG_DIR_.$id_manufacturer.'.jpg';
                $object = new Manufacturer($id_manufacturer, $this->context->language->id);
                $this->smarty->assign('og_desc', $object->meta_description ? $object->meta_description : $object->short_description);
                $this->assignImageDimension($img);
            }
            elseif ($page_name == 'supplier' && $id_supplier = Tools::getValue('id_supplier'))
            {
                $img = _PS_SUPP_IMG_DIR_.$id_supplier.'.jpg';
                $object = new Supplier($id_supplier, $this->context->language->id);
                $this->smarty->assign('og_desc', $object->meta_description ? $object->meta_description : $object->description);
                $this->assignImageDimension($img);
            }
            elseif($page_name == 'article' && Tools::getValue('module') == 'stblog' && $id_st_blog = Tools::getValue('id_st_blog'))
            {
                ...
                $img = _PS_ROOT_DIR_.__PS_BASE_URI__.ltrim(strstr($cover['links']['large']['image'],'/upload/'), '/');
                $this->assignImageDimension($img);
                $this->smarty->assign(array(
                        ...
                        'og_desc' => $blog->meta_description ? $blog->meta_description : $blog->content_short
                    ));
            }
            elseif(($id_product = Tools::getValue('id_product')) && $page_name == 'list' && Tools::getValue('module') == 'stproductcomments') {
                $product = new product($id_product, false, $id_lang);
                $image_link = '';
                $w = $h = 200;
                if(($cover = Product::getCover($id_product)) && $cover['id_image'])
                {
                    $image_link = $this->context->link->getImageLink($product->link_rewrite, $cover['id_image'],'medium_default');
                    $size = Image::getSize('medium_default');
                    $w = $size['width'];
                    $h = $size['height'];
                }
                $this->smarty->assign(array(
                    'url' => $this->context->link->getModuleLink('stproductcomments', 'list', array('id_product'=>$id_product)),
                    'image_link'=> $image_link, 
                    'meta_title'=> $product->name,
                    'meta_description' => $product->meta_description,
                    'image_size'=>array('width'=>$w, 'height'=>$h)
                ));
            }
            elseif ($logo_image = Configuration::get($this->_prefix_st.'LOGO_IMG'))
            {
                $img = _PS_ROOT_DIR_._THEME_PROD_PIC_DIR_.$logo_image;
                $this->assignImageDimension($img);
                $this->smarty->assign(array('logo_image_link' => _PS_BASE_URL_._THEME_PROD_PIC_DIR_.$logo_image));
            }
            $this->smarty->assign(array('fb_app_id' => Configuration::get($this->_prefix_st.'FB_ID')));
        }
        $this->smarty->assign(array('has_facebook' => $has_facebook));
        return $this->display(__FILE__, 'header.tpl');
    }
    public function assignImageDimension($img = '')
    {
        $w = $h = 0;
        ...
        $this->smarty->assign(
            array('image_size'=>array('width'=>$w, 'height'=>$h))
        );
    }
    ...
    public function renderWidget($hookName = null, array $configuration = [])
    {
        ...
        if(Configuration::get($this->_prefix_st.'AS_DROP_DOWN') && $hookName!='displayStBlogArticleFooter')
            $template = $this->templateFile[1];
        elseif($hookName=='displayNav1' || $hookName=='displayNav2' || $hookName=='displayNav3' || $hookName=='displayTop' || $hookName=='displayHeaderCenter' || $hookName=='displayHeaderLeft' || $hookName=='displayHeaderBottom')
