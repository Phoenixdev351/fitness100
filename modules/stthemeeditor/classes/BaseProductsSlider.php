...
class BaseProductsSlider extends BaseSlider implements WidgetInterface
{
        ...
        $this->_hooks = array(
            'Hooks' => array(
                ...
                array(
                    ...
                    'val' => '1',
                    'name' => $this->getTranslator()->trans('displayOrderConfirmation1', array(), 'Admin.Theme.Transformer')
                ),
                array(
                    'id' => 'displayMiddleProduct',
                    'val' => '1',
                    'name' => $this->getTranslator()->trans('displayMiddleProduct', array(), 'Admin.Theme.Transformer')
                ),
                array(
                    'id' => 'displayProductDescRightColumn',
                    'val' => '1',
                    'name' => $this->getTranslator()->trans('Right side of Product description', array(), 'Admin.Theme.Transformer'),
                    'is_column'=>1,
                ),
            ),
            'Column' => array(
    ...
    public function initFieldsForm()
    {
        ...
        $fields['setting']['out_of_stock_on']=array(
            'type' => 'radio',
            'label' => $this->getTranslator()->trans('Don\'t show out-of-stock products', array(), 'Admin.Theme.Transformer'),
            ...
            'values' => array(
                array(
                    ...
                    'value' => 0,
                    'label' => $this->getTranslator()->trans('Show all products.', array(), 'Admin.Theme.Transformer')),
                array(
                    'id' => 'out_of_stock_on_1',
                    'value' => 1,
                    'label' => $this->getTranslator()->trans('Hide out-of-stock products.', array(), 'Admin.Theme.Transformer')),
                array(
                    'id' => 'out_of_stock_on_2',
                    'value' => 2,
                    'label' => $this->getTranslator()->trans('Hide products if their default combinations are out-of-stock.', array(), 'Admin.Theme.Transformer')),
            ),
            'validation' => 'isUnsignedInt',
        );
        $fields['setting']['title_io'] = array(
                'type' => 'radio',
                'label' => $this->getTranslator()->trans('How to display block title when this block has custom content on its left or right side:', array(), 'Admin.Theme.Transformer'),
                'name' => 'title_io',
                'default_value' => 0,
                'values' => array(
                    array(
                        'id' => 'title_io_inner',
                        'value' => 0,
                        'label' => $this->getTranslator()->trans('Above products.', array(), 'Admin.Theme.Transformer')),
                    array(
                        'id' => 'title_io_outter',
                        'value' => 1,
                        'label' => $this->getTranslator()->trans('Above this whole block.', array(), 'Admin.Theme.Transformer')),
                ),
                'validation' => 'isUnsignedInt',
                'desc' => $this->getTranslator()->trans('The Advanced custom content module can put custom content to this block. You can use this setting to decide where the title shoud be displayed.', array(), 'Admin.Theme.Transformer'),
            );
        $fields['setting']['title_bottom_margin'] = array(
                'type' => 'text',
                'label' => $this->getTranslator()->trans('Title bottom margin:', array(), 'Admin.Theme.Transformer'),
                'name' => 'title_bottom_margin',
                'validation' => 'isNullOrUnsignedId',
                'prefix' => 'px',
                'class' => 'fixed-width-lg',
                'desc' => $this->getTranslator()->trans('Set a small vaule to this field when you want to add a sub header to this block using the Advanced custom content module.', array(), 'Admin.Theme.Transformer'),
            );
    ...
    public function renderWidget($hookName = null, array $configuration = [])
    {
        if (strpos($hookName, 'display') !== false) {
            return $this->hookDisplayHome($configuration, $hookName);
        }
        return ;
    }
