...
{	
	...
	public function __construct()
	{
        ...
        $this->tabs = array(
                ...
                array('id'  => '3,27,28,65', 'name' => $this->getTranslator()->trans('Font', array(), 'Admin.Theme.Transformer')),
                ...
                array('id'  => '64,66', 'name' => $this->getTranslator()->trans('Other pages', array(), 'Admin.Theme.Transformer')),
        ...
        $this->_product_ways = array(
                array(
                    'id' => 'grid',
                    'val' => '1',
                    'name' => $this->getTranslator()->trans('Grid', array(), 'Admin.Theme.Transformer')
                ),
                array(
                    'id' => 'list',
                    'val' => '2',
                    'name' => $this->getTranslator()->trans('List', array(), 'Admin.Theme.Transformer')
                ),
                /*array(
                    'id' => 'small_list',
                    'val' => '4',
                    'name' => $this->getTranslator()->trans('List with small product images', array(), 'Admin.Theme.Transformer')
                ),*/
            );
        $this->_clear_list_view = array(
                array(
                    'id' => 'image',
                    'val' => '1',
                    'name' => $this->getTranslator()->trans('Product image', array(), 'Admin.Theme.Transformer')
                ),
                array(
                    'id' => 'desc',
                    'val' => '2',
                    'name' => $this->getTranslator()->trans('Product description', array(), 'Admin.Theme.Transformer')
                ),
                array(
                    'id' => 'add_to_cart',
                    'val' => '4',
                    'name' => $this->getTranslator()->trans('Add to cart button', array(), 'Admin.Theme.Transformer')
                ),
                array(
                    'id' => 'price',
                    'val' => '8',
                    'name' => $this->getTranslator()->trans('Price', array(), 'Admin.Theme.Transformer')
                ),
                array(
                    'id' => 'buttons',
                    'val' => '16',
                    'name' => $this->getTranslator()->trans('Buttons, like "add to wishlist", "quick view"', array(), 'Admin.Theme.Transformer')
                ),
            );
        $this->defaults = array(
            ...
            'icon_iphone_16'                         => array('exp'=>1,'val'=>'img/favicon-16.png'), 
            'icon_iphone_32'                         => array('exp'=>1,'val'=>'img/favicon-32.png'), 
            'icon_iphone_150'                         => array('exp'=>1,'val'=>'img/favicon-150.png'), 
            'icon_iphone_180'                         => array('exp'=>1,'val'=>'img/favicon-180.png'), 
            'icon_iphone_192'                         => array('exp'=>1,'val'=>'img/favicon-192.png'), 
            'icon_iphone_512'                         => array('exp'=>1,'val'=>'img/favicon-512.png'), 
            'icon_iphone_svg'                         => array('exp'=>1,'val'=>'img/favicon-svg.svg'), 
            ...
            */
            ...
            'pro_thumnbs_per_fw'                      => array('exp'=>1,'val'=>2,'smarty_val'=>1,'js_val'=>1),
            'pro_thumnbs_per_xxl'                     => array('exp'=>1,'val'=>1,'smarty_val'=>1,'js_val'=>1),
            'pro_thumnbs_per_xl'                      => array('exp'=>1,'val'=>1,'smarty_val'=>1,'js_val'=>1),
            'pro_thumnbs_per_lg'                      => array('exp'=>1,'val'=>1,'smarty_val'=>1,'js_val'=>1),
            'pro_thumnbs_per_md'                      => array('exp'=>1,'val'=>3,'smarty_val'=>1,'js_val'=>1),
            'pro_thumnbs_per_sm'                      => array('exp'=>1,'val'=>2,'smarty_val'=>1,'js_val'=>1),
            'pro_thumnbs_per_xs'                      => array('exp'=>1,'val'=>1,'smarty_val'=>1,'js_val'=>1),
	...
	public function getContent()
	{
        ...
        if($this->isEbTransPandaInstalled())
            $this->_html .= $this->displayError($this->getTranslator()->trans('You\'ve installed the Easy builder, use its front-end theme editor feature to change theme settings. Most of your changes here will not take effect anymore.', array(), 'Modules.Stthemeeditor.Admin'));
        ...
        if(Tools::getValue('act') == 'update_theme' && Tools::getValue('ajax') == 1)
        {
            $result = array(
                'r' => true,
                'm' => $this->displayConfirmation($this->getTranslator()->trans('Your theme has been upgraded, please go to the Installed modules page to upgrade modules which have a "Upgrade" button.', array(), 'Modules.Stthemeeditor.Admin')),
            );
            $m = $license->upgrade();
            if ($m !== true) {
                $result['r'] = false;
    ...
    public function generateBrowserConfig($favicon_update_time){
        if($favicon_update_time){
            if($browserconfig_icon = Configuration::get('STSN_ICON_IPHONE_150')){
                $browserconfig = '<?xml version="1.0" encoding="UTF-8"?><browserconfig><msapplication><tile><square150x150logo src="'.$this->context->link->getMediaLink(($browserconfig_icon!=$this->defaults['icon_iphone_150']['val'] ? _THEME_PROD_PIC_DIR_  : $this->_path).$browserconfig_icon).'"/></tile></msapplication></browserconfig>';
            ...
            if($icon_iphone_192 || $icon_iphone_512){
                ...
                if($icon_iphone_192)
                    $site_webmanifest['icons'][] = array(
                        'src' => $this->context->link->getMediaLink(($icon_iphone_192!=$this->defaults['icon_iphone_192']['val'] ? _THEME_PROD_PIC_DIR_  : $this->_path).$icon_iphone_192),
                        'sizes' => '192x192',
                        'type' => 'image/png',
                    );
                if($icon_iphone_512)
                    $site_webmanifest['icons'][] = array(
                        'src' => $this->context->link->getMediaLink(($icon_iphone_512!=$this->defaults['icon_iphone_512']['val'] ? _THEME_PROD_PIC_DIR_  : $this->_path).$icon_iphone_512),
                        'sizes' => '512x512',
                        'type' => 'image/png',
                    );
                $this->_checkImageDir(_PS_UPLOAD_DIR_.$this->name.'/'.$this->context->shop->id.'/');
    ...
    public function initForm()
    {
        ...
        foreach($iphone_icon_array as $v)
        {
            if ($icon = Configuration::get('STSN_ICON_IPHONE_'.strtoupper($v))) {
                $this->fields_form[18]['form']['input']['icon_iphone_'.$v.'_field']['image'] = $this->getImageHtml(($icon != $this->defaults['icon_iphone_'.$v]['val'] ? _THEME_PROD_PIC_DIR_ : $this->_path).$icon,'icon_iphone_'.$v);
            }
        }
    ...
    public function writeCss()
    {
        ...
        */    
        ...
        switch ($btn_fill_animation) {
            case 1:
                $button_hover_css .= '-webkit-box-shadow: inset 0 100px 0 0.01px '.$btn_hover_bg_color.'; box-shadow: inset 0 100px 0 0.01px '.$btn_hover_bg_color.';background-color:transparent;';
                // $primary_btn_hover_bg_color && $primary_button_hover_css .= '-webkit-box-shadow: inset 0 100px 0 0.01px '.$primary_btn_hover_bg_color.'; box-shadow: inset 0 100px 0 0.01px '.$primary_btn_hover_bg_color.';background-color:transparent;';
                $btn_white_hover .= '-webkit-box-shadow: inset 0 100px 0 0.01px #ffffff; box-shadow: inset 0 100px 0 0.01px #ffffff;background-color:transparent;';
                break;
            case 2:
                $button_hover_css .= '-webkit-box-shadow: inset 0 -100px 0 0.01px '.$btn_hover_bg_color.'; box-shadow: inset 0 -100px 0 0.01px '.$btn_hover_bg_color.';background-color:transparent;';
                // $primary_btn_hover_bg_color && $primary_button_hover_css .= '-webkit-box-shadow: inset 0 -100px 0 0.01px '.$primary_btn_hover_bg_color.'; box-shadow: inset 0 -100px 0 0.01px '.$primary_btn_hover_bg_color.';background-color:transparent;';
                ...
                break;
            case 3:
                $button_hover_css .= '-webkit-box-shadow: inset 360px 0 0 0.01px '.$btn_hover_bg_color.'; box-shadow: inset 360px 0 0 0.01px '.$btn_hover_bg_color.';background-color:transparent;';
                $res_primary_button_hover_css .= '-webkit-box-shadow: inset 800px 0 0 0.01px '.$btn_hover_bg_color.'; box-shadow: inset 800px 0 0 0.01px '.$btn_hover_bg_color.';background-color:transparent;';
                // $primary_btn_hover_bg_color && $primary_button_hover_css .= '-webkit-box-shadow: inset 360px 0 0 0.01px '.$primary_btn_hover_bg_color.'; box-shadow: inset 360px 0 0 0.01px '.$primary_btn_hover_bg_color.';background-color:transparent;';
                // $primary_btn_hover_bg_color && $res_primary_button_hover_css .= '-webkit-box-shadow: inset 800px 0 0 0.01px '.$primary_btn_hover_bg_color.'; box-shadow: inset 800px 0 0 0.01px '.$primary_btn_hover_bg_color.';background-color:transparent;';
                $btn_white_hover .= '-webkit-box-shadow: inset 360px 0 0 0.01px #ffffff; box-shadow: inset 360px 0 0 0.01px #ffffff;background-color:transparent;';
                break;
            case 4:
                $button_hover_css .= '-webkit-box-shadow: inset -360px 0 0 0.01px '.$btn_hover_bg_color.'; box-shadow: inset -360px 0 0 0.01px '.$btn_hover_bg_color.';background-color:transparent;';
                $res_primary_button_hover_css .= '-webkit-box-shadow: inset -800px 0 0 0.01px '.$btn_hover_bg_color.'; box-shadow: inset -800px 0 0 0.01px '.$btn_hover_bg_color.';background-color:transparent;';
        ...
        if ($side_panel_heading_bg = Configuration::get('STSN_SIDE_PANEL_HEADING_BG'))
            $css .= '.st-menu-title{background-color:'.$side_panel_heading_bg.';}';
        ...
        if($footer_tertiary_border = Configuration::get('STSN_FOOTER_TERTIARY_BORDER'))
            $css .='#footer-tertiary '.($footer_tertiary_border>20 ? ' #footer-secondary-container' : '').'{border-top-width:'.($footer_tertiary_border%10).'px;border-top-style: solid;}';
        if (Configuration::get('STSN_FOOTER_TERTIARY_BORDER_COLOR'))
            $css .= '#footer-tertiary, #footer-tertiary #footer-secondary-container{border-top-color:'.Configuration::get('STSN_FOOTER_TERTIARY_BORDER_COLOR').';}';
        ...
        if (Configuration::get('STSN_FOOTER_SECONDARY_CON_BG'))
			$css .= '#footer-tertiary #footer-secondary-container{background-color:'.Configuration::get('STSN_FOOTER_SECONDARY_CON_BG').';}';
        ...
        if($base_border_color = Configuration::get('STSN_BASE_BORDER_COLOR'))
        {
            ...
            .dropdown_list .dropdown_list_ul li,
                    ...
                    #footer .title_block,#footer .st_open .footer_block_content{ border-color: '.$base_border_color.'; }';
        }  
        ...
        /*else
            $css .= '.products_slider .swiper-button-lr .swiper-button.swiper-button-disabled, .products_slider .swiper-button-lr .swiper-button.swiper-button-disabled:hover{background-color:transparent;}';*/
    ...
    public function isEbTransPandaInstalled(){
        ...
        $this->has_eb_trans_panda = Module::isInstalled('steasybuilder') && Module::isEnabled('steasybuilder');
        return $this->has_eb_trans_panda;
    ...
    public function getStBasicVals(){
        ...
        foreach($iphone_icon_array as $v)
        {
            ...
            $extra_smarty_val['icon_iphone_'.$v] = $icon ? $this->context->link->getMediaLink(($icon != $this->defaults['icon_iphone_'.$v]['val'] ? _THEME_PROD_PIC_DIR_ : $this->_path).$icon) : '';
        } 
        ...
        $lang_js_val = array(
            'magnificpopup_tprev' =>  $this->getTranslator()->trans('Previous', array(), 'Shop.Theme.Transformer'),
            'magnificpopup_tnext' =>  $this->getTranslator()->trans('Next', array(), 'Shop.Theme.Transformer'),
            'magnificpopup_tcounter' =>  $this->getTranslator()->trans('%curr% of %total%', array(), 'Shop.Theme.Transformer'),
        );
    ...
    public function get_fontello()
    {
        ...
        if (file_exists(_PS_THEME_DIR_.'font-fontello/config.json')){
            $icons = \Tools::jsonDecode(\Tools::file_get_contents(_PS_THEME_DIR_.'font-fontello/config.json'));
        ...
        elseif (_PS_PARENT_THEME_DIR_ && file_exists(_PS_PARENT_THEME_DIR_.'font-fontello/config.json')){
            $icons = \Tools::jsonDecode(\Tools::file_get_contents(_PS_PARENT_THEME_DIR_.'font-fontello/config.json'));
        ...
        if($icons && is_array($icons->glyphs))
            foreach ($icons->glyphs as $icon) {
                $name = 'fto-'.str_replace('_', '-', $icon->css);
    ...
    public function calcImageWidth($option = array())
    {
        ...
        if (isset($option['for_product_gallery']) && $option['for_product_gallery']) {
            ...
            if ($thumb_width == 1 || $thumb_width == 2) {
                $thumb_width = Configuration::get('STSN_GALLERY_THUMBNAILS_WIDTH_V');
                $per_width = floor($per_width - $per_width*$thumb_width/12);
            }
            $per_width = $per_width - 30;
        } else {
