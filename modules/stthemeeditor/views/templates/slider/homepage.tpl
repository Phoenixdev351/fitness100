...
{if isset($products) && count($products)}{$pro_or_blog_slider=1}{/if}
{if isset($blogs) && count($blogs)}{$pro_or_blog_slider=2}{/if}
...
<div id="{$module}_container_{$hook_hash}" class="{$module}_container {if $hide_mob == 1} hidden-md-down {elseif $hide_mob == 2} hidden-lg-up {/if} block {if $pro_or_blog_slider==1 && $countdown_on} s_countdown_block {/if} {if !$column_fix && $has_background_img && $speed} st_parallax_block {/if} products_container{$column_fix} {if $column_slider && !isset($is_quarter)} column_block {/if} {if !$column_fix && $video_mpfour} video_bg_block {/if}" 
