...
class StVideo extends Module implements WidgetInterface
{
	...
	public function installDb()
	{
		$return = (bool)Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'st_video` (
                ...
                `offset_x` varchar(10) DEFAULT NULL,
                `offset_y` varchar(10) DEFAULT NULL,
	...
	{        
        ...
        if (Tools::getValue('ajax') && Tools::getValue('act') == 'changeProductVideo' ) {
	       ...
	       if ($id_product=Tools::getValue('id_product')) {
               $id_st_video = StVideoClass::changeProductVideo($id_product, array('url'=>Tools::getValue('url'),'video_position'=>Tools::getValue('video_position')), 0);
                ...
                case 1:
                    if ($id_product = Tools::getValue('id_product')) {
                        ...
                        /*foreach($id_product AS $id) {
                            if (!StVideoClass::getByIdProduct($id, $video->type)) {
                                $id_product_arr[] = $id;
                            }
                        }
                        ...
                        if ($id_product_arr = array_unique($id_product)) {
    ...
    {   
        if (!$this->isCached($this->templatePath.'header.tpl', $this->getCacheId()))
        {
            ...
            $icon_width = (int)Configuration::get($this->_prefix_st.'ICON_WIDTH');
            if($icon_width)
                $custom_css .= '.st_popup_video{with:'.$icon_width.';}';
            else
                $icon_width = 30;
            $icon_height = (int)Configuration::get($this->_prefix_st.'ICON_HEIGHT');
            if($icon_height)
            ...
            $custom_css_arr = StVideoClass::getAll(0,null,1);
            foreach ($custom_css_arr as $v) {
                if(($v['video_position']!=9 && isset($v['offset_x']) && isset($v['offset_y'])) || ($v['video_position']==9 && isset($v['offset_x']) && isset($v['offset_y']) && ($v['offset_x'] || $v['offset_x']==='0' || $v['offset_y'] || $v['offset_y']==='0')))
                {
                    switch ($v['video_position']) {
                        case 1:
                            $custom_css .= '.st_popup_video_'.$v['id_st_video'].'{left:'.(int)$v['offset_x'].'px;right:auto;}';
                            $custom_css .= '.st_popup_video_'.$v['id_st_video'].'{top:'.(int)$v['offset_y'].'px;bottom:auto;}';
                            break;
                        case 2:
                            $custom_css .= '.st_popup_video_'.$v['id_st_video'].'{left:50%;margin-left:'.((int)$v['offset_x']-$icon_width/2).'px;right:auto;}';
                            $custom_css .= '.st_popup_video_'.$v['id_st_video'].'{top:'.(int)$v['offset_y'].'px;bottom:auto;}';
                            break;
                        case 3:
                            $custom_css .= '.st_popup_video_'.$v['id_st_video'].'{right:'.(int)$v['offset_x'].'px;left:auto;}';
                            $custom_css .= '.st_popup_video_'.$v['id_st_video'].'{top:'.(int)$v['offset_y'].'px;bottom:auto;}';
                            break;
                        case 4:
                            $custom_css .= '.st_popup_video_'.$v['id_st_video'].'{left:'.(int)$v['offset_x'].'px;right:auto;}';
                            $custom_css .= '.st_popup_video_'.$v['id_st_video'].'{top:50%;margin-top:'.((int)$v['offset_y']-$icon_height/2).'px;bottom:auto;}';
                            break;
                        case 5:
                            $custom_css .= '.st_popup_video_'.$v['id_st_video'].'{left:50%;margin-left:'.((int)$v['offset_x']-$icon_width/2).'px;right:auto;}';
                            $custom_css .= '.st_popup_video_'.$v['id_st_video'].'{top:50%;margin-top:'.((int)$v['offset_y']-$icon_height/2).'px;bottom:auto;}';
                            break;
                        case 12:
                            $custom_css .= '.st_popup_video_'.$v['id_st_video'].'{left:'.(int)$v['offset_x'].'px;right:'.(int)$v['offset_x'].'px;margin-left: auto;margin-right:auto;right:auto;}';
                            $custom_css .= '.st_popup_video_'.$v['id_st_video'].'{top:50%;margin-top:'.((int)$v['offset_y']-$icon_height/2).'px;bottom:auto;}';
                            break;
                        case 6:
                            $custom_css .= '.st_popup_video_'.$v['id_st_video'].'{right:'.(int)$v['offset_x'].'px;left:auto;}';
                        ...
                        case 7:
                            $custom_css .= '.st_popup_video_'.$v['id_st_video'].'{left:'.(int)$v['offset_x'].'px;right:auto;}';
                            $custom_css .= '.st_popup_video_'.$v['id_st_video'].'{bottom:'.(int)$v['offset_y'].'px;top:auto;}';
                            break;
                        case 8:
                            $custom_css .= '.st_popup_video_'.$v['id_st_video'].'{left:50%;margin-left:'.((int)$v['offset_x']-$icon_width/2).'px;right:auto;}';
                            $custom_css .= '.st_popup_video_'.$v['id_st_video'].'{bottom:'.(int)$v['offset_y'].'px;top:auto;}';
    ...
    public function hookDisplayAdminProductsExtra($params)
    {
        ...
        $this->smarty->assign(array(
            ...
            'video_position' => array_merge(self::$video_position, self::$size_chart_position),
