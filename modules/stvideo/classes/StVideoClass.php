...
class StVideoClass extends ObjectModel
{
	...
	public static $definition = array(
		...
		'fields'    => array(
            ...
            'offset_x'     => array('type' => self::TYPE_STRING, 'size' => 10),
            'offset_y'  => array('type' => self::TYPE_STRING, 'size' => 10),
    ...
    public static function changeProductVideo($id_product = 0, $data=array(), $type=0)
    {
        if (!$id_product) {
            return false;
        }
        $url = trim($data['url']);
        $video_position = (int)$data['video_position'];
        if(!$video_position)
            $video_position = 9;
        ...
        if ($row = self::getByIdProduct($id_product, (int)$type)) {
            ...
            if (!trim($id_products,',')) {
                if (!$url) {
                    ...
                    $video->url = pSQL($url);
                    $video->video_position = $video_position;
