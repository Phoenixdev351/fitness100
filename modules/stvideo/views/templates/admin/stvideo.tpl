    ...
    <div class="row">
        <div class="col-xl-12 col-lg-12">
          <fieldset class="form-group">
                <input type="text" id="video_url" name="video_url" class="tokenfield form-control" value="{if $video}{$video.url}{/if}" />
          </fieldset>
        </div>
    </div>
    <div class="row">
        <div class="col-xl-9 col-lg-8">
          <fieldset class="form-group">
                <select name="video_position" id="video_position" class="form-control">
                    {foreach $video_position as $vp}
                    <option value="{$vp.id}" {if $video && $vp.id==$video.video_position} selected="selected" {/if}>{$vp.name}</option>
                    {/foreach}
                </select>
          </fieldset>
        </div>
