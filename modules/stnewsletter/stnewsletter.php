...
class StNewsLetter extends Module implements WidgetInterface
{
    ...
    public static $location = array(
        ...
        118 => array('id' =>118 , 'name' => 'Blog left column', 'hook' => 'StBlogLeftColumn', 'column'=>1),
        119 => array('id' =>119 , 'name' => 'Blog right column', 'hook' => 'StBlogRightColumn', 'column'=>1),
    ...
    private function initPages()
    {
        $this->_pages = array(
                ...
                array(
                    'id' => 'module-stblog-default',
                    'val' => '1024',
                    'name' => $this->getTranslator()->trans('Blog homepage', array(), 'Admin.Theme.Transformer')
                ),
                array(
                ...
                array(
                    ...
                    'val' => '4096',
                    'name' => $this->getTranslator()->trans('Blog post', array(), 'Admin.Theme.Transformer')
                ),
                array(
                    'id' => 'authentication',
                    'val' => '8192',
                    'name' => $this->getTranslator()->trans('Login and registration', array(), 'Admin.Theme.Transformer')
                ),
            );
    }
	...
	public function install()
	{
        $res = parent::install() &&
            ...
            Configuration::updateValue($this->_prefix_st.'UNIQUE_VOUCHER', 0) &&
            Configuration::updateValue($this->_prefix_st.'VALIDITY', '7') &&
            Configuration::updateValue($this->_prefix_st.'USER_ONLY', 0) &&
        ...
        Configuration::updateValue($this->_prefix_st.'NW_CONDITIONS', $conditions, true);
        $this->prepareHooks();
    ...
    public function installDB()
    {
        ...
        if(!is_array($field) || !count($field)) {
            Db::getInstance()->Execute('ALTER TABLE `'._DB_PREFIX_.'emailsubscription` ADD `id_gender` int(10) unsigned NOT NULL DEFAULT 0');    
        }
        $field = Db::getInstance()->executeS('Describe `'._DB_PREFIX_.'emailsubscription` `voucher_sent`');
        if(!is_array($field) || !count($field)) {
            Db::getInstance()->Execute('ALTER TABLE `'._DB_PREFIX_.'emailsubscription` ADD `voucher_sent` tinyint(1) unsigned NOT NULL DEFAULT 0');    
        }
    ...
    public function getContent()
    {
        ...
        if (isset($_POST['savesettingstnewsletter']) || isset($_POST['savesettingstnewsletterAndStay']))
        {
            ...
            else 
            {
                ...
                $languages = Language::getLanguages(false);
                foreach ($languages as $lang) {
                    $conditions[(int) $lang['id_lang']] = Tools::getValue('nw_conditions_'.$lang['id_lang']);
                }
                Configuration::updateValue($this->_prefix_st.'NW_CONDITIONS', $conditions, true);
                $this->clearStNewsLetterCache();
    ...
    public static function showApplyTo($value,$row)
    {
        if(isset(self::$location[$value]))
            ...
            return Module::isInstalled('jscomposer') && Module::isEnabled('jscomposer') ? $context->getTranslator()->trans('For visual composer module', array(), 'Admin.Theme.Transformer') : $context->getTranslator()->trans('--', array(), 'Admin.Theme.Transformer');
        }
    }
    ...
    {        
        ...
        $this->fields_form[0]['form'] = array(
            ...
            'input' => array(
                ...
                array(
                    'type' => 'radio',
                    'label' => $this->getTranslator()->trans('On mobile footer:', array(), 'Admin.Theme.Transformer'),
                    'name' => 'hide_on_mobile',
                    'default_value' => 0,
                    'values' => array(
                        array(
                        ...
                        array(
                            'id' => 'hide_on_mobile_2',
                            'value' => 2,
                            'label' => $this->getTranslator()->trans('Open', array(), 'Admin.Theme.Transformer')),
                        array(
                            'id' => 'hide_on_mobile_off',
                            'value' => 0,
                            'label' => $this->getTranslator()->trans('Collapse', array(), 'Admin.Theme.Transformer')),
                    ),
    ...
    public function initFieldsForm()
    {
        $this->fields_form[0]['form'] = array(
            ...
            'input' => array(
                ...
                array(
                    'type' => 'switch',
                    'label' => $this->getTranslator()->trans('Create a unique voucher code for each subscriber?', array(), 'Modules.Stnewsletter.Admin'),
                    'name' => 'unique_voucher',
                    'values' => array(
                        array(
                            'id' => 'unique_voucher_on',
                            'value' => 1,
                            'label' => $this->getTranslator()->trans('Yes', array(), 'Admin.Theme.Transformer')
                        ),
                        array(
                            'id' => 'unique_voucher_off',
                            'value' => 0,
                            'label' => $this->getTranslator()->trans('No', array(), 'Admin.Theme.Transformer')
                        )
                    ),
                    'desc' => $this->getTranslator()->trans('Create a voucher code for each subscriber base on given voucher code.', array(), 'Admin.Theme.Transformer'),
                    'validation' => 'isBool',
                ),
                array(
                    'type' => 'switch',
                    'label' => $this->getTranslator()->trans('Send voucher to registered user only', array(), 'Modules.Stnewsletter.Admin'),
                    'name' => 'user_only',
                    'values' => array(
                        array(
                            'id' => 'user_only_on',
                    ...
                    'desc' => $this->getTranslator()->trans('Send a voucher to registered user to restrict the voucher, just the user can use the voucher. if you disable the option, anyone can apply the voucher if he / she gets it.', array(), 'Admin.Theme.Transformer'),
                    'validation' => 'isBool',
                ),
                array(
                    'type' => 'text',
                    'label' => $this->getTranslator()->trans('Voucher code validity', array(), 'Modules.Stnewsletter.Admin'),
                    'name' => 'validity',
                    'suffix' => 'days',
                    'class' => 'fixed-width-md',
                    'desc' => $this->getTranslator()->trans('How many days the voucher code is valid.', array(), 'Admin.Theme.Transformer'),
                    'validation' => 'isString',
                ),
                array(
                    'type' => 'textarea',
                    'label' => $this->getTranslator()->trans('Newsletter conditions', array(), 'Modules.Stnewsletter.Admin'),
                    'lang' => true,
                    'name' => 'nw_conditions',
                    'cols' => 40,
                    'rows' => 100,
                    'hint' => $this->getTranslator()->trans(
                        ...
                        array(),
                        'Modules.Stnewsletter.Admin'
                    ),
                    'desc' => $this->getTranslator()->trans('Leave blank to disable by default.', array(), 'Modules.Stnewsletter.Admin'),
                ),
            ),
			'submit' => array(
    ...
    { 
        $fields_values = array(
            ...
            'unique_voucher'              => Configuration::get($this->_prefix_st.'UNIQUE_VOUCHER'),
            'validity'                    => Configuration::get($this->_prefix_st.'VALIDITY'),
            'user_only'                   => Configuration::get($this->_prefix_st.'USER_ONLY'),  
        ...
        $languages = Language::getLanguages(false);
        foreach ($languages as $lang) {
            $conditions[$lang['id_lang']] = Configuration::get($this->_prefix_st.'NW_CONDITIONS', $lang['id_lang']);
        }
    ...
    public function hookDisplayHeader($params)
    {
        Media::addJsDef(array(
            'wrongemailaddress_stnewsletter' => $this->getTranslator()->trans('Invalid email address', array(), 'Shop.Theme.Transformer'),
        ));
            ...
            if(is_array($options) && count($options))
                foreach($options as $v)    
                {
                    ...
                    if(isset($v['top_padding']) && ($v['top_padding'] || $v['top_padding']==='0'))
                        $custom_css .= $classname.',#footer '.$classname.'{margin-top:'.(int)$v['top_padding'].'px;}';
                    if(isset($v['bottom_padding']) && ($v['bottom_padding'] || $v['bottom_padding']==='0'))
                        $custom_css .= $classname.',#footer '.$classname.'{margin-bottom:'.(int)$v['bottom_padding'].'px;}';
    ...
    */
    ...
    {        
        ...
        if(Context::getContext()->controller->controller_type=='modulefront')
            $page = Context::getContext()->smarty->getTemplateVars('page')['page_name'];
        if(is_array($news_letter_array) && count($news_letter_array))
            foreach($news_letter_array as $k => &$v)
            {
                if($v['location']==4)
                {
                    ...
                    if (isset($_COOKIE['st_popup_do_not_show_'.$v['id_st_news_letter']]) && preg_match('/\d+/Us', $_COOKIE['st_popup_do_not_show_'.$v['id_st_news_letter']], $ma)) {
                        if ($ma[0] == $v['show_popup']) {
                            unset($news_letter_array[$k]);
                            continue;
                        }
                    }
                ...
                if(!is_null($vc) && isset($vc['is_full_width']) && $vc['is_full_width'])
                    $v['is_full_width'] = true;
                $v['is_column'] = isset(self::$location[$v['location']]['column']) ? true : false;
                if(!is_null($vc) && isset($vc['is_column']) && $vc['is_column'])
                    $v['is_column'] = true;
                ...
                if(!is_null($vc) && isset($vc['is_stacked_footer']) && $vc['is_stacked_footer'])
                    $v['is_stacked_footer'] = true;
                $v['span'] = isset(self::$location[$v['location']]['span']) ? self::$location[$v['location']]['span'] : 3;
            }
    ...
    {
        $id_st_news_letter = isset($params['id_st_news_letter']) ? (int)$params['id_st_news_letter'] : null;
        if(!is_null($id_st_news_letter))
        {
            $params['is_full_width'] = true;
        }
        elseif(Dispatcher::getInstance()->getController()!='index')
            return false;
    ...
    {
        return $this->hookDisplayHome($params, 17);
    }
    public function hookDisplayHome($params, $location=array())
    {
        $location = $location ? $location : 1;
        $id_st_news_letter = isset($params['id_st_news_letter']) ? (int)$params['id_st_news_letter'] : null;
        $cache_id = (is_array($location) ? implode('', $location) : $location).(string)$id_st_news_letter;
        if (!$this->isCached($this->templateFile, $this->stGetCacheId($cache_id)))
        {
            if(!is_null($id_st_news_letter))
            {
                $vc = array();
                if(isset($params['is_full_width']) && $params['is_full_width'])
                    $vc['is_full_width'] = true;
                $this->_prepareHook($id_st_news_letter,100,$vc);
            }
            else
    ...
    public function hookDisplayBlogLeftColumn($params)
    {
        return $this->hookDisplayHome($params, 118);
    }
    public function hookDisplayStBlogRightColumn($params)
    {
        return $this->hookDisplayHome($params, 119);
    }
    ...
    public function hookDisplayFooter($params, $location=array())
    {
        ...
        if (!$this->isCached($tpl, $this->stGetCacheId($cache_id)))
        {
            if(!is_null($id_st_news_letter))
            {
                $this->_prepareHook($id_st_news_letter,100,array('is_stacked_footer'=>true));
            }
            else
                $this->_prepareHook($location);
        }
    ...
    {
        $nw_conditions = Configuration::get($this->_prefix_st.'NW_CONDITIONS', $this->context->language->id);
    ...
    {
        // Send voucher to registered user only.
        $id_customer = 0;
        if (Configuration::get($this->_prefix_st.'USER_ONLY')) {
            if (!$id_customer = Db::getInstance()->getValue('SELECT `id_customer` FROM `' . _DB_PREFIX_ . 'customer` WHERE `email` = "' . pSQL($email) . '"')) {
                return false;
            }
        }
        // Don't send duplicated voucher.
        if (Db::getInstance()->getValue('SELECT `voucher_sent` FROM `' . _DB_PREFIX_ . 'emailsubscription` WHERE `email` = "' . pSQL($email) . '"')) {
            return false;
        }
        ...
        if (!$code = $this->createVoucher($code, $id_customer)) {
            return false;
        }
        $ret = Mail::Send(
        ...
        if ($ret) {
           $ret = Db::getInstance()->execute('UPDATE `' . _DB_PREFIX_ . 'emailsubscription` SET `voucher_sent` = 1 WHERE `email` = "' . pSQL($email) . '"');
        }
        return $ret;
    }

    public function createVoucher($code, $id_customer = 0)
    {
        if ($code && Configuration::get($this->_prefix_st.'UNIQUE_VOUCHER')) {
            //voucher code
            if(!$id_cart_rule = CartRule::getIdByCode($code)) {
                return false;
            }
            $cart_rule_base = new CartRule($id_cart_rule);

            $code_voucher   = 'STNSV-'.Tools::strtoupper(Tools::passwdGen(10));
            $discount_name  = array();
            $languages      = Language::getLanguages(false);

            foreach ($languages as $language) {
                $discount_name[$language['id_lang']] = $cart_rule_base->name[$language['id_lang']].'-NS';
            }

            $cart_rule = clone $cart_rule_base;
            $cart_rule->id_cart_rule= 0;
            $cart_rule->id          = 0;
            $cart_rule->active      = 1;
            $cart_rule->code        = $code_voucher;
            $cart_rule->name        = $discount_name;
            $cart_rule->id_customer = $id_customer;

            if ($days = (int)Configuration::get($this->_prefix_st.'VALIDITY')) {
                $date_validity       = strtotime('+'.$days.' day');
                $date_validity_final = strftime('%Y-%m-%d', $date_validity);

                $cart_rule->date_to               = $date_validity_final;
    ...
    private function getConditionFixtures($lang)
    {
        ...
        return
            $this->trans('You may unsubscribe at any moment. For that purpose, please find our contact info in the legal notice.', array(), 'Shop.Theme.Transformer')
        ;
    }
    ...
    public function vc_shortcode_init($atts, $content = null)
    {
        ...
        if (method_exists($this, $hook)) {
            return $this->$hook(array('st_time'=>$st_time,'id_st_news_letter'=>$id_st_news_letter));
        }
    }
    function vc_map_init()
    {
        $content = array();
        $default = 0;
        foreach(StNewsLetterClass::getNewsLetter($this->context->language->id, 0, 1) AS $value) {
            $content[$this->getTranslator()->trans('ID:', array(), 'Admin.Theme.Transformer').$value['id_st_news_letter']] = $value['id_st_news_letter'];
            !$default && $default = $value['id_st_news_letter'];
        }
        $vc_hooks = array();
    ...
    public function hookDisplayAdminCustomersForm()
    {
        ...
        $input = array(
            'type' => 'switch',
            'label' => $this->trans('Newsletter', array(), 'Modules.Stnewsletter.Admin'),
            'name' => 'newsletter',
            'required' => false,
            'class' => 't',
            'is_bool' => true,
            ...
            'hint' => $this->trans('This customer will receive your newsletter via email.', array(), 'Modules.Stnewsletter.Admin'),
        );
    ...
    public function hookActionDeleteGDPRCustomer($customer)
    {
        if (!empty($customer['email']) && Validate::isEmail($customer['email'])) {
            ...
            return json_encode($this->getTranslator()->trans('Newsletter subscription: Unable to delete customer using email.', array(), 'Shop.Theme.Transformer'));
        }
    }
    public function hookActionExportGDPRData($customer)
    {
        if (!Tools::isEmpty($customer['email']) && Validate::isEmail($customer['email'])) {
            ...
            return json_encode($this->getTranslator()->trans('Newsletter subscription: No data found.', array(), 'Shop.Theme.Transformer'));
       }
   }
}