...
class StNewsLetterClass extends ObjectModel
{
    ...
    public static function getNewsLetter($id_lang, $identify, $type=1)
    {
        ...
        if($type==100)
            $where .= ' AND snl.`id_st_news_letter` IN ('.implode(',',$identify).')';
        if ($type && !$where) {
            return false;
        }
	...
	public function registerUser($email)
    {
        $sql = 'UPDATE '._DB_PREFIX_.'customer
                SET `newsletter` = 1, newsletter_date_add = NOW(), `ip_registration_newsletter` = \''.pSQL(Tools::getRemoteAddr()).'\', id_gender='.(int)Tools::getValue('id_gender', 0).'
    ...
    {
        $sql = 'INSERT INTO '._DB_PREFIX_.'emailsubscription (id_shop, id_shop_group, email, newsletter_date_add, ip_registration_newsletter, http_referer, active, id_gender)
                VALUES
                ...
                '.(int) $active.',
                '.(int)Tools::getValue('id_gender', 0).'
                )';
