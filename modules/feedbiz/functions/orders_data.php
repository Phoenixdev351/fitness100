...
class FeedBizGetOrders extends Feedbiz
{
    public $debug   = false;
    ...
    /**
     * Exports settings to Feed.biz and the inverse
     */
    public function dispatch()
    {
        ...
        $params = array();
        $params['order_history'] = new StdClass();
        $params['order_history']->id_order = Tools::getValue('id_order');

        $amazon_features = unserialize(Configuration::get('FEEDBIZ_AMAZON_FEATURES'));

        if (!$amazon_features instanceof StdClass) {
            if (FeedBiz::$debug_mode) {
                echo Tools::displayError(sprintf('%s: %s::%s(%d) - Unavailable: Amazon Features', basename(__FILE__), __CLASS__, __FUNCTION__, __LINE__));
            }
            return;
        }

        if (!(property_exists($amazon_features, 'has_fba') && $amazon_features->has_fba == true && property_exists($amazon_features, 'has_fba_multichannel') && $amazon_features->has_fba_multichannel == true)) {
            if (FeedBiz::$debug_mode) {
                echo Tools::displayError(sprintf('%s: %s::%s(%d) - FBA Multichannel is not active', basename(__FILE__), __CLASS__, __FUNCTION__, __LINE__));
            }
            return (false);
        }
        ...
        if (!isset($params['order_history']->id_order)) {
            if (FeedBiz::$debug_mode) {
                echo Tools::displayError(sprintf('%s: %s::%s(%d) - Empty id_order', basename(__FILE__), __CLASS__, __FUNCTION__, __LINE__));
            }
            return (false);
        }
        ...
        if (!Validate::isLoadedObject($order)) {
            if (FeedBiz::$debug_mode) {
                echo Tools::displayError(sprintf('%s: %s::%s(%d) - Unable to load Order id: %d', basename(__FILE__), __CLASS__, __FUNCTION__, __LINE__, $id_order));
            }
            return (false);
        }
        ...
        if (!is_array($ordered_products) || !count($ordered_products)) {
            if (FeedBiz::$debug_mode) {
                echo Tools::displayError(sprintf('%s: %s::%s(%d) - No products belonging to this order: %d', basename(__FILE__), __CLASS__, __FUNCTION__, __LINE__, $id_order));
            }
            return (false);
        }
        ...
        if (!$pass) {
            if (FeedBiz::$debug_mode) {
                echo Tools::displayError(sprintf('%s: %s::%s(%d) - One or more products are not eligible for multichannel because of a missing reference: %d', basename(__FILE__), __CLASS__, __FUNCTION__, __LINE__, $id_order));
            }
            return (false);
        }
        ...
        foreach (array(
            ...
            if (empty($order->{$required_id})) {
                if (FeedBiz::$debug_mode) {
                    echo Tools::displayError(sprintf('%s: %s::%s(%d) - Invalid value for %s: "%s"', basename(__FILE__), __CLASS__, __FUNCTION__, __LINE__, $required_id, $order->{$required_id}));
                }
                return (false);
            }
        }
        ...
        foreach (array(
            ...
            if (!Validate::isLoadedObject($$required_class)) {
                if (FeedBiz::$debug_mode) {
                    echo Tools::displayError(sprintf('%s: %s::%s(%d) - Unable to load instance "%s", validation failed', basename(__FILE__), __CLASS__, __FUNCTION__, __LINE__, $required_class));
                }
                return (false);
            }
        }
        ...
        $OrderDom->setAttribute('ShopName', Configuration::get('PS_SHOP_NAME', null, null, $this->context->shop->id));
        $OrderDom->setAttribute('IdShop', $this->context->shop->id);
    ...
    /**
     * Ends the program displaying an XML error
     */
...
$feedBizgetorders = new FeedBizGetOrders();
$feedBizgetorders->dispatch();
