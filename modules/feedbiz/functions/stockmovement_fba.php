...
class FeedbizStockMovement extends Feedbiz
{
    ...
    public function dispatch()
    {
        ...
        $id_product = Tools::getValue('fbproduct');
        $id_combination = Tools::getValue('fbcombination');
        $stock_fba = Tools::getValue('fbstock_fba');
        $stock = Tools::getValue('fbstock');
        $sku = Tools::getValue('fbsku');
        ...
        if (Validate::isLoadedObject($productObj) && $combinationValid) {
            ...
            if ((bool)Configuration::get('PS_ADVANCED_STOCK_MANAGEMENT') && StockAvailable::dependsOnStock($id_product) == 1) {
                if (version_compare(_PS_VERSION_, '1.5', '>=')) {
                    $currentQty = Product::getRealQuantity(
                        $id_product,
                        $id_combination ? $id_combination : null,
                        $id_warehouse,
                        $id_shop
                    );
                    ...
                    if ($id_warehouse > 0) {
                        ...
                        FeedBizLog::log(FeedBizLog::FILE_LOG_STOCK_MOVEMENT_FBA, $log);
            ...
            $sold = $stock_fba - $currentQty;

            if ($sold <> 0) {
                // update stock

                if (version_compare(_PS_VERSION_, '1.5', '>=')) {
                    $out = StockAvailable::updateQuantity($id_product, $id_combination, $sold);
                } else {
                    $out = Product::updateQuantity(array(
                        'id_product' => $id_product,
                        'id_product_attribute' => $id_combination,
                        'cart_quantity' => (-1 * $sold),
                        'out_of_stock' => 0
                    ));
                }
            ...
            $this->return_dom[] = array(
                'flagged' => (!$out) ? false : true,
                'id_product' => (int)$id_product,
                'id_product_attribute' => (int)$id_combination,
                'stock_fba' => (string)$stock_fba,
                'stock' => (string)$stock,
                'sku' => (string)$sku,
                'result' => (string)$out
            );
    ...
    public function FBShutdowFunction()
    {
        ...
        $StockFlagPackage = $Document->appendChild($Document->createElement('Result'));
        $StockFlagPackage->setAttribute('ShopName', Configuration::get('PS_SHOP_NAME'));
        $StockFlagPackage->appendChild($StatusDoc = $Document->createElement('Status', ''));
        ...
        if (!empty($this->return_dom)) {
            $StockFlagPackage->appendChild($StockFlagDom = $Document->createElement('StockFlag', ''));
            foreach ($this->return_dom as $item) {
                $itemDom = $StockFlagDom->appendChild($Document->createElement('Item'));
                $itemDom->setAttribute('ProductId', $item['id_product']);
                $itemDom->setAttribute('ProductAttributeId', $item['id_product_attribute']);
                $itemDom->setAttribute('Reference', $item['sku']);
                $itemDom->setAttribute('Flagged', $item['flagged']);
                $itemDom->setAttribute('StockFBA', $item['stock_fba']);
                $itemDom->setAttribute('Stock', $item['stock']);
            }
        }
