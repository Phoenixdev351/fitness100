...
class FeedbizTools extends Feedbiz
{
    ...
    public static $feed_config = array();
    ...
    public static function securityCheck()
    {
        ...
        if (!$security_passed) {
            ...
            $domain = Configuration::get('FEEDBIZ_DOMAIN');
            if (empty($domain)) {
                Configuration::updateValue('FEEDBIZ_DOMAIN', 'https://'.$host);
                $domain = Configuration::get('FEEDBIZ_DOMAIN');
            }
...
//                $code = '03';
     ...
     * @return bool
     */
    public static function getConfig($debug = false)
    {
        $sql = 'SELECT * FROM `'._DB_PREFIX_.'configuration` WHERE name like "FEEDBIZ%";';
        $result = Db::getInstance()->executeS($sql);
        foreach ($result as $c) {
            if (empty(self::$feed_config[$c['name']])) {
                self::$feed_config[$c['name']] = $c['value'];
            }
        }
        if ($debug) {
            echo "<pre>";
