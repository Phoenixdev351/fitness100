...
class FeedBizProductTabCdiscount extends FeedBizProductTab
{
    /** @var array */
    ...
    /**
     * FeedBizProductTabCdiscount constructor.
     *
    ...
    /**
     * Generate product tab for Cdiscount
     *
     * @param $product
     * @param $combinations
     *
    ...
    public static function getProductOptionFields()
    {
        ...
        $option_fields_config = Configuration::get('FEEDBIZ_OPTION_FIELDS_CDISCOUNT', null, 0, 0);
    ...
    public function countrySelector()
    {
        ...
        foreach (Feedbiz::$cdiscount_regions as $domain => $region) {
            ...
            $marketplaces[$region]['name'] = sprintf('Cdiscount %s', FeedbizTools::cdiscountRegionToDomain($region));
    ...
    public static function getProductOptions($id_product, $id_product_attribute = null, $region = null)
    {
        ...
        $sql = 'SELECT *
                FROM `'._DB_PREFIX_.Feedbiz::TABLE_FEEDBIZ_CDISCOUNT.'` p
    ...
    public function productOptionsDetails($region, $defaults)
    {
        ...
        $view_params['alignment'] = array();
        if (in_array('price_up', self::$field_list)) {
            $view_params['alignment']['price_up'] = array(
                'default' =>
                    isset($defaults['price_up']) && is_numeric($defaults['price_up']) && $defaults['price_up'] > 0 ?
                        (float)$defaults['price_up'] : '',
                'region' => $region
            );
        }

        if (in_array('price_down', self::$field_list)) {
            $view_params['alignment']['price_down'] = array(
                'default' =>
                    isset($defaults['price_down']) && is_numeric($defaults['price_down']) && $defaults['price_down'] > 0 ?
                        (float)$defaults['price_down'] : '',
                'region' => $region
            );
        }
        ...
        if (in_array('shipping_tracked_override', self::$field_list)) {
            $view_params['shipping_tracked_override'] = array(
                'default' =>
                    isset($defaults['shipping_tracked_override']) && (int)$defaults['shipping_tracked_override'] > 0 ? (float)$defaults['shipping_tracked_override'] : '',
                'region' => $region
            );
        }

        if (in_array('shipping_registered_override', self::$field_list)) {
            $view_params['shipping_registered_override'] = array(
                'default' =>
                    isset($defaults['shipping_registered_override']) && (int)$defaults['shipping_registered_override'] > 0 ? (float)$defaults['shipping_registered_override'] : '',
                'region' => $region
            );
        }

        if (in_array('shipping_delay', self::$field_list)) {
            $view_params['shipping_delay_overrides'] = array(
                'default' => isset($defaults['shipping_delay']) && (int)$defaults['shipping_delay'] > 0 ?
                    $defaults['shipping_delay'] : null,
                'region' => $region
            );
        }

        if (in_array('clogistique', self::$field_list)) {
            $view_params['clogistique'] = array(
                'default' => isset($defaults['clogistique']) && (bool)$defaults['clogistique'],
                'region' => $region
            );
        }

        if (in_array('valueadded', self::$field_list)) {
            $view_params['valueadded'] = array(
                'default' =>
                    isset($defaults['valueadded']) && is_numeric($defaults['force']) && $defaults['valueadded'] > 0 ?
                        (float)$defaults['valueadded'] : '',
                'region' => $region
            );
        }

        if (in_array('text', self::$field_list)) {
            $view_params['extra_text'] = array(
                'default' => isset($defaults['text']) && $defaults['text'] ? $defaults['text'] : '',
                'region' => $region
            );
        }

        if (in_array('force_public', self::$field_list)) {
            $view_params['force_public'] = array(
                'force_public' => array('Adulte'=>'Adulte', 'Bébé'=>'Bébé', 'Enfant'=>'Enfant'),
                'default' => isset($defaults['force_public']) && $defaults['force_public'] ? $defaults['force_public'] : '',
                'region' => $region
            );
        }

        if (in_array('force_gender', self::$field_list)) {
            $view_params['force_gender'] = array(
                'force_gender' => array('Femme'=>'Femme', 'Homme'=>'Homme', 'Mixte'=>'Mixte', 'Fille'=>'Fille', 'Garçon'=>'Garçon'),
                'default' => isset($defaults['force_gender']) && $defaults['force_gender'] ? $defaults['force_gender'] : '',
                'region' => $region
            );
        }
    ...
    public static function setProductOptions($id_product, $region, $options, $id_product_attribute = null)
    {
        ...
        $sql = 'REPLACE INTO `'._DB_PREFIX_.Feedbiz::TABLE_FEEDBIZ_CDISCOUNT.'`
            (`'.pSQL(implode('`, `', array_keys($options))).'`)
            VALUES ('.implode(', ', $options).')';

        $rt = Db::getInstance()->execute($sql);
    ...
    public static function propagateProductOptionToCategory($region, $id_category, $field, $value)
    {
        ...
        } elseif (!$value && !in_array($field, array('force', 'clogistique'))) {
            $value = 'NULL';
        } elseif (!$value) {
            $value = 0;
        } elseif (is_string($value)) {
        ...
        $sql = 'INSERT INTO `'._DB_PREFIX_.Feedbiz::TABLE_FEEDBIZ_CDISCOUNT.'`
    ...
    public static function updateProductOptions($id_product, $options, $region, $id_product_attribute = null)
    {
        ...
        if (self::getProductOptions($id_product, $id_product_attribute, $region)) {
            if (Feedbiz::$debug_mode) {
                echo "<pre>\n";
                printf("%s(%d): Option - '%s'\n", basename(__FILE__), __LINE__, $options);
//            print_r($rq);
                echo "</pre>\n";
            }
            ...
            $sql_update = 'UPDATE `'._DB_PREFIX_.Feedbiz::TABLE_FEEDBIZ_CDISCOUNT.'` SET '.rtrim(trim($sql), ',').'
    ...
    */
    public static function deleteProductOptions($id_product, $region, $id_product_attribute = null)
    {
        ...
        $sql = 'DELETE FROM `'._DB_PREFIX_.Feedbiz::TABLE_FEEDBIZ_CDISCOUNT.'` WHERE `id_product` ='.(int)$id_product.
