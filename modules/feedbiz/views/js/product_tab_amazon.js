...
$(document).ready(function () {
    ...
    var sub_feedbiz_context = $('#feedbiz-product-subtab-amazon');
    var id_product = parseInt($('#id-product').val());
    var id_product_attribute = 0;
    var complex_id_product = null;

    $('.amazon-sub-tab', feedbiz_context).not('.main').find('.propagation').hide();
    ...
    function getComplexProductId() {
        ...
        if (window.console)
            console.log('getComplexProductId - Amazon', complex_id_product, id_product, id_product_attribute);
    ...
    $('.table.feedbiz-item tbody tr', feedbiz_context).click(function () {
        ...
        if ($('.amazon-tab-selector.active:visible', sub_feedbiz_context).length) {
            if (window.console)
                console.log('Item Selector - Amazon', feedbiz_context, sub_feedbiz_context);

            $('.amazon-tab-selector.active:visible', sub_feedbiz_context).trigger('click');
        }
        

        if($('.country-selector', sub_feedbiz_context).length < 1){
            var complex_id = getComplexProductId();
            var iso_code = $('span.amazon-tab-selector', feedbiz_context).attr('data-iso-code');

            $('.amazon-tab', sub_feedbiz_context).hide();
            $('.amazon-tab[data-iso-code="' + iso_code + '"]', sub_feedbiz_context).show();

            $('.amazon-sub-tab', sub_feedbiz_context).hide();
            $('.amazon-sub-tab[data-complex-id="' + complex_id + '"]', sub_feedbiz_context).show();
        }
    });
    ...
    $('.marketplace-selector', feedbiz_context).delegate('td', 'click', function () {
        if ($('.amazon-tab-selector.active:visible', sub_feedbiz_context).length) {
            if (window.console)
                console.log('Marketplace Selector - Amazon', feedbiz_context, sub_feedbiz_context);

            $('.amazon-tab-selector.active:visible', sub_feedbiz_context).parent().trigger('click');
        }
    });
    ...
    $('.table.feedbiz-item tbody tr td .delete-product-option', feedbiz_context).click(function () {
        ...
        if (current_marketplace != 'amazon')
            return;

        if (window.console)
            console.log('Delete Product Option - Amazon', current_tab, current_marketplace);
        ...
        if (id_product_attribute) // this is a combination, we delete only the subtab
            ...
            target_tab = $('.amazon-tab[data-iso-code="' + region + '"]', feedbiz_context);
        ...
        AmazonAjaxAction('delete-amazon', current_tab);
    ...
    $('.country-selector', sub_feedbiz_context).delegate('td', 'click', function () {
        ...
        var complex_id = getComplexProductId();
        var iso_code = $('span.amazon-tab-selector', this).attr('data-iso-code');

        if (window.console)
            console.log('Country Tabs - Amazon', sub_feedbiz_context, complex_id, iso_code);

        $('span.amazon-tab-selector', sub_feedbiz_context).removeClass('active');
        $('span.amazon-tab-selector', this).addClass('active');
    ...
    function DeleteBulletPointItem(obj) {
        ...
        var bullet_point_values = $('span[class^="amazon-bullet-container-"] input', target_section).serializeArray();
    ...
    $('.amazon-sub-tab', feedbiz_context).delegate('.amazon-bullet-point-del', 'click', function () {
        DeleteBulletPointItem($(this));
    });

    $('.amazon-sub-tab', feedbiz_context).delegate('.amazon-bullet-point-add', 'click', function (ev) {
    ...
    $('.amazon-sub-tab', feedbiz_context).delegate('input[name="fba"]', 'change', function (ev) {

        var target_subtab = ev.delegateTarget;

        if ($(this).attr('checked') === true || $(this).attr('checked') === 'checked') {
            $('tr.fba', target_subtab).show();
        } else {
            $('tr.fba', target_subtab).hide();
        }
    });
     ...
     * Save Form
     */
    $('.amazon-sub-tab', feedbiz_context).delegate('input', 'change', function (ev) {

        var target_subtab = ev.delegateTarget;

        AmazonAjaxAction('set-amazon', target_subtab);
    });

    $('.amazon-sub-tab', feedbiz_context).delegate('select', 'change', function (ev) {
    ...
    function AmazonAjaxAction(action, target_subtab) {
    ...
    $('.amazon-sub-tab', feedbiz_context).delegate('.propagate', 'click', function (ev) {
        ...
        $.ajax({
            type: 'POST',
            dataType: 'jsonp',
            url: $('#feedbiz-product-options-json-url').val() + '&action=propagate&field=' + field + '&scope=' + scope + '&entity=amazon&seed=' + new Date().getTime() + '&callback=?',
