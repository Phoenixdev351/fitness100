...
$(document).ready(function () {
    ...
    var sub_feedbiz_context = $('#feedbiz-product-subtab-fnac');
    var id_product = parseInt($('#id-product').val());
    var id_product_attribute = 0;
    var complex_id_product = null;

    $('.fnac-sub-tab', feedbiz_context).not('.main').find('.propagation').hide();
    ...
    function getComplexProductId() {
        ...
        if (window.console)
            console.log('getComplexProductId - fnac', complex_id_product, id_product, id_product_attribute);
    ...
    $('.table.feedbiz-item tbody tr', feedbiz_context).click(function () {
        ...
        if ($('.fnac-tab-selector.active:visible', sub_feedbiz_context).length) {
            if (window.console)
                console.log('Item Selector - fnac', feedbiz_context, sub_feedbiz_context);

            $('.fnac-tab-selector.active:visible', sub_feedbiz_context).trigger('click');
        }
        if($('.country-selector', sub_feedbiz_context).length < 1){
            var complex_id = getComplexProductId();
            var iso_code = $('span.fnac-tab-selector', feedbiz_context).attr('data-iso-code');

            $('.fnac-tab', sub_feedbiz_context).hide();
            $('.fnac-tab[data-iso-code="' + iso_code + '"]', sub_feedbiz_context).show();

            $('.fnac-sub-tab', sub_feedbiz_context).hide();
            $('.fnac-sub-tab[data-complex-id="' + complex_id + '"]', sub_feedbiz_context).show();
        }
    });
    ...
    $('.marketplace-selector', feedbiz_context).delegate('td', 'click', function () {
        if ($('.fnac-tab-selector.active:visible', sub_feedbiz_context).length) {
            if (window.console)
                console.log('Marketplace Selector - fnac', feedbiz_context, sub_feedbiz_context);

            $('.fnac-tab-selector.active:visible', sub_feedbiz_context).parent().trigger('click');
        }
    });
    ...
    $('.table.feedbiz-item tbody tr td .delete-product-option', feedbiz_context).click(function () {
        ...
        if (current_marketplace != 'fnac')
            return;

        if (window.console)
            console.log('Delete Product Option - fnac', current_tab, current_marketplace);
        ...
        if (id_product_attribute) // this is a combination, we delete only the subtab
            ...
            target_tab = $('.fnac-tab[data-iso-code="' + region + '"]', feedbiz_context);
        ...
        fnacAjaxAction('delete-fnac', current_tab);
    ...
    $('.country-selector', sub_feedbiz_context).delegate('td', 'click', function () {
        ...
        var complex_id = getComplexProductId();
        var iso_code = $('span.fnac-tab-selector', this).attr('data-iso-code');

        if (window.console)
            console.log('Country Tabs - fnac', sub_feedbiz_context, complex_id, iso_code);

        $('span.fnac-tab-selector', sub_feedbiz_context).removeClass('active');
        $('span.fnac-tab-selector', this).addClass('active');
    ...
    function DeleteBulletPointItem(obj) {
        ...
        var bullet_point_values = $('span[class^="fnac-bullet-container-"] input', target_section).serializeArray();
        ...
        $('span[class^=fnac-bullet-container]:last input', target_section).val('');
    }

    $('.fnac-sub-tab', feedbiz_context).delegate('.fnac-bullet-point-del', 'click', function () {
        DeleteBulletPointItem($(this));
    });

    $('.fnac-sub-tab', feedbiz_context).delegate('.fnac-bullet-point-add', 'click', function (ev) {
        var target_sub_tab = ev.delegateTarget;

        if (window.console)
            console.log('Feed.biz - Add Bullet Point', target_sub_tab);

        if ($('span[class^="fnac-bullet-container-"]:visible', target_sub_tab).length >= 5) {
            alert($('input[class="amz-text-max-bullet"]', target_sub_tab).val());
            return (false);
        }

        var target_bullet = $('span[class^=fnac-bullet-container]:not(:visible):first', target_sub_tab).show();

        $('input', target_bullet).val('');

        var bullet_point_values = $('span[class^="fnac-bullet-container-"] input', target_sub_tab).serializeArray();
        ...
        $('span[class^=fnac-bullet-container]:last input', target_sub_tab).val('');
    });
     ...
     * Save Form
     */
    $('.fnac-sub-tab', feedbiz_context).delegate('input', 'change', function (ev) {

        var target_subtab = ev.delegateTarget;

        fnacAjaxAction('set-fnac', target_subtab);
    });
    
    $('.fnac-sub-tab', feedbiz_context).delegate('select', 'change', function (ev) {
    ...
    function fnacAjaxAction(action, target_subtab) {
    ...
    $('.fnac-sub-tab', feedbiz_context).delegate('.propagate', 'click', function (ev) {
        ...
        $.ajax({
            type: 'POST',
            dataType: 'jsonp',
            url: $('#feedbiz-product-options-json-url').val() + '&action=propagate&field=' + field + '&scope=' + scope + '&entity=fnac&seed=' + new Date().getTime() + '&callback=?',
