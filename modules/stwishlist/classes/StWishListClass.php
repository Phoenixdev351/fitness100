...
class StWishListClass extends ObjectModel 
{
	...
	*/
	...
	public static function getProductTotal($id_product)
	{
		return (int)Db::getInstance()->getValue('SELECT count(0)
                            FROM `'._DB_PREFIX_.'st_wishlist_product` whp
                            LEFT JOIN `'._DB_PREFIX_.'st_wishlist` wh ON (wh.`id_st_wishlist`=whp.`id_st_wishlist`)
                            WHERE whp.`id_product` = '.(int)$id_product.
                            ' AND wh.`id_shop`= '.(int)Context::getContext()->shop->id);
	}
	public static function getCustomerTotal($id_customer)
	{
		return (int)Db::getInstance()->getValue('SELECT count(0)
                            FROM `'._DB_PREFIX_.'st_wishlist_product` whp
                            LEFT JOIN `'._DB_PREFIX_.'st_wishlist` wh ON (wh.`id_st_wishlist`=whp.`id_st_wishlist`)
                            WHERE wh.`id_customer`='.(int)$id_customer.
                            ' AND wh.`id_shop`= '.(int)Context::getContext()->shop->id);
	}
	public static function countProCustomer($id_customer,$id_product)
	{
		return (int)Db::getInstance()->getValue('SELECT count(0)
                            FROM `'._DB_PREFIX_.'st_wishlist_product` whp
                            LEFT JOIN `'._DB_PREFIX_.'st_wishlist` wh ON (wh.`id_st_wishlist`=whp.`id_st_wishlist`)
                            WHERE whp.`id_product` = '.(int)$id_product.
                            ' AND wh.`id_customer`='.(int)$id_customer.
                            ' AND wh.`id_shop`= '.(int)Context::getContext()->shop->id);
	}
}