...
class StTwitterEmbeddedTimelines extends Module implements WidgetInterface
{
    ...
    public function getWidgetVariables($hookName = null, array $configuration = [])
    {
        if(!Configuration::get($this->_prefix_st.'NAME') || !Configuration::get($this->_prefix_st.'WIDGET_ID'))
            return false;
        $name = isset($configuration['name']) && $configuration['name'] ? $configuration['name'] : Configuration::get($this->_prefix_st.'NAME');
        return array(
            'name' => $name,
    ...
    public function vc_shortcode_init($atts, $content = null)
    {
        ...
        return $this->renderWidget($hook, array('name'=>$name,'st_time'=>$st_time));
    }
    function vc_map_init()
    {
        ...
        $params = array(
            ...
            'params' => array(
                ...
                array(
                    'type' => 'textfield',
                    'heading' => $this->getTranslator()->trans('Twitter user name:', array(), 'Modules.Sttwitterembeddedtimelines.Admin'),
                    'class' => 'hide_in_vc_editor',
                    'holder' => 'div',
                    'param_name' => 'name',
                    'value' => Configuration::get($this->_prefix_st.'NAME'),
                ),
                array(
                    'type' => 'textfield',
                    'holder' => 'div',
