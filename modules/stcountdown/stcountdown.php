...
class StCountDown extends Module
{
    ...
    public function initFieldsForm()
    {
        $this->fields_form[0]['form'] = array(
			'legend' => array(
				'title' => $this->getTranslator()->trans('Countdown settings', array(), 'Modules.Stcountdown.Admin'),
                'icon'  => 'icon-cogs'
			),
        ...
        $this->fields_form[1]['form'] = array(
            ...
            'input' => array(
                ...
                array(
                 ...
                 array(
                    'type' => 'switch',
                    'label' => $this->getTranslator()->trans('Display countdown title on grid view:', array(), 'Modules.Stcountdown.Admin'),
                    'name' => 'title_text',
                    'is_bool' => true,
                    'default_value' => 0,
                    'values' => array(
                 ...
                 array(
                    'type' => 'color',
                    'label' => $this->getTranslator()->trans('Countdown title color:', array(), 'Modules.Stcountdown.Admin'),
                    'name' => 'title_color',
                    'class' => 'color',
                    'size' => 20,
                    'validation' => 'isColor',
                 ...
                 array(
                    'type' => 'color',
                    'label' => $this->getTranslator()->trans('Countdown title background:', array(), 'Modules.Stcountdown.Admin'),
                    'name' => 'title_bg',
                    'class' => 'color',
                    'size' => 20,
                    'validation' => 'isColor',
    ...
    public function hookDisplayHeader($params)
    {
        ...
        if (!$this->isCached('module:stcountdown/views/templates/hook/header.tpl', $cache_id))
        {
            $id_products = array();
            ...
            if (!Configuration::get($this->_prefix_st.'TITLE_TEXT'))
                $custom_css .= '.countdown_name{display:none;}';
            if ($title_color = Configuration::get($this->_prefix_st.'TITLE_COLOR'))
                $custom_css .= '.countdown_name{color:'.$title_color.';}';
            if ($title_bg = Configuration::get($this->_prefix_st.'TITLE_BG'))
                $custom_css .= '.countdown_name{background:'.$title_bg.';}';

            if($custom_css)
                $this->smarty->assign('custom_css', preg_replace('/\s\s+/', ' ', $custom_css));

        }
        return $this->fetch('module:stcountdown/views/templates/hook/header.tpl', $cache_id);
    }
    ...
    private function getConfigFieldsValues()
    {
        $fields_values = array(
            ...
            'title_text'      => Configuration::get($this->_prefix_st.'TITLE_TEXT'),
            'title_color'      => Configuration::get($this->_prefix_st.'TITLE_COLOR'),
            'title_bg'      => Configuration::get($this->_prefix_st.'TITLE_BG'),
