...
class StLovedProductMyLovedModuleFrontController extends ModuleFrontController
{
	...
	public function assign()
	{
		...
		if ($this->context->customer->isLogged())
		{
            ...
            if ($products = StLovedProductClass::getMyLoved($this->context->customer->id, $this->context->customer->id_guest, 0, 1)) {
            ...
            if ($show_blog && ($blogs = StLovedProductClass::getMyLoved($this->context->customer->id, $this->context->customer->id_guest, 0, 2))) {
    ...
    public function ajaxProcessDeleteProduct()
    {
        ...
        $type || $type = 1;
        //
        if (StLovedProductClass::DeleteProduct($id_source, $this->context->customer->id,$this->context->customer->id_guest, $type)) {
            ...
            'all' => StLovedProductClass::getMyLovedCount($this->context->customer->id, $this->context->customer->id_guest),
            )));
        }
    ...
    public function ajaxProcessAddLovedProduct()
    {
        ...
        if (StLovedProductClass::AddProduct($id_source, $this->context->customer->id,$this->context->customer->id_guest, $unloveable, $type)) {
            // $row = StLovedProductClass::getOne($id_source, $this->context->customer->id);
            die(Tools::jsonEncode(array('success' => 1,
            // 'action' => $unloveable && !StLovedProductClass::exists($id_source, $this->context->customer->id, $this->context->customer->id_guest, $type) ? 0 : 1, //to tell: add or remove
            'message' => $this->trans('Loved it!', array(), 'Shop.Theme.Transformer'),
            'total' => StLovedProductClass::getTotal($id_source, $type),
            'all' => StLovedProductClass::getMyLovedCount($this->context->customer->id, $this->context->customer->id_guest),
            // 'last_time' => strtotime($row['date_upd']),
            'sidebar' => $this->module->hookDisplaySideBar(array()),
            )));
        }
        die(Tools::jsonEncode(array('success' => 0,
    ...
    {
        if (Configuration::get($this->module->_prefix_st.'LOGIN') && !$this->context->customer->isLogged())
