...
class StOwlCarousel extends Module implements WidgetInterface
{
    ...
    public function hookDisplayCategoryHeader($params)
    {
        ...
        if(!$id_category)
            return false;
		if (!$this->isCached($this->templatePath.'stowlcarousel.tpl', $this->stGetCacheId($id_category, 'category-header')))
            $this->_prepareHook($id_category,2);
		return $this->fetch($this->templatePath.'stowlcarousel.tpl', $this->stGetCacheId($id_category, 'category-header'));
    }
    public function hookDisplayCMSExtra($params)
    {
        ...
        if(!$id_cms)
            return false;
		if (!$this->isCached($this->templatePath.'stowlcarousel.tpl', $this->stGetCacheId($id_cms, 'cms')))
            $this->_prepareHook($id_cms,4);
		return $this->fetch($this->templatePath.'stowlcarousel.tpl', $this->stGetCacheId($id_cms, 'cms'));
    }
    public function hookDisplayManufacturerHeader($params)
    {
        ...
        if(!$id_manufacturer)
            return false;
		if (!$this->isCached($this->templatePath.'stowlcarousel.tpl', $this->stGetCacheId($id_manufacturer, 'manufacturer')))
            $this->_prepareHook($id_manufacturer,6);
		return $this->fetch($this->templatePath.'stowlcarousel.tpl', $this->stGetCacheId($id_manufacturer, 'manufacturer'));
    }
    public function displayBySlideId($identify)
    {
        ...
        if(!$slide_group_obj->id || !$slide_group_obj->active)
            return false;
		if (!$this->isCached($this->templatePath.'stowlcarousel.tpl', $this->stGetCacheId($slide_group_obj->id, 'id')))
            $this->_prepareHook($identify,3);
		return $this->fetch($this->templatePath.'stowlcarousel.tpl', $this->stGetCacheId($slide_group_obj->id, 'id'));
    }
    ...
    public function renderWidget($hookName = null, array $configuration = [])
    {
        ...
        if(!$identify)
            return false;
        $id_st_owl_carousel_group = isset($configuration['id_st_owl_carousel_group']) ? (int)$configuration['id_st_owl_carousel_group'] : null;

        /*if ($index_slider && !$blog_slider && Dispatcher::getInstance()->getController()!='index' && is_null($id_st_owl_carousel_group)) {
            return false;
        }*/
        if($blog_slider && (!Module::isInstalled('stblog') || !Module::isEnabled('stblog')) && is_null($id_st_owl_carousel_group))
            return false;
        $cache_id = implode('-',$identify).(string)$id_st_owl_carousel_group.Dispatcher::getInstance()->getController();
        if (!$this->isCached($this->templatePath.'stowlcarousel.tpl', $this->stGetCacheId($cache_id)))
        {
            $this->smarty->assign(array(
                'st_time' => isset($configuration['st_time']) ? (string)$configuration['st_time'] : '',
            ));
            if(is_null($id_st_owl_carousel_group))
                $this->_prepareHook($identify,1);
            else
                $this->_prepareHook($id_st_owl_carousel_group, 3, $configuration);
        }
        return $this->fetch($this->templatePath.'stowlcarousel.tpl', $this->stGetCacheId($cache_id));
    }
    ...
    public function vc_shortcode_init($atts, $content = null)
    {
        ...
        $config = array(
            'id_st_owl_carousel_group'          => $id_st_owl_carousel_group,
            'st_time'                           => $st_time,
    ...
    function vc_map_init()
    {
        $content = array();
        $deafult = 0;
        foreach(StOwlCarouselGroup::getSlideGroup(0, 100) AS $value) {
            $content[$value['name']] = $value['id_st_owl_carousel_group'];
            !$deafult && $deafult = (int)$value['id_st_owl_carousel_group'];
        }
        $vc_hooks = array();
        ...
        $params = array(
            ...
            'params' => array(
                array(
                    ...
                    'param_name' => 'id_st_owl_carousel_group',
                    'value' => $content,
                    'std' => $deafult
                ),
                array(
