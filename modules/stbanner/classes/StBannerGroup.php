...
class StBannerGroup extends ObjectModel
{
	...
	public static $definition = array(
		...
		'fields'    => array(
            ...
            'width_md'           => array('type' => self::TYPE_INT, 'validate' => 'isunsignedInt'),
            'width_xs'           => array('type' => self::TYPE_INT, 'validate' => 'isunsignedInt'),
            ...
            'lazy_loading'             => array('type' => self::TYPE_INT, 'validate' => 'isunsignedInt'),
        ),
	);
    ...
    public static function getBannerGroup($identify,$type=1)
    {
        ...
        elseif ($type == 5) {
            ...
            $where .= ' AND smsg.`location`=0  AND smsg.`id_manufacturer`=0  AND smsg.`id_cms`=0  AND smsg.`id_category`=0 ';
        }
        if($type && !$where)
            return false;
        return Db::getInstance()->executeS('
