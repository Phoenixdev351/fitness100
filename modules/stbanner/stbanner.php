...
class StBanner extends Module implements WidgetInterface
{
    ...
    public static $location = array(
        ...
        70 => array('id' =>70 , 'name' => 'Right side of product description', 'hook' => 'ProductDescRightColumn', 'auto_height' => 0, 'column'=>1),
        ...
        */
        16 => array('id' =>16 , 'name' => 'At bottom of product page', 'hook' => 'FooterProduct', 'auto_height' => 2),
	...
	public function __construct()
	{
        ...
        $this->vc_hooks = array(
            'Block'                         => 'displayHome',
            'Full width block'              => 'displayFullWidthTop',
            'Full width boxed block'        => 'displayFullWidthTop2',
	...
	public function installDB()
	{
		...
		$return &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'st_banner_lang` (
                ...
                `image_multi_lang_sm` varchar(255) DEFAULT NULL,
                `image_multi_lang_xs` varchar(255) DEFAULT NULL,
                `image_lang_default` varchar(255) DEFAULT NULL,
                `image_lang_default_sm` varchar(255) DEFAULT NULL,
                `image_lang_default_xs` varchar(255) DEFAULT NULL,
                `width` int(10) unsigned NOT NULL DEFAULT 0,
                `width_sm` int(10) unsigned NOT NULL DEFAULT 0,
                `width_xs` int(10) unsigned NOT NULL DEFAULT 0,
                `height` int(10) unsigned NOT NULL DEFAULT 0,
                `height_sm` int(10) unsigned NOT NULL DEFAULT 0,
                `height_xs` int(10) unsigned NOT NULL DEFAULT 0,
		...
		$return &= (bool)Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'st_banner_group` (
                ...
                `width_md` tinyint(2) unsigned NOT NULL DEFAULT 0,
                `width_xs` tinyint(2) unsigned NOT NULL DEFAULT 12,
                `height` int(10) unsigned NOT NULL DEFAULT 0,
    ...
    public function sampleData($id_shop)
    {
		...
		foreach($samples as $k=>&$sample)
		{
            ...
            $module->width_xs = $sample['width_xs'];
    ...
    private function initPages()
    {
        $this->_pages = array(
                ...
                array(
                    'id' => 'search',
                    'val' => '512',
                    'name' => $this->getTranslator()->trans('Search', array(), 'Admin.Theme.Transformer')
                ),
                array(
                    'id' => 'module-stblog-category',
                    'val' => '1024',
                    'name' => $this->getTranslator()->trans('Blog category', array(), 'Admin.Theme.Transformer')
                ),
                array(
                    'id' => 'module-stblog-article',
                    'val' => '2048',
                    'name' => $this->getTranslator()->trans('Blog article', array(), 'Admin.Theme.Transformer')
                ),
                array(
                    'id' => 'module-stblogarchives-default',
                    'val' => '4096',
                    'name' => $this->getTranslator()->trans('Blog archives', array(), 'Admin.Theme.Transformer')
                ),
                array(
                    'id' => 'module-stblogsearch-default',
                    'val' => '8192',
                    'name' => $this->getTranslator()->trans('Blog search', array(), 'Admin.Theme.Transformer')
                ),
            );
    }
	...
	{ 
        ...
        if(Tools::getValue('act')=='delete_image' && $id_st_banner)
        {
            ...
            $mqv = Tools::getValue('mqv');
            $banner = new StBannerClass($id_st_banner);
            $image_multi_lang = 'image_multi_lang'.$mqv;
        ...
        if (isset($_POST['savestbannergroup']) || isset($_POST['savestbannergroupAndStay']))
        {
            ...
            if (!$display_on) {
                ...
                if(in_array($group->location, $location_full_width)) {
                    $display_on = 1;
                }
            }
            
            $group->display_on = $display_on;
		...
		if (isset($_POST['savestbanner']) || isset($_POST['savestbannerAndStay']))
		{
            ...
            else
            {
                ...
                foreach (self::$media_query as $mqv){
                    $width='width'.$mqv;
                    $height='height'.$mqv;
                    $image_multi_lang='image_multi_lang'.$mqv;
                    $image_lang_default='image_lang_default'.$mqv;
                    foreach ($languages as $lang)
                    {   
                        $banner->{$width}[$lang['id_lang']] = $banner->{$width}[$default_lang];
                        $banner->{$height}[$lang['id_lang']] = $banner->{$height}[$default_lang];
                        $res = $this->stUploadImage('image_multi_lang'.$mqv.'_'.$lang['id_lang']);
                        if(count($res['error'])){
                            $error = array_merge($error,$res['error']);
                        }
                        elseif($res['image'])
        ...
        if (isset($_FILES[$item]) && isset($_FILES[$item]['tmp_name']) && !empty($_FILES[$item]['tmp_name']))
		{
				...
				in_array($type, array('jpg', 'gif', 'jpeg', 'png')))
			{
				...
				elseif (!$temp_name || !move_uploaded_file($_FILES[$item]['tmp_name'], $temp_name))
                   ...
                   if($type == 'gif' && !copy($temp_name, _PS_UPLOAD_DIR_.$this->name.'/'.$c_name.'.'.$type)) {
				       $result['error'][] = $this->getTranslator()->trans('An error occurred during the image upload.', array(), 'Admin.Theme.Transformer');
                   } elseif ($type != 'gif' && !ImageManager::resize($temp_name, _PS_UPLOAD_DIR_.$this->name.'/'.$c_name.'.'.$type, null, null, $type)) {
                        $result['error'][] = $this->getTranslator()->trans('An error occurred during the image upload.', array(), 'Admin.Theme.Transformer');
                   }
				} 
				if (isset($temp_name))
    ...
    protected function initForm()
    {
        ...
        $this->fields_form[0]['form'] = array(
            ...
            'input' => array(
                array(
                    ...
                    'size' => 64,
                    'required'  => true,
                    'desc' => $this->getTranslator()->trans('Would not show out on the front office, just as a reminder:', array(), 'Modules.Stbanner.Admin'),
                ),
                'location' => array(
                    'type' => 'select',
                ...
                array(
                    'type' => 'checkbox',
                    'label' => $this->getTranslator()->trans('Which pages to show', array(), 'Modules.Stbanner.Admin'),
                    'name' => 'display_on',
                    'lang' => true,
                    'values' => array(
                        'query' => $this->_pages,
                    ...
                    'desc' => $this->getTranslator()->trans('This setting is only for hooks starting with "Full width *" in the above "Display on" dropdown list.', array(), 'Modules.Stbanner.Admin'),
                ),
				array(
					'type' => 'radio',
                    ...
                    'values' => array(
                        ...
                        array(
                            'id' => 'style_0',
                            'value' => 0,
                            'label' => $this->getTranslator()->trans('Advanced. Images will be scaled to cover banner areas, which means some parts of image may not be visible, the bright side is that you do not have to pay much attention to the dimensions of images.', array(), 'Modules.Stbanner.Admin')
                        ),
                    ),
                ),   
                array(
                    'type' => 'switch',
                    'label' => $this->getTranslator()->trans('Lazy loading:', array(), 'Modules.Stbanner.Admin'),
                    'name' => 'lazy_loading',
                    'default_value' => 1,
                    'is_bool' => true,
                    'values' => array(
        ...
        if($group->id) {
            ...
            foreach($this->_pages as $v) {
                $helper->tpl_vars['fields_value']['display_on_'.$v['id']] = (int)$v['val']&(int)$group->display_on;
            }
        }
	...
	protected function initFormColumn()
	{
		...
		$this->fields_form_column[0]['form'] = array(
			...
			'input' => array(
                ...
                array(
                    'type' => 'select',
                    'label' => $this->getTranslator()->trans('Width on desktop (screen width > 768px):', array(), 'Admin.Theme.Transformer'),
                    'name' => 'width',
                    'options' => array(
                        'query' => array(
                ...
                array(
                    'type' => 'select',
                    'label' => $this->getTranslator()->trans('Width on tablet (screen width < 768px and >480px):', array(), 'Admin.Theme.Transformer'),
                    'name' => 'width_md',
                    'options' => array(
                        'query' => array(
                                array('id'=>1, 'name'=> '1/12'),
                        ...
                        'default' => array(
                            'value' => 0,
                            'label' => $this->getTranslator()->trans('The same with as on desktop:', array(), 'Admin.Theme.Transformer'),
                        ),
                    ),
                ),
                array(
                    'type' => 'select',
                    'label' => $this->getTranslator()->trans('Width on phone (when screen width < 480px):', array(), 'Admin.Theme.Transformer'),
                    'name' => 'width_xs',
                    'options' => array(
                        'query' => array(
                                array('id'=>0, 'name'=> $this->getTranslator()->trans('The same with as on tablet:', array(), 'Admin.Theme.Transformer')),
                                array('id'=>1, 'name'=> '1/12'),
                                array('id'=>2, 'name'=> '2/12'),
                                array('id'=>3, 'name'=> '3/12'),
                                array('id'=>4, 'name'=> '4/12'),
	...
	{        
        ...
        $default_lang = (int)Configuration::get('PS_LANG_DEFAULT');
        foreach (self::$media_query as $mqk=>$mqv){
            foreach ($languages as $lang)
            {
                $this->fields_form_banner[0]['form']['input']['image_multi_lang'.$mqv.'_'.$lang['id_lang']] = array(
                        'type' => 'file',
                        'label' => $this->getTranslator()->trans('Image', array(), 'Admin.Theme.Transformer').($mqk ? ' '.$this->getTranslator()->trans('for screen width<%1%px', array('%1%'=>$mqk), 'Admin.Theme.Transformer') : '').' - '.$lang['name'].($default_lang == $lang['id_lang'] ? '('.$this->gettranslator()->trans('default language', array(), 'Admin.Theme.Transformer').')' : '').':',
                        'name' => 'image_multi_lang'.$mqv.'_'.$lang['id_lang'],
                        'desc' => $this->getTranslator()->trans('please ensure the image name is unique, or it will override the same name files. The feature of showing different images for different devices only works for Simple model.', array(), 'Modules.Stbanner.Admin').'<br/>',
                    );
            }
        }
        if($banner->id)
            ...
            foreach (self::$media_query as $mqk=>$mqv){
                foreach ($languages as $lang){
                    ...
                    if($banner->{$image_multi_lang}[$lang['id_lang']])
                    {
                        ...
                        $this->fields_form_banner[0]['form']['input']['image_multi_lang'.$mqv.'_'.$lang['id_lang']]['required'] = false;
                        $this->fields_form_banner[0]['form']['input']['image_multi_lang'.$mqv.'_'.$lang['id_lang']]['desc'] = '<img src="'.$banner->{$image_multi_lang}[$lang['id_lang']].'" width="200"/><p><a class="btn btn-default st_delete_image" data-id="'.$banner->id.'" data-lang="'.(int)$lang['id_lang'].'" data-mqv="'.$mqv.'" href="javascript:;"><i class="icon-trash"></i> '.$this->getTranslator()->trans(' Delete', array(), 'Admin.Theme.Transformer').'</a></p>';
                    }
                }
            }
        }
    ...
    private function _prepareHook($identify,$type=1, $vc=array())
    {
        ...
        if (isset(self::$location[$identify]['full_width']) && !isset(self::$location[$identify]['is_blog']) && $type==1 && $group) {
            $page = Dispatcher::getInstance()->getController();
            if (Tools::getvalue('fc') == 'module' && ($module=Tools::getValue('module'))) {
        ...
        foreach($group as $k=>$v)
        {
            ...
            if ($vc && isset($vc['is_full_width']) && isset($vc['stretched'])) {
                $group[$k]['is_full_width'] = $vc['is_full_width'];
                $group[$k]['stretched'] = $vc['stretched'];
            }
    ...
    public function hookDisplayCategoryHeader($params)
    {
        ...
        if(!$id_category)
            return false;
		if (!$this->isCached($this->templatePath.'st_banner.tpl', $this->stGetCacheId($id_category,'category_main_column','st_banner')))
            $this->_prepareHook($id_category,2,0);
		return $this->fetch($this->templatePath.'st_banner.tpl', $this->stGetCacheId($id_category,'category_main_column','st_banner'));
    }
    public function hookDisplayManufacturerHeader($params)
    {
        ...
        if(!$id_manufacturer)
            return false;
		if (!$this->isCached($this->templatePath.'st_banner.tpl', $this->stGetCacheId($id_manufacturer,'manufacturer_main_column','st_banner')))
            $this->_prepareHook($id_manufacturer,3,0);
		return $this->fetch($this->templatePath.'st_banner.tpl', $this->stGetCacheId($id_manufacturer,'manufacturer_main_column','st_banner'));
    }
    public function hookDisplayCMSExtra($params)
    {
        ...
        if(!$id_cms)
            return false;
		if (!$this->isCached($this->templatePath.'st_banner.tpl', $this->stGetCacheId($id_cms,'cms')))
            $this->_prepareHook($id_cms,4);
		return $this->fetch($this->templatePath.'st_banner.tpl', $this->stGetCacheId($id_cms,'cms'));
    }
    public function displayByBannerId($identify)
    {
		...
		if (!$this->isCached($this->templatePath.'st_banner.tpl', $this->stGetCacheId($identify,'id')))
            $this->_prepareHook($identify, 5);
		return $this->fetch($this->templatePath.'st_banner.tpl', $this->stGetCacheId($identify,'id'));
    }
    ...
    public function hookDisplayCategoryFooter($params)
    {
		...
		if (!$this->isCached($this->templatePath.'st_banner.tpl', $this->stGetCacheId($cache_id)))
            $this->_prepareHook(17,1);
		return $this->fetch($this->templatePath.'st_banner.tpl', $this->stGetCacheId($cache_id));
    }
    ...
    public function renderWidget($hookName = null, array $configuration = [])
    {
        ...
        $id_st_banner_group = isset($configuration['id_st_banner_group']) ? (int)$configuration['id_st_banner_group'] : null;
        $identify = array();
        foreach (self::$location as $k=>$v) {
            ...
            if ('display'.Tools::strtolower($v['hook'])==$hookName) {
                /*if (isset($v['full_width']) && $v['full_width'] && !isset($v['is_blog']) && Dispatcher::getInstance()->getController()!='index' && is_null($id_st_banner_group)) {
                    return false;
                }*/
                if (isset($v['is_blog']) && (!Module::isInstalled('stblog') || !Module::isEnabled('stblog')) && is_null($id_st_banner_group)) {
                    return false;
                }
        ...
        $page = Dispatcher::getInstance()->getController();
        $cache_id = implode('-', $identify).(string)$id_st_banner_group.$page;
        if (!$this->isCached($this->templatePath.'st_banner.tpl', $this->stGetCacheId($cache_id)))
        {
            $this->smarty->assign(array(
                'st_time' => isset($configuration['st_time']) ? (string)$configuration['st_time'] : '',
            ));
            if(is_null($id_st_banner_group))
                $this->_prepareHook($identify,1);
            else
                $this->_prepareHook($id_st_banner_group, 5, $configuration);
        }
        return $this->fetch($this->templatePath.'st_banner.tpl', $this->stGetCacheId($cache_id));
    }
    ...
    public function vc_shortcode_init($atts, $content = null)
    {
        ...
        $config = array(
            'id_st_banner_group'        => $id_st_banner_group,
            'st_time'                   => $st_time,
            'is_full_width'             => strpos(strtolower($hook_name), 'full width') !== false,
            'stretched'                 => strpos(strtolower($hook_name), 'boxed') === false,
        );
        return $this->renderWidget($hook, $config);
    }
    function vc_map_init()
    {
        $banners = array();
        $deafult = 0;
        foreach(StBannerGroup::getBannerGroup(0, 100) AS $value) {
            $banners[$this->getTranslator()->trans('ID:', array(), 'Admin.Theme.Transformer').$value['id_st_banner_group'].'|'.$value['name']] = $value['id_st_banner_group'];
            !$deafult && $deafult = $value['id_st_banner_group'];
        }
        $vc_hooks = array();
