<div class="search_widget_block search_widget_{$quick_search_simple} {if isset($headerclass)} {$headerclass} {/if}">
<div class="search_widget" data-search-controller-url="{$search_controller_url}">
	<form method="get" action="{$search_controller_url}" class="search_widget_form">
		...
		<div class="search_widget_form_inner input-group round_item js-parent-focus input-group-with-border">
	      <input type="text" class="form-control search_widget_text js-child-focus" name="s" value="{$search_string}" placeholder="{if isset($quick_search_placeholder) && $quick_search_placeholder}{$quick_search_placeholder}{else}{l s='Search our catalog' d='Shop.Theme.Transformer'}{/if}">
	      <span class="input-group-btn">
	        <button class="btn btn-search btn-no-padding btn-spin search_widget_btn link_color icon_btn" type="submit"><i class="fto-search-1"></i><span class="icon_text">{l s='Search' d='Shop.Theme.Transformer'}</span></button>
	      </span>
	    </div>
