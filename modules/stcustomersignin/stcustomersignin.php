...
class StCustomerSignIn extends Module implements WidgetInterface
{
	...
	public function install()
	{
		return (parent::install() 
            ...
            && Configuration::updateValue('ST_HEADER_ICON_SIZE', 0)
            && Configuration::updateValue('ST_HEADER_TEXT_SIZE', 0)
		);
	}
            ...
            'input' => array(
                ...
                array(
                    ...
                    'values' => array(
                        ...
                        array(
                            'id' => 'show_user_info_icons_vertical',
                            'value' => 3,
                            'label' => $this->getTranslator()->trans('Icon + Text (Vertical)', array(), 'Admin.Theme.Transformer')),
                    ),
                    'validation' => 'isUnsignedInt',
                    'desc' => $this->gettranslator()->trans('Go the the theme editor module to set an icon.', array(), 'Modules.Stcustomersignin.Admin'),
                ),
                array(
                    'type' => 'text',
    ...
    private function getConfigFieldsValues()
    {
        $fields_values = array(
            ...
            'header_icon_size' => (int)Configuration::get('ST_HEADER_ICON_SIZE'),
            'header_text_size' => (int)Configuration::get('ST_HEADER_TEXT_SIZE'),
        );
    ...
    {
        if (!$this->isCached('module:'.$this->name.'/views/templates/hook/header.tpl', $this->getCacheId()))
        {
            $custom_css = '';
            if($icon_size = (int)Configuration::get('ST_HEADER_ICON_SIZE')){
                $custom_css .= '.userinfo_mod_top.top_bar_item .header_icon_btn_icon i, .quick_login.top_bar_item .header_icon_btn_icon i, .login.top_bar_item .header_icon_btn_icon i{font-size:'.$icon_size.'px;}';
            }
            if($text_size = (int)Configuration::get('ST_HEADER_TEXT_SIZE')){
                $custom_css .= '.userinfo_mod_top.top_bar_item .header_icon_btn_text, .quick_login.top_bar_item .header_icon_btn_text, .login.top_bar_item .header_icon_btn_text{font-size:'.$text_size.'px;}';
            }
            $this->smarty->assign('custom_css', preg_replace('/\s\s+/', ' ', $custom_css));
        }
        return $this->fetch('module:'.$this->name.'/views/templates/hook/header.tpl', $this->getCacheId());
    }
    public function getWidgetVariables($hookName, array $configuration)
    {
        ...
        $variables = array(
            ...
            'welcome_link'    => Configuration::get('STSN_WELCOME_LINK'),
        ...
        if(Configuration::get('ST_USERINFO_LOGIN'))
        {
            ...
            $userinfo_redirect = Configuration::get('ST_USERINFO_REDIRECT');
            $url = ($userinfo_redirect==1 ? 'my-account' : ($userinfo_redirect==2 ? 'index' : $this->context->shop->getBaseURL(true, false).$_SERVER['REQUEST_URI']));
            $variables['formFields']['back']['value'] = $url;
            $variables['my_account_url']  = $link->getPageLink('authentication', true).'?back='.$url;
        }
