...
class StStickers extends Module implements WidgetInterface
{
	...
	public function __construct()
	{
        ...
        self::$type = array(
            ...
            7 => array(
                'id' => 'type_7',
                'value' => 7,
                'label' => $this->getTranslator()->trans('In stock, Product available with different options and allowed backorders(Native)', array(), 'Modules.Ststickers.Admin'),
                'class' => 'in-stock',
            ),
            5 => array(
                'id' => 'type_5',
        ...
        self::$position = array(
            ...
            13 => array(
                'id' => 13,
                'name' => $this->getTranslator()->trans('Along with price', array(), 'Modules.Ststickers.Admin'),
            ),
            1 => array(
                'id' => 1,
	...
	public function installDb()
	{
		$return = (bool)Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'st_sticker` (
                ...
                `text_width` smallint(6)  unsigned DEFAULT 0,
                `text_height` smallint(6)  unsigned DEFAULT 0,
    ...
    public function sampleData($id_shop)
    {
        ...
        foreach($samples as $sample)
        {
            ...
            $module->offset_y = $sample['offset_y'];
            $module->bg_color = $sample['bg_color'];
            $module->text_color = $sample['text_color'];
		...
		$this->fields_form[0]['form'] = array(
			...
			'input' => array(
                ...
                array(
                    ...
                    'default_value' => 0,
                    'values' => self::$type,
                    'desc' => $this->getTranslator()->trans('"In stock" and "Sold out" stickers would not show out on the product page, it would be reductant to show availability messages twice.', array(), 'Modules.Ststickers.Admin'),
                ), 
                array(
					'type' => 'text',
                    ...
                    'size' => 64,
                    'lang' => true,
                    'desc' => $this->getTranslator()->trans('"In stock" and "Sold out" stickers do not use this setting. They use texts defined on the product detail page or the product settings page.', array(), 'Modules.Ststickers.Admin'),
				),
                array(
                    'type' => 'select',
        ...
        if(Validate::isLoadedObject($sticker))
        {
            foreach ($languages as $lang)
                if($sticker->image_multi_lang[$lang['id_lang']])
                {
                    ...
                    '<p><a class="btn btn-default st_delete_image" data-id="'.$sticker->id.'" data-lang="'.(int)$lang['id_lang'].'" href="javascript:;"><i class="icon-trash"></i> '.$this->getTranslator()->trans(' Delete', array(), 'Admin.Theme.Transformer').'</a></p>';
                }
        }
    ...
    {
        if ($value || $row['text']) {
            return ($row['text'] ? $row['text'] : '').($value ? '<img style="max-width:200px;" src="'.$value.'" />' : '');    
        } else {
            return '-';
        }
    }
    ...
    {
        return array(
            'ststickers' => StStickersClass::getAll(0, 1, array(1,2,3,4,5,6,7)),
            );
    }
