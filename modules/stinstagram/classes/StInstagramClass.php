...
{
    public $_prefix_st = 'ST_INSTAGRAM_';
    /** @var integer id*/
    public $id;
    /** @var integer id*/
    ...
    public function getListInstagram(){
        $limit = $this->count;
        ...
        $id_shop=Context::getContext()->shop->id;
        $sql=(new DbQuery())->select("*")->from("st_instagram_img")->where('st_instagram_uid='.$user_id)->orderBy("id desc")->limit((int)$limit);
        $tmpData=Db::getInstance()->executeS($sql);
        if($tmpData && !empty($tmpData)){
            foreach($tmpData as $key=>&$val){
                if(is_null($val["media_type"])){
                    $tmp=$this->getImgInfos($val['instagram_id'],$access_token);
                    if(!$tmp){
                        unset($tmpData[$key]);
                    }elseif($tmp==2){
                    }else{
    ...
    public function getApidata1000id($id=0,$st=0){
        if($id<=0)
            return;
        $sql=(new DbQuery())->from('st_instagram_bind')->where('id_st_instagram_bind='.(int)$id);
        if(!$data=Db::getInstance()->getRow($sql)){
            return;
        }
        $this->tongbu($data['utoken'],$data['userid'],$st);
        //Configuration::updateGlobalValue('ST_INST_TONGBU_'.$data['userid'],time());
        return ;
    }

    public function tongbu($token=false,$userid=false,$st=0){
        if(!$token || !$userid){
            return false;
        }
        $url="https://graph.instagram.com/".$userid."?fields=media_count&access_token=".$token;
        $userData=Tools::file_get_contents($url,false,null,20,false);
        if(empty($userData) || empty(json_decode($userData,true))){
            return false;
        }
        // $tmpUserData=json_decode($userData,true);
        $limit=1000;
        $url_list='https://graph.instagram.com/'.$userid.'/media?limit='.$limit.'&access_token='.$token;
        $listData=Tools::file_get_contents($url_list,false,null,20,false);
        if(empty($listData) || empty(json_decode($listData,true))){
            return false;
        }
        if($st==1){
            Db::getInstance()->execute("DELETE FROM "._DB_PREFIX_."st_instagram_img WHERE `st_instagram_uid`=".$userid);
        }
        $tmpListData=json_decode($listData,true);
        if(isset($tmpListData["data"])){
            $data=array_reverse($tmpListData["data"]);
            foreach($data as $val){
                $sql="
                INSERT INTO `"._DB_PREFIX_.
                    "st_instagram_img`(`instagram_id`,`st_instagram_uid`,`ctime`,`utime`) VALUES ('".$val['id']."',".$userid.",".time().",'".time()."') ON DUPLICATE KEY UPDATE `st_instagram_uid`=VALUES(`st_instagram_uid`),`utime`=VALUES(`utime`);";
                Db::getInstance()->execute($sql);
            }
            if($st==1 || $st==2){
                foreach(array_slice($data,-50,50) as $val){
                    $this->getImgInfos($val['id'],$token);
                }
            }
    ...
    public function getImgInfos($id,$token){
        ...
        $data=Tools::file_get_contents($url_img_info,false,null,30,false);
        if($data && !empty($data)){
            ...
            if(isset($data["error"])){
                ...
                $sql="
                            ...
                            SET `media_type`='".pSQL($data['media_type'])."' ,
                            `inst_time`='".pSQL($data['timestamp'])."' ,
                            `caption`='".pSQL(isset($data['caption'])?$data['caption']:"")."' ,
                            `media_url`='".pSQL(isset($data['media_url'])?$data['media_url']:"")."' ,
                            `permalink`='".pSQL($data['permalink'])."' ,
                            `username`='".pSQL($data['username'])."' ,
                            `thumbnail_url`='".pSQL(isset($data['thumbnail_url'])?$data['thumbnail_url']:"")."' ,
                            `utime`=".time()." 
