...
class StProductCategoriesSliderClass extends ObjectModel
{
    ...
    public static function getListContent($active=0,$display_on=0,$display_on_is_id=false)
    {
        return  Db::getInstance()->executeS('
            SELECT spcs.*
            FROM `'._DB_PREFIX_.'st_product_categories_slider` spcs
            WHERE 1 '.Shop::addSqlRestrictionOnLang('spcs').($active ? ' AND spcs.`active`=1 ' : '').($display_on && !$display_on_is_id ? ' AND spcs.`display_on`&'.(int)$display_on : '').($display_on && $display_on_is_id ? ' AND spcs.`id_st_product_categories_slider`='.(int)$display_on : '').'
