...
<section id="brands_slider_{$hook_hash}" class="brands_slider">
    ...
    {if $title_position!=3}
        <div class="title_block flex_container title_align_{(int)$title_position} title_style_{(int)$sttheme.heading_style} {if isset($sub_title) && $sub_title} st_has_sub_title {/if}">
            ...
            <a href="{url entity='manufacturer'}" class="title_block_inner" title="{if isset($title) && $title}{$title}{else}{l s='Marcas destacadas' d='Shop.Theme.Transformer'}{/if}">{if isset($title) && $title}{$title}{else}{l s='Marcas destacadas' d='Shop.Theme.Transformer'}{/if}</a>
    ...
    {include file="catalog/slider/script.tpl" block_name="#brands_slider_container_{$hook_hash}" is_product_slider=0}
            ...
            {if isset($view_more) && $view_more}<div class="product_view_more_box text-center"><a href="{url entity='manufacturer'}" class="btn btn-default btn-more-padding btn-large" title="{l s='View more' d='Shop.Theme.Transformer'}">{l s='View more' d='Shop.Theme.Transformer'}</a></div>{/if}
