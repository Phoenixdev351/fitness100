...
class StSidebar extends Module implements WidgetInterface
{
    ...
    private function initNativeModules()
    {
        $this->_native_modules = array(
            ...
            14 => array('name' => $this->getTranslator()->trans('Search by category with auto suggestions', array(), 'Modules.Stsidebar.Admin'), 'module' => 'stadvancedsearch'),
            ...
            13 => array('name' => $this->getTranslator()->trans('Product comparison', array(), 'Modules.Stsidebar.Admin')),//, 'module' => 'stcompare'
            15 => array('name' => $this->getTranslator()->trans('Next and previous products', array(), 'Modules.Stsidebar.Admin')),//, 'module' => 'stproductlinknav'
            ...
            16 => array('name' => $this->getTranslator()->trans('My account', array(), 'Modules.Stsidebar.Admin')),
            17 => array('name' => $this->getTranslator()->trans('Language selector module', array(), 'Modules.Stsidebar.Admin'), 'module' => ''),
            18 => array('name' => $this->getTranslator()->trans('Currency selector module', array(), 'Modules.Stsidebar.Admin'), 'module' => ''),
            19 => array('name' => $this->getTranslator()->trans('Easymenu module', array(), 'Modules.Stsidebar.Admin'), 'module' => ''),
        );
    }
	public function install()
	{
        $res = parent::install() &&
            ...
            Configuration::updateValue($this->_prefix_st.'WIDTH_SM', 0) &&
            Configuration::updateValue($this->_prefix_st.'WIDTH_XS', 0) &&
    ...
    public function sampleData($id_shop)
    {
        ...
        foreach($samples as $i => $sample)
        {
            ...
            $module->active = 1;
            $module->position = (int)$sample['position'];
        ...
        $this->fields_form[0]['form'] = array(
            ...
            'input' => array(
                ...
                array(
                    ...
                    'lang' => true,
                    'size' => 64,
                    'desc' => $this->gettranslator()->trans('If you fill in this field, this sidebar would not open when customers click on it, customers will be redirected to the url you field.', array(), 'Modules.Stsidebar.Admin'),
                ),
                array(
                    'type' => 'textarea',
    ...
    public function initFieldsForm()
    {
        $this->fields_form[0]['form'] = array(
            ...
            'input' => array(
                ...
                array(
                    'type' => 'text',
                    'label' => $this->getTranslator()->trans('Sidebar width on small devices (<768px):', array(), 'Modules.Stsidebar.Admin'),
                    'name' => 'width_sm',
                    'validation' => 'isNullOrUnsignedId',
                    'prefix' => 'px',
                    'default_value' => 0,
                ...
                array(
                    'type' => 'text',
                    'label' => $this->getTranslator()->trans('Sidebar width on extra small devices (<480px):', array(), 'Modules.Stsidebar.Admin'),
                    'name' => 'width_xs',
                    'validation' => 'isNullOrUnsignedId',
                    'prefix' => 'px',
                    'default_value' => 0,
    ...
    private function getConfigFieldsValues()
    {
        $fields_values = array(
            ...
            'width_sm'                => Configuration::get($this->_prefix_st.'WIDTH_SM'),
            'width_xs'                => Configuration::get($this->_prefix_st.'WIDTH_XS'),
        );
    ...
    {        
        ...
        {   
            ...
            $with_template = array(
                '.st-menu{width:*px;}.open_bar_right #rightbar{right: *px;}.open_bar_left #leftbar{left: *px;}.is_rtl .open_bar_right #rightbar{right: auto;left: *px;}.is_rtl .open_bar_left #leftbar{left:auto;right: *px;}',
                '.st-effect-0.open_bar_right .st-pusher {-webkit-transform: translate3d(-*px, 0, 0); transform: translate3d(-*px, 0, 0); } .st-effect-0.open_bar_left .st-pusher {-webkit-transform: translate3d(*px, 0, 0); transform: translate3d(*px, 0, 0); } .is_rtl .st-effect-0.open_bar_right .st-pusher {-webkit-transform: translate3d(*px, 0, 0); transform: translate3d(*px, 0, 0); } .is_rtl .st-effect-0.open_bar_left .st-pusher {-webkit-transform: translate3d(-*px, 0, 0); transform: translate3d(-*px, 0, 0); }',
            );
            if ($width = Configuration::get($this->_prefix_st.'WIDTH'))
            {
                $custom_css .= str_replace('*', $width, $with_template[0]);
                $custom_css .= str_replace('*', $width, $with_template[1]);
            }
            if ($width_sm = Configuration::get($this->_prefix_st.'WIDTH_SM'))
            {
                $custom_css .= '@media (max-width: 767px) {'.str_replace('*', $width_sm, $with_template[0]).'}';
                $custom_css .= '@media (max-width: 767px) {'.str_replace('*', $width_sm, $with_template[1]).'}';
            }
            if ($width_xs = Configuration::get($this->_prefix_st.'WIDTH_XS'))
            {
                $custom_css .= '@media (max-width: 479px) {'.str_replace('*', $width_xs, $with_template[0]).'}';
                $custom_css .= '@media (max-width: 479px) {'.str_replace('*', $width_xs, $with_template[1]).'}';
            }
            ...
            foreach ($sidebar_items as $v) {
                /*if($v['hide_on_mobile']==1)
                    continue;*/
                if($v['location']==1 && $v['hide_on_mobile']!=1 && $v['native_modules']!=11 && $v['native_modules']!=12)
                    $sidebar_left_count++;
                if($v['location']==0 && $v['hide_on_mobile']!=1 && $v['native_modules']!=11 && $v['native_modules']!=12)
                    $sidebar_right_count++;
                ...
                else
                {
                    ...
                    switch ($v['native_modules']) {
                        ...
                        case 8:
                            ...
                            break;
                        case 14:
                            $native_modules = '#side_advanced_search';
                            break;
                        case 9:
                        ...
                        case 10:
                            ...
                            break;
                        case 13:
                            $native_modules = '#side_compare';
                            break;
                        case 17:
                            $native_modules = '#side_language';
                            break;
                        case 18:
                            $native_modules = '#side_currency';
                            break;
                        case 19:
                            $native_modules = '#side_steasymenu';
                            break;
                        case 11:
                        case 12:
                        case 15:
            ...
            if(!$sidebar_left_count && !$sidebar_right_count)
            {
    ...
    public function hookDisplayRightBar($params)
    {
        ...
        foreach ($sidebar_items as $k=>$v) {
            ...
            if ($v['native_modules']) {
                ...
                if($v['location']==1)
                    $sidebar_items_left['custom_'.$v['id_st_sidebar']] = $v;
                elseif($v['location']==0)
                    $sidebar_items_right['custom_'.$v['id_st_sidebar']] = $v;
            }
        }
    ...
    public function _prepare_bar($bar){
        ...
        if(array_key_exists('10', $bar))
        {
            $products_loved_nbr = 0;
            if($this->context->customer->id || $this->context->customer->id_guest){
                $c_g_id = $this->context->customer->id ? (int)Guest::getFromCustomer($this->context->customer->id) : 0;
                $products_loved_nbr = (int)Db::getInstance()->getValue('SELECT count(0)
                FROM `'._DB_PREFIX_.'st_loved_product`
                WHERE `id_shop`='.(int)Context::getContext()->shop->id.'
                AND ('.($this->context->customer->id ? '`id_customer`='.(int)$this->context->customer->id.($c_g_id ? ' OR `id_guest` ='.$c_g_id : '') : '').(!$this->context->customer->id && $this->context->customer->id_guest ? ' `id_guest` = '.(int)$this->context->customer->id_guest : '').')');
            }

            $this->smarty->assign(array(
                'products_loved_nbr' => $products_loved_nbr,
            ));
        }
        if(array_key_exists('13', $bar))
        {
            $arr = array();
            ...
            $this->smarty->assign(array(
                'stcompare_nbr' => count(array_unique($arr)),
                'stcompare_url' => $this->context->link->getModuleLink('stcompare', 'compare'),
            ));
        }
        if(array_key_exists('15', $bar))
        {
            if( Dispatcher::getInstance()->getController() != 'product' )
                return true;
            $moduleInstance = Module::getInstanceByName('stproductlinknav');
            if ($moduleInstance && is_callable(array($moduleInstance, '_prepareHook'))) {
                $this->smarty->assign(array(
                    'nav_products' => array(
                        'prev'=>$moduleInstance->_prepareHook('prev'),
                        'next'=>$moduleInstance->_prepareHook('next'),
                    ),
                ));
            }
        }
        if(array_key_exists('16', $bar))
        {
            $this->smarty->assign(array(
        ...
        foreach ($sidebar_items as $k=>$v) {
            if($v['location'] == $location) {
                if ($v['native_modules']) {
                    ...
                    $mobilebar['custom_'.$v['id_st_sidebar']] = $v;
                }
            }
        }
    ...
    protected function getViewedProductIds()
    {
        ...
        return array_slice($arr, 0, (int) $max_nbr);
    }
