...
class StBlogEditor extends BaseSlider implements WidgetInterface
{
    ...
    protected $sort_by = array(
        1 => array('id' =>1 , 'name' => 'Date published: Desc', 'orderBy'=>'date_add', 'orderWay'=>'DESC'),
        2 => array('id' =>2 , 'name' => 'Date published: Asc', 'orderBy'=>'date_add', 'orderWay'=>'ASC'),
    ...
    public function getConfigFieldsValues()
    {
        ...
        $fields_values['rount_name'] = array();    
    ...
    public function saveForm()
    {
        if (parent::saveForm()) {
            ...
            // Remove st blog route that saved by Preastashop.
            $routes = array(
                'module-stblog-category',
                'module-stblog-article',
                'module-stblog-default',
                'module-stblog-rss',
                'module-stblogarchives-default',
                'module-stblogsearch-default'
            );
            foreach($routes as  $route) {
                Configuration::deleteByName('PS_ROUTE_'.strtoupper($route));
            }
        }
    }
    ...
    public function resizeImage($src_file, $image_type = 1, $basename = '', $ext = 'jpg')
    {
        ...
        foreach($types[$image_type] AS $key => $type)
        {
            ...
            // If webp module enabled, use webp cut method.
            if (Module::isEnabled('stwebp') && method_exists('ImageManager', 'cut')) {
    ...
    public function hookDisplayHeader($params)
    {
        ...
        if (!$this->isCached($this->templateFile[0], $this->stGetCacheId('header')))
        {
            ...
            if($name_size = (int)Configuration::get($this->_prefix_st.'NAME_SIZE'))
                $css .='.block_blog .s_title_block{font-size: '.$name_size.'px;}';
            ...
            if(Configuration::get($this->_prefix_st.'BLOCK_HEADINGS_BG'))
                $css .='.is_blog .title_block .title_block_inner{backgorund-color: '.Configuration::get($this->_prefix_st.'BLOCK_HEADINGS_BG').';padding-left:6px;}.is_blog.is_rtl #left_column .title_block,.is_blog.is_rtl #right_column .title_block{padding-right:6px;padding-left:0;}';
            ...
            {
                $spacing_between = (int)Configuration::get($this->_prefix_st.'SPACING_BETWEEN');
    ...
    public function fontOptions() {
        $google = array();
        foreach((array)$this->googleFonts as $v)
