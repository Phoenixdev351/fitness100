...
class AdminKlaviyoPsConfigController extends ModuleAdminController
{
    ...
    const KLAVIYO_SMS_CONSENT_LABEL_DEFAULT = [
        ...
        'pl' => 'Subskrybuj aktualizacje SMS-owe',
        'sv' => 'Prenumerera på sms-uppdateringar',
        'nl' => 'Abonneren op sms-updates',
        'mx' => 'Suscribirse a las actualizaciones por SMS',
    ];
    ...
    const KLAVIYO_SMS_CONSENT_DISCLOSURE_TEXT = [
        ...
        'pl' => 'Zaznaczając to pole i wpisując powyżej swój numer telefonu, wyrażasz zgodę, by na podany numer otrzymywać od [Nazwa firmy] marketingowe wiadomości SMS (np. promocje, przypomnienia o porzuconym koszyku itd.), w tym wiadomości wysyłane przez automat wybierający. Zgoda nie jest warunkiem zakupu. Mogą obowiązywać opłaty za wiadomości i dane. Częstotliwość wysyłania wiadomości jest różna. Możesz zrezygnować w dowolnym momencie, odpowiadając STOP lub klikając link rezygnacji z subskrypcji (jeśli jest dostępny). Polityka prywatności [link] i Warunki[link]',
        'sv' => 'Genom att kryssa i denna ruta och ange ditt telefonnummer ovan samtycker du till att ta emot textmeddelanden för marknadsföring (t.ex. kampanjer, påminnelser om kundvagn) från [Company Name] på det angivna numret, inklusive meddelanden som skickas via automatisk uppringning. Samtycke är inte ett villkor för köp. Meddelande- och datataxor kan tillkomma. Frekvensen av meddelanden varierar. Avsluta prenumerationen när som helst genom att svara STOP eller klicka på länken för att avsluta prenumerationen (om sådan finns). Sekretesspolicy [link] och Villkor [link]',
        'nl' => 'Als je dit vakje aanvinkt en je telefoonnummer hierboven invoert, geef je toestemming om marketingberichten per sms te ontvangen (zoals promoties en winkelwagenherinneringen) van [Company Name] op het opgegeven nummer, inclusief berichten die door automatische kiestechnologie worden verzonden. Toestemming is geen voorwaarde voor aankoop. Bericht- en datatarieven kunnen van toepassing zijn. De frequentie van berichten varieert. Je kunt je op elk moment afmelden door STOP te antwoorden of door op de afmeldingslink te klikken (indien beschikbaar). Privacybeleid [link] en voorwaarden [link]',
        'mx' => 'Al marcar esta casilla e ingresar tu número de teléfono más arriba, das tu consentimiento para recibir mensajes de texto de marketing (p. ej. promociones, recordatorios de carritos) de parte de [Nombre de la empresa] en el número proporcionado, incluidos mensajes enviados por un marcador automático. El consentimiento no es una condición para realizar ninguna compra. Pueden aplicarse tarifas de mensajes y datos. La frecuencia de los mensajes varía. Si deseas cancelar tu suscripción en cualquier momento, responde STOP o haz clic en el enlace de cancelación de suscripción (cuando esté disponible). Política de privacidad [link] y Condiciones [link]',
    ];
