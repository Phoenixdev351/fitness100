...
class StFeaturedSlider extends BaseProductsSlider
{
    ...
    public function getProducts($ext='')
    {
        ...
        foreach ($result->getProducts() as $rawProduct) {
            ...
            if(count($products_for_template)>=$nProducts)
                break;
        }
    ...
    {
        $soby = (int)Configuration::get($this->_prefix_st.'SOBY');
        return parent::hookDisplayHome($params, $soby == 13 ? false : $func, $flag);
    }
    public function hookDisplayFooter($params, $func = '')
    {
        $soby = (int)Configuration::get($this->_prefix_st.'SOBY_FOT');
        return parent::hookDisplayFooter($params, $soby == 13 ? false : $func);
    }
    public function hookDisplayLeftColumn($params, $func = '')
    {
        $soby = (int)Configuration::get($this->_prefix_st.'SOBY_COL');
        return parent::hookDisplayLeftColumn($params, $soby == 13 ? false : $func);
    }
}