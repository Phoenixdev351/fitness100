...
class SequraPSOrderUpdater
{
     ...
     *
     * @return SequraPSOrderUpdater
     */
    public static function getInstance($context_or_webhook, $order_id = null)
    {
        static $inst = null;
        if ($inst === null) {
            $inst = new SequraPSOrderUpdater($context_or_webhook, $order_id);
        }
        return $inst;
    }
    ...
    public function processCancellationRequest()
    {
        ...
        die(json_encode(array('result' => 'cancelled')));
    }
    ...
    public function cancelOrder()
    {
        foreach (Order::getByReference($this->order->reference) as $order) {
            ...
            $history->changeIdOrderState(Configuration::get('SEQURA_OS_CANCELED'), (int)($order->id));
    ...
    public function setRiskLevelToOrder()
    {
        foreach (Order::getByReference($this->order->reference) as $order) {
            ...
            switch (Tools::getValue('risk_level')) {
                case 'low_risk':
                    $new_status = Configuration::get('SEQURA_OS_APPROVED_LOWRISK');
                    break;
                case 'high_risk':
                    $new_status = Configuration::get('SEQURA_OS_APPROVED_HIGHRISK');
                    break;
                case 'under_evaluation':
                    $new_status = Configuration::get('SEQURA_OS_APPROVED_UNKNOWNRISK');
                    break;
                default:
                    return;
            }
    ...
    public function orderHasBeenShipped()
    {
        if ($this->order->id && $this->order->hasBeenShipped()) {
            ...
            $this->respondWithError(
                json_encode(array('result' => 'toolate', 'since' => (int)$since))
            );
        }
    }
    ...
    public function orderIsToOld()
    {
        ...
        if ($tooLateSince > 0) {
            $this->respondWithError(
                json_encode(array('result' => 'toolate', 'since' => (int)$tooLateSince * 60))
            );
        }
    }
    ...
    public function prepareSequra()
    {
        ...
        $builder = new SequraOrderBuilder($this->getMerchantIdUsed(), $this->cart, $pm);
        $this->sequra_order = $builder->build('cancelled');
        $this->emptyCart();
        $this->sequra_order['merchant_reference'] = SequraReportBuilder::getMerchantRefs($this->order);
        if (! $this->webhook) {
    ...
    public static function webhookUrl($cart_id = null)
    {
        ...
        $params = array();
        if ($cart_id) {
            $params = array_merge($params, self::cancelParams($cart_id));
        }
        if (_PS_VERSION_ >= 1.5) {
            return $linker->getModuleLink('sequrapayment', 'webhook', $params, true);
        }

        return $linker->getPageLink('modules/sequrapayment/webhook.php', true);
    }
