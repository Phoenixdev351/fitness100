...
<ul id="st_mobile_menu_ul" class="mo_mu_level_0">
	{foreach $stmenu as $mm}
		...
		{if $mm.m_name}
		<li id="st_mo_menu_{$mm.id_st_mega_menu}" class="mo_ml_level_0 mo_ml_column">
			{assign var='has_children' value=(isset($mm.column) && count($mm.column))}
			<div class="menu_a_wrap">
			<a id="st_mo_ma_{$mm.id_st_mega_menu}" href="{if $mm.m_link}{$mm.m_link}{else}javascript:;{/if}" class="mo_ma_level_0"{if !$menu_title} title="{$mm.m_title}"{/if}{if $mm.nofollow} rel="nofollow"{/if}{if $mm.new_window} target="_blank"{/if}>{if $mm.m_icon}{$mm.m_icon nofilter}{else}{if $mm.icon_class}<i class="{$mm.icon_class}"></i>{/if}{$mm.m_name nofilter}{/if}{if $mm.cate_label}<span class="cate_label">{$mm.cate_label}</span>{/if}</a>
						...
						{foreach $column.children as $block}
							...
							{if $block.item_t==1}
								{if $block.subtype==2  && isset($block.children) && count($block.children)}
									<ul id="st_mo_menu_block_{$block.id_st_mega_menu}" class="mo_mu_level_1 mo_sub_ul">
										{if $mm.is_mega}
										<li class="mo_ml_level_1 mo_sub_li">
											...
											{if $has_children}
												...
												<li class="mo_ml_level_2 mo_sub_li"><a href="{$product.link}"{if !$menu_title} title="{$product.name}"{/if}{if $block.nofollow} rel="nofollow"{/if}{if $block.new_window} target="_blank"{/if} class="mo_ma_level_2 mo_sub_a">{$product.name}</a></li>
												{/foreach}
												</ul>	
											{/if}
										</li>
										{else}
											{if isset($block.children.children) && is_array($block.children.children) && count($block.children.children)}
												...
												<li class="mo_ml_level_1 mo_sub_li">
													<div class="menu_a_wrap">
													<a href="{$product.link}"{if !$menu_title} title="{$block.children.name}"{/if}{if $block.nofollow} rel="nofollow"{/if}{if $block.new_window} target="_blank"{/if} class="mo_ma_level_1 mo_sub_a">{$product.name}</a>
													</div>
												</li>
												{/foreach}
											{/if}
								...
								{elseif $block.subtype==1 || $block.subtype==3}
									<ul id="st_mo_menu_block_{$block.id_st_mega_menu}" class="mo_mu_level_1 mo_sub_ul">
										<li class="mo_ml_level_1 mo_sub_li">
											...
											<div class="menu_a_wrap">
											<a  id="st_mo_ma_{$block.id_st_mega_menu}" href="{$block.children.link}"{if !$menu_title} title="{$block.children.name}"{/if}{if $block.nofollow} rel="nofollow"{/if}{if $block.new_window} target="_blank"{/if} class="mo_ma_level_1 mo_sub_a">{$block.children.name}{if $block.cate_label}<span class="cate_label">{$block.cate_label}</span>{/if}</a>
											{if $block.subtype==1 && $has_children}<span class="opener"><i class="fto-plus-2 plus_sign"></i><i class="fto-minus minus_sign"></i></span>{/if}
											</div>
    										{if $block.subtype==1 && $has_children}
							...
							{elseif $block.item_t==2 && isset($block.children) && count($block.children)}
								<div id="st_mo_menu_block_{$block.id_st_mega_menu}" class="stmobilemenu_column">
	    									...
	    									<div class="mo_brand_div">
												<a href="{$brand.url}" title="{$brand.name}"{if $block.nofollow} rel="nofollow"{/if}{if $block.new_window} target="_blank"{/if} class="st_menu_brand">
												    ...
												    <!--[if IE 9]><video style="display: none;"><![endif]-->
												      <source srcset="{$brand.image|regex_replace:'/\.jpg$/':'.webp'}{if isset($sttheme) && $sttheme.retina && isset($brand.image_2x) && $brand.image_2x}, {$brand.image_2x|regex_replace:'/\.jpg$/':'.webp'} 2x{/if}"
												        title="{$brand.name}"
												        type="image/webp"
												        >
												    <!--[if IE 9]></video><![endif]-->
												    {/if}
								                    <img src="{$brand.image}" {if isset($sttheme) && $sttheme.retina && isset($brand.image_2x) && $brand.image_2x} srcset="{$brand.image_2x} 2x"{/if} alt="{$brand.name}" width="{$manufacturerSize.width}" height="{$manufacturerSize.height}" />
												    </picture>
								                </a>
											</div>
										{/foreach}
							...
							{elseif $block.item_t==5 && $block.html}
								<div id="st_mo_menu_block_{$block.id_st_mega_menu}" class="stmobilemenu_column style_content">
									{$block.html nofilter}
								</div>
							{/if}
						{/foreach}
