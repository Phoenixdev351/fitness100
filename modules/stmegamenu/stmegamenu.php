...
class StMegaMenu extends Module implements WidgetInterface
{
    ...
    {
        $mobile_only = false;
        if(Configuration::get('STSN_USE_MOBILE_HEADER')){
            $mobile_detect = $this->context->getMobileDetect();
            $mobile_only = $mobile_detect->isMobile() || $mobile_detect->isTablet();
        }
        ...
        if(is_array($all) && count($all))
        {
            ...
            {
                if($mobile_only && $v['hide_on_mobile']==1){
                    unset($all[$index]);
                    continue;
                }
                ...
                {
                    if($mobile_only && $col['hide_on_mobile']==1){
                        unset($columns[$ci]);
                        continue;
                    }
                        ...
                        switch ($k['item_t']) {
                            case '1':
                                if($k['subtype']==2)
                                {
                                    ...
                                    if (!$category->active || !Validate::isLoadedObject($category)) {
                                        continue 2;
                                    }
                                ...
                                if ($k['item_k'] == 1)
                                {
                                    ...
                                    if($menuBrands)
                                    {
                                        foreach ($menuBrands as $b_k => $b_v)
                                        {
                                            ...
                                            $menuBrands[$b_k]['image'] = Context::getContext()->link->getManufacturerImageLink($b_v['id_manufacturer'], 'brand_default');
                                            $menuBrands[$b_k]['image_2x'] = Context::getContext()->link->getManufacturerImageLink($b_v['id_manufacturer'], 'brand_default_2x');
                                        }
                                    }
                                }
                                else
    ...
    {
        if((int)Configuration::get('STSN_USE_MOBILE_HEADER')==1){
            $mobile_detect = $this->context->getMobileDetect();
            $mobile_device = $mobile_detect->isMobile() || $mobile_detect->isTablet();
            $is_responsive = (int)Configuration::get('STSN_RESPONSIVE');
            if($mobile_device && $is_responsive)
                return;
        }
        if (!$this->isCached($this->templatePath.'stmegamenu.tpl', $this->stGetCacheId('stmegamenu')))
        {
        ...
        return $this->fetch($this->templatePath.'stmegamenu.tpl', $this->stGetCacheId('stmegamenu'));
    }
    public function hookDisplayLeftColumn($param)
    {
        $this->setLastVisitedCategory();
        if (!$this->isCached($this->templatePath.'stmegamenu-column.tpl', $this->stGetCacheId('column')))
        {
            if (!isset(StMegaMenu::$cache_stmegamenu))
                StMegaMenu::$cache_stmegamenu = $this->_prepareHook();
        ...
        return $this->fetch($this->templatePath.'stmegamenu-column.tpl', $this->stGetCacheId('column'));
    }
    ...
    {
        if (!$this->isCached($this->templatePath.'stmobilemenu.tpl', $this->getCacheId()))
        {
            if (!isset(StMegaMenu::$cache_stmegamenu))
                StMegaMenu::$cache_stmegamenu = $this->_prepareHook();
        ...
        return $this->fetch($this->templatePath.'stmobilemenu.tpl', $this->getCacheId());
    }
    public function hookDisplayMobileNav($param)
    {
        if (!$this->isCached($this->templatePath.'stmobilemenu-ul.tpl', $this->getCacheId()))
        {
            if (!isset(StMegaMenu::$cache_stmegamenu))
                StMegaMenu::$cache_stmegamenu = $this->_prepareHook();
        ...
        return $this->fetch($this->templatePath.'stmobilemenu-ul.tpl', $this->getCacheId());
    }
        ...
        switch($row['item_k'])
        {
			...
			case 7:
                if(Configuration::get('PS_SSL_ENABLED') && Configuration::get('PS_SSL_ENABLED_EVERYWHERE'))
                {
                    ...
                    $link = $this->context->link->getPageLink('index', true);
                }
                else
	...
	{
        $id_cateogry = array($params['category']->id);
        if (isset($params['deleted_children']) && $params['deleted_children']) {
            foreach($params['deleted_children'] as $child) {
                $id_cateogry[] = $child->id;
            }
        }
		$rs = Db::getInstance()->executeS('SELECT id_st_mega_menu FROM '._DB_PREFIX_.'st_mega_menu WHERE item_k=1 AND item_v IN ('.implode(',', $id_cateogry).') ORDER BY id_st_mega_menu');
        foreach($rs AS $value)
        {
    ...
    {
        if (!$id_product = $params['product']->id) {
            return false;
        }
        $rs = Db::getInstance()->getValue('SELECT COUNT(0) FROM '._DB_PREFIX_.'st_menu_product 
            ...
            WHERE id_product='.(int)$id_product.') AND id_product != '.(int)$id_product);
        if (!$rs)
        {
        ...
        StMegaProductClass::deleteByIdProduct($id_product);
        $this->clearStMegamenuCache();
    }
    ...
    public function renderWidget($hookName = null, array $param = [])
    {
         return $this->hookDisplayMainMenu($param, true);
    }
    
    public function getWidgetVariables($hookName = null, array $param = [])
    {
        return;
    }
