...
class StSwiper extends Module implements WidgetInterface
{
	...
	public function __construct()
	{
        ...
        self::$location = array(
            ...
            60 => array('id' =>60 , 'name' => 'Most top of the page', 'hook' => 'Banner', 'full_width' => 1),
            2 => array('id' =>2 , 'name' => 'Left column except for the product page', 'hook' => 'LeftColumn', 'column_slider' => 1),
            5 => array('id' =>5 , 'name' => 'Right column except for the product page', 'hook' => 'RightColumn', 'column_slider' => 1),
            ...
            52 => array('id' =>52 , 'name' => 'Right side of product description', 'hook' => 'ProductDescRightColumn', 'column_slider' => 1),
	...
	public function getContent()
	{
        ...
        if(Tools::getValue('act')=='delete_image' && $id_st_swiper)
        {
            ...
            $slide->{$image_multi_lang}[$id_lang] = '';
            $result['r'] = $slide->save();
        ...
        if (isset($_FILES[$item]) && isset($_FILES[$item]['tmp_name']) && !empty($_FILES[$item]['tmp_name']))
		{
				...
				in_array($type, array('jpg', 'gif', 'jpeg', 'png')))
			{
				...
				elseif (!$temp_name || !move_uploaded_file($_FILES[$item]['tmp_name'], $temp_name))
                   ...
                   } elseif($type != 'gif' && !ImageManager::resize($temp_name, _PS_UPLOAD_DIR_.$this->name.'/'.$c_name.'.'.$type, null, null, $type) || !ImageManager::resize($temp_name, _PS_UPLOAD_DIR_.$this->name.'/'.$c_name_thumb.'.'.$type, $ratio_x, $ratio_y, $type))
				       $result['error'][] = $this->displayError($this->getTranslator()->trans('an error occurred during the image upload.', array(), 'Admin.Theme.Transformer'));
				} 
				if (isset($temp_name))
					@unlink($temp_name);
			...
			'input' => array(
                ...
                array(
                    ...
                    'desc' => array(
                            ...
                            $this->getTranslator()->trans('1. Images you uploaded would be used as background images.', array(), 'Modules.Stswiper.Admin'), 
                            $this->getTranslator()->trans('2. The height of this slider would not decrease on samll screen devices. Images will be kind like cropped on small screen devices, some parts of your images may not be visible.', array(), 'Modules.Stswiper.Admin'), 
                            $this->getTranslator()->trans('3. Images will be kind like cropped on small screen devices, some parts of your images may not be visible.', array(), 'Modules.Stswiper.Admin'), 
                            $this->getTranslator()->trans('4. This module would work exactly like the "Iosslider" module which has been removed.', array(), 'Modules.Stswiper.Admin'), 
                            $this->getTranslator()->trans('5. "Transition style" setting below would not take work for this slider.', array(), 'Modules.Stswiper.Admin'), 
                            $this->getTranslator()->trans('Bright sides are:', array(), 'Modules.Stswiper.Admin'),  
                            $this->getTranslator()->trans('1. This slider will work in a different way comparing with other slider modules, there must be someone like the way.', array(), 'Modules.Stswiper.Admin'),                            
                ...
                array(
                    ...
                    'desc' => $this->getTranslator()->trans('If this setting is enabled, images will be used as backgorund, images for mobile won\'t show out, because of no perfect way to show different background images on different types of devices.', array(), 'Modules.Stswiper.Admin'),
                ),
	...
	protected function initFormSlide($isbanner=0)
	{
        ...
        foreach (self::$media_query as $mqk=>$mqv){
            foreach ($languages as $lang)
            {
                $this->fields_form_slide[0]['form']['input']['image_multi_lang'.$mqv.'_'.$lang['id_lang']] = array(
                        'type' => 'file',
            ...
            foreach (self::$media_query as $mqk=>$mqv){
                foreach ($languages as $lang){
                    ...
                    if($slide->{$image_multi_lang}[$lang['id_lang']])
                    {
                        ...
                        $this->fields_form_slide[0]['form']['input']['image_multi_lang'.$mqv.'_'.$lang['id_lang']]['required'] = false;
                        $this->fields_form_slide[0]['form']['input']['image_multi_lang'.$mqv.'_'.$lang['id_lang']]['desc'] = '<img src="'.$slide->{$image_multi_lang}[$lang['id_lang']].'" width="200"/><p><a class="btn btn-default st_delete_image" data-id="'.$slide->id.'" data-lang="'.(int)$lang['id_lang'].'" data-mqv="'.$mqv.'" href="javascript:;"><i class="icon-trash"></i> '.$this->getTranslator()->trans(' Delete', array(), 'Admin.Theme.Transformer').'</a></p>';
                    }
                }
            }
        }
    ...
    private function _prepareHook($identify,$type=1, $vc=array())
    {
        ...
        foreach($slide_group as $k => &$v)
        {
            ...
            if ($vc && isset($vc['is_full_width'])) {
                $v['is_full_width'] = $vc['is_full_width'];
            }
    ...
    public function hookDisplayCMSExtra($params)
    {
        ...
        if(!$id_cms)
            return false;
		if (!$this->isCached($this->templatePath.'stswiper.tpl', $this->stGetCacheId($id_cms, 'cms')))
            $this->_prepareHook($id_cms,4);
		return $this->fetch($this->templatePath.'stswiper.tpl', $this->stGetCacheId($id_cms, 'cms'));
    }	
    public function hookDisplayCategoryHeader($params)
    {
        ...
        if(!$id_category)
            return false;
		if (!$this->isCached($this->templatePath.'stswiper.tpl', $this->stGetCacheId($id_category,'category-header')))
            $this->_prepareHook($id_category,2);
		return $this->fetch($this->templatePath.'stswiper.tpl', $this->stGetCacheId($id_category,'category-header'));
    }
    public function hookDisplayManufacturerHeader($params)
    {
        ...
        if(!$id_manufacturer)
            return false;
		if (!$this->isCached($this->templatePath.'stswiper.tpl', $this->stGetCacheId($id_manufacturer,'manufacturer')))
            $this->_prepareHook($id_manufacturer,6);
		return $this->fetch($this->templatePath.'stswiper.tpl', $this->stGetCacheId($id_manufacturer,'manufacturer'));
    }
    public function displayBySlideId($identify)
    {
		...
		if (!$this->isCached($this->templatePath.'stswiper.tpl', $this->stGetCacheId($identify,'id')))
            $this->_prepareHook($identify, 3);
		return $this->fetch($this->templatePath.'stswiper.tpl', $this->stGetCacheId($identify,'id'));
    }
    ...
    public function renderWidget($hookName = null, array $configuration = [])
    {
        ...
        if(!$identify)
            return false;
        $id_st_swiper_group = isset($configuration['id_st_swiper_group']) ? (int)$configuration['id_st_swiper_group'] : null;
        
        /*if ($index_slider && !$blog_slider && Dispatcher::getInstance()->getController()!='index' && is_null($id_st_swiper_group)) {
            return false;
        }*/
        if($blog_slider && (!Module::isInstalled('stblog') || !Module::isEnabled('stblog')) && is_null($id_st_swiper_group))
            return false;
        ...
        $cache_id = implode('-',$identify).(string)$id_st_swiper_group.Dispatcher::getInstance()->getController();
        if (!$this->isCached($this->templatePath.'stswiper.tpl', $this->stGetCacheId($cache_id)))
        {
            $this->smarty->assign(array(
                'st_time' => isset($configuration['st_time']) ? (string)$configuration['st_time'] : '',
            ));
            if(is_null($id_st_swiper_group))
                $this->_prepareHook($identify,1);
            else
                $this->_prepareHook($id_st_swiper_group, 3, $configuration); 
        }
        return $this->fetch($this->templatePath.'stswiper.tpl', $this->stGetCacheId($cache_id));
    }
    ...
    public function vc_shortcode_init($atts, $content = null)
    {
        ...
        $config = array(
            'id_st_swiper_group'        => $id_st_swiper_group,
            'st_time'                   => $st_time,
    ...
    function vc_map_init()
    {
        $content = array();
        $default = 0;
        foreach(StSwiperGroup::getSlideGroup(0, 100) AS $value) {
            $content[$value['name']] = $value['id_st_swiper_group'];
            !$default && $default = $value['id_st_swiper_group'];
        }
        $vc_hooks = array();
