		...
		<div class="steasy_element_item text-{$element.st_el_text_align} {if isset($element.st_el_mobile_text_align)} text-md-{$element.st_el_mobile_text_align} {/if} {if $element.st_el_content_width} width_{$element.st_el_content_width} {/if} clearfix {if $pre_template[0]==2} row {/if}">  
			...
			{if $element.st_image_block_width && (isset($element.st_image) && $element.st_image)}
				...
				<div class="sttext_item_image {if $element.st_image_hover} hover_effect_{$element.st_image_hover} {/if} {if $pre_template[0]==2} col-lg-{$element.st_image_block_width|replace:'.':'-'} {if $pre_template[1]==2 || $pre_template[1]==4} push-lg-{$column_width_total|replace:'.':'-'} {/if} {/if} {if $pre_template[0]==3}{if $pre_template[1]==1} fl {else} fr  {/if}{/if}">
					{if isset($element.st_image_link) && $element.st_image_link}<a href="{$element.st_image_link}" class="sttext_item_image_inner sttext_item_image_link" title="{if isset($element.st_image_alt)}{$element.st_image_alt}{else}{$element.st_el_header}{/if}">
					{elseif isset($element.st_image_big) && $element.st_image_big}<a href="{$easy_image_path|cat:$element.st_image_big}" class="sttext_item_image_inner view_large_box st_popup_image" data-group="sttext_{$sub_column.id_st_easy_content_column}" rel="nofollow" title="{if isset($element.st_image_alt)}{$element.st_image_alt}{else}{$element.st_el_header}{/if}"><i class="fto-arrows-alt "></i>{else}<div class="sttext_item_image_inner">{/if}
						<img src="{if $element.st_image|strpos:'/modules/' !== false}{$element.st_image}{else}{$easy_image_path|cat:$element.st_image}{/if}" alt="{if isset($element.st_image_alt)}{$element.st_image_alt}{else}{$element.st_el_header}{/if}" width="{$element.st_image_width}" height="{$element.st_image_height}" class="hover_effect_target" />
					{if (isset($element.st_image_link) && $element.st_image_link) || (isset($element.st_image_big) && $element.st_image_big)}</a>{else}</div>{/if}
				</div>
			{/if}
