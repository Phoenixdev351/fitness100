...
jQuery(function($){
    ...
    $('#id_category_brand').each(function(){
        handle_apply_to($(this));
        return false;
    })
    ...
    $('#ac_products').autocomplete(currentIndex+'&token='+token+'&configure=stthemeeditor&ajax_products_list&disableCombination=true', {
        minChars: 1,
        autoFill: true,
        max:200,
        matchContains: true,
...
function checkEasyContetTypeInto(type)
{
    ...
    var items = {
        ...
        'module' : [2], 
        'id_cms' : [3], 
        'id_category' : [4,5,23,25],
        'id_category_brand' : [6,7,8,9,10,11,26,27],
    ...
    if(c!='id_category_brand')
        $('#ac_products').parents('.form-group').hide();
}
...
function handle_apply_to(that)
{
    var val = $(that).val();
    if (val == "1_1") {
        $('#ac_products').parents('.form-group').show();
	} else {
        $('#ac_products').parents('.form-group').hide();
    } 
}