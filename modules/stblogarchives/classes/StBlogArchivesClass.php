...
class StBlogArchivesClass extends ObjectModel
{
    ...
    public static function getArchives($id_lang = null, $active = true)
	{
		...
		SELECT DISTINCT b.`id_st_blog`, DATE_FORMAT(b.`date_add`,"%Y-%m-%M") date_add, DATE_FORMAT(b.`date_add`,"%Y-%m-%d") date_add2
        ...
        if($result)
    		foreach ($result as $row)
            {
                ...
                if (in_array($date_format, array('d/m/y','d-m-y'))) {
                    ...
                    $date2 = Tools::dateFormat(array('date'=>$row['date_add2'], 'full'=>0), Context::getContext()->smarty);
                }
                list($Y, $m) =  explode('-', $row['date_add']);
                list($Y2, $M) = explode('-', date('Y-F-d', strtotime($date2)));
                $archives[$Y2]['child'][] = array(
                    'Y'     => $Y, 
                    'Y2'    => $Y2,
                    'm'     => $m, 
                    'M'     => $M,
