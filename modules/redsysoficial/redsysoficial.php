...
class Redsysoficial extends PaymentModule {
	...
	public function __construct() {
		...
		if (isset($config['REDSYS_ACTIVAR_3DS']))
			$this->activar_3ds = $config['REDSYS_ACTIVAR_3DS'];
	...
	public function install() {
		if (! parent::install () 
				...
				|| ! Configuration::updateValue ( 'REDSYS_TIPOPAGO', ' ' ) 
				...
				|| ! Configuration::updateValue ( 'REDSYS_ACTIVAR_3DS', 'no')
	...
	private function _postProcess() {
		...
		if (Tools::isSubmit ( 'btnSubmit' )) {
			...
			Configuration::updateValue ( 'REDSYS_ACTIVAR_3DS', Tools::getValue ( 'activar_3ds' ) );
		}
	...
	{
		// lista de payments
		$this->html .= '<img src="../modules/redsysoficial/img/redsys.png" style="float:left; margin-right:15px;"><b><br />'
	...
	private function _displayForm()
	{
		...
		// Opciones para activar el uso de EMV 3DS
		$activar_3ds = Tools::getValue('activar_3ds', $this->activar_3ds);
		$activar_3ds_si = ($activar_3ds == 'si') ? ' checked="checked" ' : '';
		$activar_3ds_no = ($activar_3ds == 'no') ? ' checked="checked" ' : '';
		...
		if (!Tools::getValue('urltpv'))
			...
			<legend><img src="../img/admin/contact.gif" />'.$this->l('Configuración del TPV').'</legend>
				<table border="0" width="680" cellpadding="0" cellspacing="0" id="form">
					...
					<tr><td width="255" style="height: 35px;">'.$this->l('Tipos de pago permitidos').'</td><td><select name="tipopago" style="width: 120px;"><option value=" "'.$tipopago_a.'>Todos</option><option value="C"'.$tipopago_b.'>Solo con Tarjeta</option></select></td></tr>
	...
	$this->html.='	</select>
		...
		<tr>
		<td width="340" style="height: 35px;">'.$this->l('Activar el envío de datos adicionales para EMV 3DS').'</td>
			<td>
			<input type="radio" name="activar_3ds" id="activar_3ds_si" value="si" '.$activar_3ds_si.'/>
			<img src="../img/admin/enabled.gif" alt="'.$this->l('Activado').'" title="'.$this->l('Activado').'" />
			<input type="radio" name="activar_3ds" id="activar_3ds_no" value="no" '.$activar_3ds_no.'/>
			<img src="../img/admin/disabled.gif" alt="'.$this->l('Desactivado').'" title="'.$this->l('Desactivado').'" />
			</td>
		</tr>
		</table>
	...
	private function createParameter($params){
		...
		$valorCookie=$this->getRedsysCookie($orderId);
		...
		if ($sec_pedido < 9) {
			$this->setRedsysCookie($orderId,++ $sec_pedido);
		}

		$numpedido = str_pad ( $orderId . "z" . $sec_pedido . time()%1000, 12, "0", STR_PAD_LEFT );

		escribirLog(" - NÚMERO PEDIDO: Número de pedido interno: '" . $orderId . "'. Número de pedido enviado a REDSYS: '" . $numpedido . "'", $this->activar_log);
		...
		if (empty ( $_SERVER ['HTTPS'] )) {
			$protocolo = 'http://';
			$urltienda = $protocolo . $_SERVER ['HTTP_HOST'] . __PS_BASE_URI__ . 'index.php?fc=module&module=redsysoficial&controller=validation';
		} else {
			$protocolo = 'https://';
			$urltienda = $protocolo . $_SERVER ['HTTP_HOST'] . __PS_BASE_URI__ . 'index.php?fc=module&module=redsysoficial&controller=validation';
		}
		...
		if ($idiomas_estado == 'si') {
			...
			$miObj->setParameter ( "Ds_Merchant_Module", "PR_redsys_" . $this->version );
	...
	public function hookDisplayPaymentEU($params){
		...
		return array(
				...
				'logo' => _MODULE_DIR_."redsysoficial/img/tarjetas.png",
	...
	public function setRedsysCookie($order,$valor){
		$key="redsys".str_pad($order,12,"0",STR_PAD_LEFT);
