...
class StFeaturedCategories extends BaseSlider implements WidgetInterface
{
    ...
    protected function initTabNames()
    {
        $this->_tabs = array(
            ...
            array('id'  => '1,3', 'name' => $this->getTranslator()->trans('Homepage', array(), 'Admin.Theme.Transformer')),
    ...
    public function initFieldsForm()
    {
        ...
        unset($fields['home_slider']['grid']['values'][2]);
        unset($fields['home_slider']['display_sd']);
        unset($fields['home_slider']['price_color']);
        unset($fields['home_slider']['grid_bg']);
        unset($fields['home_slider']['grid_hover_bg']);
        unset($fields['home_slider']['view_more']);
        $fields['home_slider']['grid']['image_type'] = $this->getTranslator()->trans('Image type:', array(), 'Admin.Theme.Transformer');

        $fields['home_slider']['sub_nbr'] = array(
            'type' => 'text',
            'label' => $this->getTranslator()->trans('The number of subcategories in Grid view:', array(), 'Admin.Theme.Transformer'),
            'name' => 'sub_nbr',
            'default_value' => 5,
            'validation' => 'isUnsignedInt',
            'class' => 'fixed-width-sm',
            'desc' => $this->getTranslator()->trans('Set to 0 to not showing subcategories:', array(), 'Admin.Theme.Transformer'),
        );
    ...
    public function getConfigFieldsValues()
    {
        ...
        $fields_values['sub_nbr'] = (int)Configuration::get($this->_prefix_st.'SUB_NBR');
    ...
    {
        if (!$this->isCached($this->templatePath.'header.tpl', $this->stGetCacheId('header')))
        {
            $custom_css = '';
            ...
            if ($text_color = Configuration::get($this->_prefix_st.'TEXT_COLOR'))
                $custom_css .= '.featured_categories_container .s_title_block a, .featured_categories_container .cate_outer_box ul a{color:'.$text_color.';}';

            if ($link_hover_color = Configuration::get($this->_prefix_st.'LINK_HOVER_COLOR'))
                $custom_css .= '.featured_categories_container .s_title_block a:hover, .featured_categories_container .cate_outer_box ul a:hover{color:'.$link_hover_color.';}';
            ...
            if ($direction_color = Configuration::get($this->_prefix_st.'DIRECTION_COLOR'))
                $custom_css .= '.featured_categories_container.block .products_slider .swiper-button{color:'.$direction_color.';}';
            if ($direction_color_hover = Configuration::get($this->_prefix_st.'DIRECTION_COLOR_HOVER'))
                $custom_css .= '.featured_categories_container.block .products_slider .swiper-button:hover{color:'.$direction_color_hover.';}';
            if ($direction_color_disabled = Configuration::get($this->_prefix_st.'DIRECTION_COLOR_DISABLED'))
                $custom_css .= '.featured_categories_container.block .products_slider .swiper-button.swiper-button-disabled, .featured_categories_container.block .products_slider .swiper-button.swiper-button-disabled:hover{color:'.$direction_color_disabled.';}';
            
            if ($direction_bg = Configuration::get($this->_prefix_st.'DIRECTION_BG'))
                $custom_css .= '.featured_categories_container.block .products_slider .swiper-button{background-color:'.$direction_bg.';}';
            if ($direction_hover_bg = Configuration::get($this->_prefix_st.'DIRECTION_HOVER_BG'))
                $custom_css .= '.featured_categories_container.block .products_slider .swiper-button:hover{background-color:'.$direction_hover_bg.';}';
            if ($direction_disabled_bg = Configuration::get($this->_prefix_st.'DIRECTION_DISABLED_BG'))
                $custom_css .= '.featured_categories_container.block .products_slider .swiper-button.swiper-button-disabled, .featured_categories_container.block .products_slider .swiper-button.swiper-button-disabled:hover{background-color:'.$direction_disabled_bg.';}';
            /*else
                $custom_css .= '.featured_categories_container .products_slider .swiper-button.swiper-button-disabled, .featured_categories_container .products_slider .swiper-button.swiper-button-disabled:hover{background-color:transparent;}';*/
        ...
        return $this->fetch($this->templatePath.'header.tpl', $this->stGetCacheId('header'));
    }
    private function _prepareHook($location= null)
    {
		...
		$this->smarty->assign(array(
            ...
            'sub_nbr' => (int)Configuration::get($this->_prefix_st.'SUB_NBR'),
        ));
        return true;
    }
    protected function getTemplateVarCategories($categories)
    {
        ...
        return array_map(function (array $category) {
            ...
            if($sub_nbr = (int)Configuration::get($this->_prefix_st.'SUB_NBR')){
                if($category['children'] = $object->getSubCategories($this->context->language->id)){
                    if(count($category['children'])>$sub_nbr)
                        $category['children'] = array_slice($category['children'], 0, $sub_nbr);
                    foreach ($category['children'] as $key => $value) {
                        $category['children'][$key]['url'] = $this->context->link->getCategoryLink(
                            $value['id_category'],
                            $value['link_rewrite']
                        );
                    }
	...
	public function hookDisplayHome($params, $func = '',$flag=0)
	{
	    ...
	    if (!$this->isCached($this->templatePath.'stfeaturedcategories.tpl', $this->stGetCacheId($hook_hash))) {
    	    $this->_prepareHook();
		...
		return $this->fetch($this->templatePath.'stfeaturedcategories.tpl', $this->stGetCacheId($hook_hash));
	}
	/**
    ...
    public function vc_shortcode_init($atts, $content = null)
    {
        ...
        if (method_exists($this, $hook)) {
            return $this->$hook(array('st_time'=>$atts['st_time']));
        }
    }
