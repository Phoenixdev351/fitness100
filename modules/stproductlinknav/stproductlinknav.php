...
class StProductLinkNav extends Module
{
    ...
    public function _prepareHook($nav)
    {
        ...
        if (isset($this->context->cookie) && isset($this->context->cookie->last_visited_category) && Category::inShopStatic($this->context->cookie->last_visited_category, $this->context->shop) && Product::idIsOnCategoryId($id_product, array('0' => array('id_category' => $this->context->cookie->last_visited_category))))
            $id_last_visisted_category = $this->context->cookie->last_visited_category;
        else{
            if (isset($this->context->cookie) && isset($this->context->cookie->nav_last_visited_category) && Product::idIsOnCategoryId($id_product, array('0' => array('id_category' => $this->context->cookie->nav_last_visited_category))))
                $id_last_visisted_category = $this->context->cookie->nav_last_visited_category;
            else{
                $id_last_visisted_category = (int)($product->id_category_default);
                $this->context->cookie->nav_last_visited_category = (int)($product->id_category_default);
            }
        }
          
        $curr_position = $this->getWsPositionInCategory($id_product, $id_last_visisted_category);
        ...
        AND cp.`id_category` = '.(int)$id_last_visisted_category.'
        ...
        {  
            ...
            if(($cover = Product::getCover($data['id_product'])) && $cover['id_image'])
            {
                ...
                $product['cover'] = $this->context->link->getImageLink('', $this->context->language->iso_code . '-default', 'small_default');
                return $product;
            }
        }    
        else
        ...
        WHERE cp.`id_category` = '.(int)$$id_last_visisted_category.'
        AND cp.`position` '.($nav=='next' ? '>' : '<').$curr_position.'
