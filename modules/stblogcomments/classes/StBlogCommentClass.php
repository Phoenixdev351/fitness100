...
class StBlogCommentClass extends ObjectModel
{
    ...
    public static function getAll($id_parent=0, $active=false)
    {
        ...
        '.($active ? ' AND `active`=1' : '').self::getFilter().'            
        ');
    }
    public static function getFilter()
    {
        ...
        if (count($_POST)) {
            foreach($_POST as $key => $val) {
                if (strpos($key, 'stblogcommentsFilter_') !== false) {
                    $field = str_replace('stblogcommentsFilter_', '', $key);
                    if (($field == 'id_st_blog_comment' || $field == 'date_add') && $val) {
                        $where .= ' AND '.$field.'="'.$val.'"';
                    } elseif($field == 'active' && $val !== '') {
                        $where .= ' AND '.$field.'="'.(int)$val.'"';
                    } elseif ($val) {
