...
class StBlogComments extends Module
{
    ...
    public function hookActionDeleteGDPRCustomer($customer)
    {
        if (!empty($customer['id'])) {
            $sql = "DELETE FROM "._DB_PREFIX_."st_blog_comment WHERE id_customer = '".(int)$customer['id']."'";
            if (Db::getInstance()->execute($sql)) {
                return json_encode(true);
            }
            return json_encode($this->getTranslator()->trans('Blog comments: Unable to delete customer using ID.', array(), 'Shop.Theme.Transformer'));
        }
    }
    public function hookActionExportGDPRData($customer)
    {
        if (!Tools::isEmpty($customer['id'])) {
            $sql = "SELECT * FROM "._DB_PREFIX_."st_blog_comment WHERE id_customer = '".(int)$customer['id']."'";
            if ($res = Db::getInstance()->ExecuteS($sql)) {
                return json_encode($res);
            }
            return json_encode($this->getTranslator()->trans('Blog comments: No data found.', array(), 'Shop.Theme.Transformer'));
       }
   }
}
