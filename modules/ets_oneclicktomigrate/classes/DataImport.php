...
class DataImport extends Module
{
    ...
    public function importData($file_xml, $class_name, $definition, $foreign_key = array(), $multishop = false)
    {
        ...
        $import_history = Db::getInstance()->getRow('SELECT * FROM ' . _DB_PREFIX_ . 'ets_import_history WHERE id_import_history=' . (int)$id_import_history, false);
        $readXml = new ReadXML($file_xml);
        $languages = Language::getLanguages(false);
        while ($xml = $readXml->_readXML()) {
            if (isset($xml->$table) && $xml->$table) {
                $check_import = Db::getInstance()->getRow('SELECT * FROM ' . _DB_PREFIX_ . 'ets_' . pSQL($table) . '_import where id_import_history=' . (int)$id_import_history, false);
                if (!$check_import) {
                    if ($import_history['delete_before_importing'] && $class_name != 'Zone' && $class_name != 'Currency' && $class_name != 'Language' && $class_name != 'ShopGroup' && $class_name != 'Shop' && $class_name != 'ShopUrl' && $class_name != 'Employee' && $class_name != 'Country') {
                        try {
                            if ($class_name == 'Group') {
                                ...
                                $add_group = ' AND id_group NOT IN (' . implode(',', array_map('pSQL', $groups_to_keep)) . ')';
                            } else
                                $add_group = '';
                            if ($multishop || (Configuration::get('PS_MULTISHOP_FEATURE_ACTIVE') && Shop::getContext() == Shop::CONTEXT_ALL) || (!$multishop && !Configuration::get('PS_MULTISHOP_FEATURE_ACTIVE'))) {
                                if ($class_name == 'Carrier') {
                                    ...
                                    if ($table != 'lang')
                                        Db::getInstance()->execute('DELETE FROM ' . _DB_PREFIX_ . pSQL($table) . ' WHERE 1 ' . $add_group . ($class_name == 'Category' ? ' AND id_parent!=0 AND is_root_category!=1' : ''));
                                    if (isset($definition['multilang']) && $definition['multilang'])
                                        Db::getInstance()->execute('DELETE  FROM ' . _DB_PREFIX_ . pSQL($table) . '_lang WHERE 1 ' . $add_group . ($class_name == 'Category' ? ' AND id_category NOT IN (SELECT id_category FROM ' . _DB_PREFIX_ . 'category WHERE id_parent=0 OR is_root_category=1)' : '') . '');
                                    if (isset($definition['multilang_shop']) && $definition['multilang_shop'])
                                        Db::getInstance()->execute('DELETE FROM ' . _DB_PREFIX_ . pSQL($table) . '_shop WHERE 1 ' . $add_group . ($class_name == 'Category' ? ' AND id_category NOT IN (SELECT id_category FROM ' . _DB_PREFIX_ . 'category WHERE id_parent=0 OR is_root_category=1)' : '') . '');
                                    if ($class_name != 'Category' && $class_name != 'Cart')
                                        Db::getInstance()->execute('ALTER TABLE ' . _DB_PREFIX_ . pSQL($table) . ' AUTO_INCREMENT = 1');
                                    elseif ($class_name == 'Category') {
                                        $max_id_category = (int)Db::getInstance()->getValue('SELECT MAX(id_category) FROM ' . _DB_PREFIX_ . 'category', false);
                                        $max_id_category++;
                                    ...
                                    if ($class_name == 'CartRule') {
                                        ...
                                        Db::getInstance()->execute('DELETE FROM ' . _DB_PREFIX_ . 'cart_rule_product_rule_group');
                                        Db::getInstance()->execute('ALTER TABLE ' . _DB_PREFIX_ . 'cart_rule_product_rule_group AUTO_INCREMENT = 1');
                                        Db::getInstance()->execute('DELETE FROM ' . _DB_PREFIX_ . 'cart_rule_product_rule');
                                        Db::getInstance()->execute('ALTER TABLE ' . _DB_PREFIX_ . 'cart_rule_product_rule AUTO_INCREMENT = 1');
                                        Db::getInstance()->execute('DELETE FROM ' . _DB_PREFIX_ . 'cart_rule_product_rule_value');
                                    }
                                ...
                                if ($class_name == 'Carrier') {
                                    ...
                                    if ($table != 'lang')
                                        Db::getInstance()->execute('DELETE FROM ' . _DB_PREFIX_ . pSQL($table) . ' WHERE 1 ' . $add . $add_group . ($class_name == 'Category' ? ' AND id_parent!=0 AND is_root_category!=1' : '') . ($class_name == 'StockAvailable' || $class_name == "Order" ? ' AND id_shop="' . (int)Context::getContext()->shop->id . '"' : ''));
                                    if (isset($definition['multilang']) && $definition['multilang'])
                                        Db::getInstance()->execute('DELETE  FROM ' . _DB_PREFIX_ . pSQL($table) . '_lang WHERE 1 ' . ($definition['multilang_temp'] ? ' AND id_shop="' . (int)Context::getContext()->shop->id . '"' : '') . $add_group . ($class_name == 'Category' ? ' AND id_category NOT IN (SELECT id_category FROM ' . _DB_PREFIX_ . 'category WHERE id_parent=0 OR is_root_category=1)' : ''));
                                    if (isset($definition['multilang_shop']) && $definition['multilang_shop'])
                                        Db::getInstance()->execute('DELETE FROM ' . _DB_PREFIX_ . pSQL($table) . '_shop WHERE id_shop="' . (int)Context::getContext()->shop->id . '" ' . $add_group . ($class_name == 'Category' ? ' AND id_category NOT IN (SELECT id_category FROM ' . _DB_PREFIX_ . 'category WHERE id_parent=0 OR is_root_category=1)' : ''));
                                    if ($definition['table'] == 'group')
                            ...
                            } elseif ($class_name == 'OrderDetails') {
                                Db::getInstance()->execute('DELETE FROM ' . _DB_PREFIX_ . 'order_detail_tax WHERE id_order_detail NOT IN (SELECT id_order_detail from ' . _DB_PREFIX_ . 'order_detail)');
                            } elseif ($class_name == 'OrderInvoice') {
                                Db::getInstance()->execute('DELETE FROM ' . _DB_PREFIX_ . 'order_invoice_tax WHERE id_order_invoice NOT IN (SELECT id_order_invoice from ' . _DB_PREFIX_ . 'ps_order_invoice)');
                            } elseif ($class_name == 'OrderPayment') {
                                Db::getInstance()->execute('DELETE FROM ' . _DB_PREFIX_ . 'order_invoice_payment WHERE id_order_invoice NOT IN (SELECT id_order_invoice from ' . _DB_PREFIX_ . 'ps_order_invoice)');
                            }
                        } catch (Exception $exception) {
                            die('error');
                        }
                    } elseif ($class_name == 'SpecificPrice') {
                        Db::getInstance()->execute('DELETE FROM ' . _DB_PREFIX_ . pSQL($table) . ' WHERE id_product in (SELECT id_new FROM ' . _DB_PREFIX_ . 'ets_product_import WHERE id_import_history="' . (int)$id_import_history . '") AND ' . pSQL($primary) . ' NOT IN (SELECT id_new FROM ' . _DB_PREFIX_ . 'ets_' . pSQL($table) . '_import WHERE id_import_history ="' . (int)$id_import_history . '")');
                    }
                }
                foreach ($xml->$table as $data_xml) {
                    $object_id = 0;
                    $imported = (int)Db::getInstance()->getValue('SELECT number_import FROM ' . _DB_PREFIX_ . 'ets_import_history WHERE id_import_history=' . (int)$id_import_history, false);
                    if (!(int)Db::getInstance()->getValue('SELECT id_new FROM ' . _DB_PREFIX_ . 'ets_' . pSQL($table) . '_import WHERE id_old=' . (int)$data_xml->$primary . ' AND id_import_history="' . (int)$id_import_history . '"', false)) {
                        if ($class_name == 'Employee') {
                            if ($id_employee = (int)Db::getInstance()->getValue('SELECT id_employee FROM ' . _DB_PREFIX_ . 'employee WHERE email="' . pSQL((string)$data_xml->email) . '"', 0)) {
                                Db::getInstance()->execute('INSERT INTO ' . _DB_PREFIX_ . 'ets_' . pSQL($table) . '_import values("","' . (int)$data_xml->$primary . '","' . (int)$id_employee . '","' . (int)$id_import_history . '")');
                                $imported++;
                                Db::getInstance()->execute('UPDATE ' . _DB_PREFIX_ . 'ets_import_history SET number_import ="' . (int)$imported . '" WHERE id_import_history=' . (int)$id_import_history);
                                continue;
                            }
                        }
                        if ($class_name == 'Customer' && !(int)$data_xml->is_guest && !(int)$data_xml->deleted) {
                            if ($id_customer = (int)Db::getInstance()->getValue('SELECT id_customer FROM ' . _DB_PREFIX_ . 'customer WHERE email="' . pSQL((string)$data_xml->email) . '" AND is_guest=0 AND deleted=0', 0)) {
                                Db::getInstance()->execute('INSERT INTO ' . _DB_PREFIX_ . 'ets_' . pSQL($table) . '_import values("","' . (int)$data_xml->$primary . '","' . (int)$id_customer . '","' . (int)$id_import_history . '")');
                                $imported++;
                                Db::getInstance()->execute('UPDATE ' . _DB_PREFIX_ . 'ets_import_history SET number_import ="' . (int)$imported . '" WHERE id_import_history=' . (int)$id_import_history);
                                continue;
                            }
                        }
                        $imported = (int)Db::getInstance()->getValue('SELECT number_import FROM ' . _DB_PREFIX_ . 'ets_import_history WHERE id_import_history=' . (int)$id_import_history, 0);
                        if ($class_name != 'Zone' && $class_name != 'Language' && $class_name != 'Currency' && $class_name != 'ShopGroup' && $class_name != 'Shop' && $class_name != 'ShopUrl' && $class_name != 'Employee' && $class_name != 'Country' && $class_name != 'Carrier') {
                            ...
                            if ($class_name == 'Language') {
                                ...
                                if ($object_id) {
                                    Db::getInstance()->execute('INSERT INTO ' . _DB_PREFIX_ . 'ets_' . pSQL($table) . '_import values("","' . (int)$data_xml->$primary . '","' . (int)$object_id . '","' . (int)$id_import_history . '")');
                                    $imported++;
                                    Db::getInstance()->execute('UPDATE ' . _DB_PREFIX_ . 'ets_import_history SET number_import ="' . (int)$imported . '" WHERE id_import_history=' . (int)$id_import_history);
                                    continue;
                                }
                        ...
                        if ($class_name == 'Category') {
                            if (isset($data_xml->is_root_category)) {
                                if ((int)$data_xml->is_root_category && (int)Db::getInstance()->getValue('SELECT id_category FROM ' . _DB_PREFIX_ . 'category WHERE id_category=' . (int)$data_xml->id_category . ' AND is_root_category=1')) {
                        ...
                        if ($fields) {
                            foreach ($fields as $key => $field) {
                                if (!isset($field['lang']) || !$field['lang']) {
                                    ...
                                    if ((!isset($field['required']) || !$field['required']) && (string)$data_xml->$key == '')
                                        continue;
                                ...
                                if ($id_lang) {
                                    foreach ($fields as $key => $field) {
                                        if (isset($field['lang']) && $field['lang']) {
                                            ...
                                            if (isset($field['required']) && $field['required'] && empty($temp[$id_lang])) {
                        ...
                        if ($foreign_key) {
                            foreach ($foreign_key as $key => $value) {
                                if (isset($object->$key) && $object->$key) {
                                    if (isset($value['table_parent']) && ($key_extra = (int)Db::getInstance()->getValue('SELECT id_new FROM ' . _DB_PREFIX_ . 'ets_' . pSQL($value['table_parent']) . '_import WHERE id_old=' . (int)$object->$key . ' AND id_import_history="' . (int)$id_import_history . '"', 0))) {
                                        $object->$key = $key_extra;
                                    } elseif (isset($value['table_parent']) && !Db::getInstance()->getRow('SELECT id_new FROM ' . _DB_PREFIX_ . 'ets_' . pSQL($value['table_parent']) . '_import WHERE id_import_history="' . (int)$id_import_history . '"')) {
                                        if ($value['table_parent'] == 'category') {
                                            if ($key == 'id_category_default')
                                                $object->$key = (int)$import_history['id_category_default'];
                        ...
                        if ($class_name == 'Customer' && $object->is_guest == 0) {
                            if (Configuration::get('ETS_DATAMASTER_NEW_PASSWD')) {
                                ...
                                if ($id_ets_datamaster_customer_pasword = Db::getInstance()->getValue('SELECT id_ets_datamaster_customer_pasword FROM ' . _DB_PREFIX_ . 'ets_datamaster_customer_pasword WHERE id_import_history="' . (int)$id_import_history . '" AND email="' . pSQL($object->email) . '"')) {
                                    Db::getInstance()->execute('UPDATE ' . _DB_PREFIX_ . 'ets_datamaster_customer_pasword set passwd="' . pSQL($new_passwd) . '" WHERE id_ets_datamaster_customer_pasword="' . (int)$id_ets_datamaster_customer_pasword . '"');
                                } else
                                    Db::getInstance()->execute('INSERT INTO ' . _DB_PREFIX_ . 'ets_datamaster_customer_pasword(id_import_history,first_name,last_name,email,passwd) VALUES("' . (int)$id_import_history . '","' . pSQL($object->firstname) . '","' . pSQL($object->lastname) . '","' . pSQL($object->email) . '","' . pSQL($new_passwd) . '")');
                            } else {
                                $fieldsCustomer = Db::getInstance()->ExecuteS('DESCRIBE ' . _DB_PREFIX_ . 'customer');
                                $check_add = true;
                                foreach ($fieldsCustomer as $field) {
                        ...
                        if ($class_name == 'Employee') {
                            if (Configuration::get('ETS_DATAMASTER_NEW_PASSWD')) {
                                $new_passwd = $this->_module->genSecure(8);
                                $object->passwd = md5(pSQL(_COOKIE_KEY_ . $new_passwd));
                                if ($id_ets_datamaster_employee_pasword = Db::getInstance()->getValue('SELECT id_ets_datamaster_employee_pasword FROM ' . _DB_PREFIX_ . 'ets_datamaster_employee_pasword WHERE id_import_history="' . (int)$id_import_history . '" AND email="' . pSQL($object->email) . '"')) {
                                    Db::getInstance()->execute('UPDATE ' . _DB_PREFIX_ . 'ets_datamaster_employee_pasword set passwd="' . pSQL($new_passwd) . '" WHERE id_ets_datamaster_employee_pasword="' . (int)$id_ets_datamaster_employee_pasword . '"');
                                } else
                                    Db::getInstance()->execute('INSERT INTO ' . _DB_PREFIX_ . 'ets_datamaster_employee_pasword(id_import_history,first_name,last_name,email,passwd) VALUES("' . (int)$id_import_history . '","' . pSQL($object->firstname) . '","' . pSQL($object->lastname) . '","' . pSQL($object->email) . '","' . pSQL($new_passwd) . '")');
                            } else {
                                $fieldsEmployee = Db::getInstance()->ExecuteS('DESCRIBE ' . _DB_PREFIX_ . 'employee');
                                $check_add = true;
                                foreach ($fieldsEmployee as $field) {
                                    if ($field['Field'] == 'passwd_old_wp') {
                                        $check_add = false;
                                        break;
                                    }
                        ...
                        if (isset($definition['multilang_shop']) && $definition['multilang_shop'] && !$multishop && $class_name != 'Currency') {
                        ...
                        if ($class_name == 'Category' && $object->id == $object->id_parent)
                            unset($object->id);
                                ...
                                if (!$multishop) {
                                    if (!$multishop && Shop::getContext() == Shop::CONTEXT_ALL) {
                                        ...
                                        foreach ($shops as $shop) {
                                            StockAvailable::addSqlShopParams($shop, $shop['id_shop']);
                                            $object->id_shop = $shop['id_shop'];
                                            $object->id_shop_group = $shop['id_shop_group'];
                                            unset($object->id);
                                            if ($object->id_product_attribute == 0 || $this->checkStockAvailable($object->id_product, $object->id_product_attribute, $object->id_shop)) {
                                                $object->add();
                                            } else {
                                                $imported++;
                                                Db::getInstance()->execute('UPDATE ' . _DB_PREFIX_ . 'ets_import_history SET number_import ="' . (int)$imported . '" WHERE id_import_history=' . (int)$id_import_history);
                                                Db::getInstance()->execute('INSERT INTO ' . _DB_PREFIX_ . 'ets_' . pSQL($table) . '_import VALUES("","' . (int)$data_xml->$primary . '","' . (int)$data_xml->$primary . '","' . (int)$id_import_history . '")');
                                                continue;
                                            }
                                        }
                                    } else {
                                        $shop = array();
                                        StockAvailable::addSqlShopParams($shop, Context::getContext()->shop->id);
                                        $object->id_shop = $shop['id_shop'];
                                        $object->id_shop_group = $shop['id_shop_group'];
                                        unset($object->id);
                                        if ($object->id_product_attribute == 0 || $this->checkStockAvailable($object->id_product, $object->id_product_attribute, $object->id_shop)) {
                                    ...
                                    $shop = array();
                                    StockAvailable::addSqlShopParams($shop, $object->id_shop);
                                    $object->id_shop = $shop['id_shop'];
                                    $object->id_shop_group = $shop['id_shop_group'];
                                    unset($object->id);
                                    if ($object->id_product_attribute == 0 || $this->checkStockAvailable($object->id_product, $object->id_product_attribute, $object->id_shop)) {
                        ...
                        if (($class_name == 'Customer' && !Configuration::get('ETS_DATAMASTER_NEW_PASSWD')) || ($class_name == 'Employee' && !Configuration::get('ETS_DATAMASTER_NEW_PASSWD'))) {
                        ...
                        if (isset($object->id) && $object->id) {
                            $imported++;
                            Db::getInstance()->execute('UPDATE ' . _DB_PREFIX_ . 'ets_import_history SET number_import ="' . (int)$imported . '" WHERE id_import_history=' . (int)$id_import_history);
                            Db::getInstance()->execute('INSERT INTO ' . _DB_PREFIX_ . 'ets_' . pSQL($table) . '_import VALUES("","' . (int)$data_xml->$primary . '","' . (int)$object->id . '","' . (int)$id_import_history . '")');
                        }
                        ...
                        if ($class_name == 'Carrier' && isset($data_xml->id_reference) && (int)$data_xml->id_reference) {
                            ...
                            $id_reference_new = (int)Db::getInstance()->getValue('SELECT id_new FROM ' . _DB_PREFIX_ . 'ets_reference_import WHERE id_old=' . (int)$data_xml->id_reference . ' AND id_import_history=' . (int)$id_import_history, 0);
                            if ($id_reference_new) {
                                Db::getInstance()->execute('UPDATE ' . _DB_PREFIX_ . 'carrier SET id_reference="' . (int)$id_reference_new . '" WHERE id_carrier=' . (int)$object->id);
                            } else {
                                Db::getInstance()->execute('INSERT INTO ' . _DB_PREFIX_ . 'ets_reference_import values("","' . (int)$data_xml->id_reference . '","' . (int)$id_reference . '","' . (int)$id_import_history . '")');
                            }
                        }
                                            ...
                                            foreach ($fields as $key => $field) {
                                                if (isset($field['shop']) && $field['shop']) {
                                                    if (isset($foreign_key[$key]) && $foreign_key[$key] && isset($datashop[$key]) && $datashop[$key]) {
                                                        ...
                                                        if ($key_extra = (int)Db::getInstance()->getValue('SELECT id_new FROM ' . _DB_PREFIX_ . 'ets_' . pSQL($value['table_parent']) . '_import WHERE id_old=' . (int)$datashop[$key] . ' AND id_import_history="' . (int)$id_import_history . '"', 0)) {
                        ...
                        if ($class_name == 'Carrier' && $object->id && isset($data_xml->link_image) && (string)$data_xml->link_image) {
                            ...
                            DataImport::copy($url, _PS_SHIP_IMG_DIR_ . '/' . (int)$object->id . '.jpg', $context);
                        }
                        Configuration::updateValue('PS_DATAMASTER_IMPORTING', _DB_PREFIX_ . $table);
        ...
        if ($readXml->imported) {
            Db::getInstance()->execute('UPDATE ' . _DB_PREFIX_ . 'ets_import_history SET currentindex =1 WHERE id_import_history=' . (int)$id_import_history, false);
        }
    }
    ...
    public function importData14($file_xml, $class_name, $foreign_key = array())
    {
        ...
        while ($xml = $readXML->_readXML()) {
            if (isset($xml->$table) && $xml->$table) {
                $check_import = Db::getInstance()->getRow('SELECT * FROM ' . _DB_PREFIX_ . 'ets_' . pSQL($table) . '_import where id_import_history=' . (int)$id_import_history);
                if (!$check_import) {
                    if ($import_history['delete_before_importing'] && $class_name != 'Zone' && $class_name != 'Currency' && $class_name != 'Language' && $class_name != 'Employee' && $class_name != 'Country') {
                        ...
                        if ($class_name == 'Carrier') {
                            ...
                            Db::getInstance()->execute('DELETE FROM ' . _DB_PREFIX_ . pSQL($table) . ' WHERE 1 ' . $add_group . ($class_name == 'Category' ? ' AND id_parent!=0' : '') . '');
                            if ($multilang) {
                                Db::getInstance()->execute('DELETE  FROM ' . _DB_PREFIX_ . pSQL($table) . '_lang WHERE 1 ' . $add_group . ($class_name == 'Category' ? ' AND ' . pSQl($primary) . ' NOT IN (SELECT id_category FROM ' . _DB_PREFIX_ . 'category WHERE id_parent=0)' : '') . '');
                            }
                            if ($class_name != 'Category' && $class_name != 'Cart')
                                Db::getInstance()->execute('ALTER TABLE ' . _DB_PREFIX_ . pSQL($table) . ' AUTO_INCREMENT = 1');
                            elseif ($class_name == 'Category') {
                ...
                foreach ($xml->$table as $data_xml) {
                    $object_id = 0;
                    if (!Db::getInstance()->getRow('SELECT * FROM ' . _DB_PREFIX_ . 'ets_' . pSQL($table) . '_import WHERE id_old=' . (int)$data_xml->$primary . ' AND id_import_history="' . (int)$id_import_history . '"', false)) {
                        $imported = (int)Db::getInstance()->getValue('SELECT number_import FROM ' . _DB_PREFIX_ . 'ets_import_history WHERE id_import_history=' . (int)$id_import_history, 0);
                        ...
                        if ($class_name == 'Category') {
                            if (isset($data_xml->is_root_category)) {
                                if ((int)$data_xml->id_parent == 0)
                                    continue;
                                //$import_history['delete_before_importing']
                                if ((int)$data_xml->is_root_category == 1 && (int)Db::getInstance()->getValue('SELECT id_category FROM ' . _DB_PREFIX_ . 'category WHERE id_category=' . (int)$data_xml->id_category . ' AND is_root_category=1')) {
                        ...
                        if ($class_name == 'Employee') {
                            if ($id_employee = (int)Db::getInstance()->getValue('SELECT id_employee FROM ' . _DB_PREFIX_ . 'employee WHERE email="' . pSQL((string)$data_xml->email) . '"')) {
                                Db::getInstance()->execute('INSERT INTO ' . _DB_PREFIX_ . 'ets_employee_import values("","' . (int)$data_xml->$primary . '","' . (int)$id_employee . '","' . (int)$id_import_history . '")');
                                $imported++;
                                Db::getInstance()->execute('UPDATE ' . _DB_PREFIX_ . 'ets_import_history SET number_import ="' . (int)$imported . '" WHERE id_import_history=' . (int)$id_import_history);
                                continue;
                            }
                        ...
                        if ($fields) {
                            foreach ($fields as $field) {
                                if (isset($field['field']))
                                    ...
                                    $key = $field['Field'];
                                if ($key != $primary) {
                                    if ((!$fieldsRequired || !in_array($key, $fieldsRequired)) && (string)$data_xml->$key == '') {
                                        continue;
                                    }
                        ...
                        if ($foreign_key) {
                            foreach ($foreign_key as $key => $value) {
                                if (isset($object->$key) && $object->$key) {
                                    if ($key == 'id_parent') {
                                        $object->$key = (int)Db::getInstance()->getValue('SELECT id_new FROM ' . _DB_PREFIX_ . 'ets_' . pSQL($value['table_parent']) . '_import WHERE id_old=' . (int)$object->$key . ' AND id_import_history="' . (int)$id_import_history . '"');
                                    } elseif ($key_extra = (int)Db::getInstance()->getValue('SELECT id_new FROM ' . _DB_PREFIX_ . 'ets_' . pSQL($value['table_parent']) . '_import WHERE id_old=' . (int)$object->$key . ' AND id_import_history="' . (int)$id_import_history . '"')) {
                                        $object->$key = $key_extra;
                                    } elseif (!Db::getInstance()->getValue('SELECT id_new FROM ' . _DB_PREFIX_ . 'ets_' . pSQL($value['table_parent']) . '_import WHERE id_import_history="' . (int)$id_import_history . '"', 0)) {
                                        if ($value['table_parent'] == 'category')
                            ...
                            foreach ($languages as $language) {
                                if (!in_array($language['id_lang'], $list_language_xml)) {
                                    foreach ($fields_lang as $field) {
                                        ...
                                        if ($key != $primary && $key != 'id_lang') {
                                            $temp = $object->$key;
                                            if ($fieldsRequiredLang && isset($language['id_lang']) && isset($temp[$language['id_lang']]) && in_array($key, $fieldsRequiredLang) && !$temp[$language['id_lang']]) {
                                                if (isset($language_xml_default) && $language_xml_default && isset($language_xml_default->$key) && (string)$language_xml_default->$key != '') {
                                                    $temp[$language['id_lang']] = (string)$language_xml_default->$key;
                                                } else
                                                    $temp[$language['id_lang']] = 'Empty.';
                                            }
                                            if (isset($fieldsValidateLang[$key]) && isset($temp[$language['id_lang']]) && method_exists('Validate', $fieldsValidateLang[$key]) && $temp[$language['id_lang']] != '') {
                                                $validate = $fieldsValidateLang[$key];
        ...
        if ($readXML->imported) {
            Db::getInstance()->execute('UPDATE ' . _DB_PREFIX_ . 'ets_import_history SET currentindex =1 WHERE id_import_history=' . (int)$id_import_history);
        }
    }
    ...
    public function copyImg14($id_entity, $id_image = null, $url, $entity = 'products')
    {
        ...
        switch ($entity) {
            ...
            case 'products':
                ...
                if (method_exists($imageObj, 'getPathForCreation'))
                    $path = $imageObj->getPathForCreation();
                else
                    $path = _PS_PROD_IMG_DIR_ . (int)($imageObj->id_product) . '-' . (int)($imageObj->id);
                if (is_file(_PS_TMP_IMG_DIR_ . 'product_mini_' . (int)$id_entity . '.jpg')) {
                    @unlink(_PS_TMP_IMG_DIR_ . 'product_mini_' . (int)$id_entity . '.jpg');
                }
                break;
