...
class ImportHistory extends ObjectModel
{
    public $file_name;
    ...
    public function delete()
    {
        ...
        $_module = Module::getInstanceByName('ets_oneclicktomigrate');
        if($_module->tables)
        foreach($_module->tables as $table)
        {
            Db::getInstance()->execute('DELETE FROM '._DB_PREFIX_.'ets_'.pSQL($table).'_import WHERE id_import_history='.(int)$this->id);
        }
