    ...
    switch ($step) {
        ...
        case 2:
            if(Tools::getValue('data_import'))
            {
                ...
                Db::getInstance()->Execute('UPDATE '._DB_PREFIX_.'ets_import_history SET data="'.pSQL($data).'" where id_import_history='.(int)$id_import_history);
                $step++;
            }
            else
                $errors = $ets_datamaster->l('Please select kinds of data you want to import');
            break;
        case 3:
            $id_import_history=$context->cookie->id_import_history;
            ...
            Configuration::updateValue('ETS_DATAMASTER_NEW_PASSWD',Tools::getValue('import_create_new_passwd'));
            Configuration::updateValue('PS_DATAMASTER_IMPORTING','');
            Db::getInstance()->Execute('UPDATE '._DB_PREFIX_.'ets_import_history 
            ...
            die(
                Tools::jsonEncode(
                    array(
                        'error'=>false,
                        'step'=>$step,
                        'form_step'=>$ets_datamaster->displayFromStep($step),
                        'popup_import'=>$ets_datamaster->displayPopupHtml(),
                    )
                )
            );
        case 4:
                $step++;
                if($ets_datamaster->pres_version==1.4)
                {
                    $ets_datamaster->processImportData14();
                }
                else
                {
                    $ets_datamaster->processImportData();
                }
            break;
        case 5:
            break;
