      ...
      >
        <div class="form-fields">
          ...
          <div class="order-options">
            ...
            <div id="delivery" class="mb-2">
              <label for="delivery_message">{l s='If you would like to add a comment about your order, please write it in the field below.' d='Shop.Theme.Checkout'}</label><br/>
              <textarea rows="2" cols="120" id="delivery_message" name="delivery_message">{$delivery_message}</textarea>
            </div>
            {/if}
            {if $recyclablePackAllowed}
            <p class="flex_container flex_left">
              <span class="custom-input-box">
                ...
                <label class="mb-0">{l s='I would like to receive my order in recycled packaging.' d='Shop.Theme.Checkout'}</label>
              </p>
            {/if}
            {if $gift.allowed}
            <p class="flex_container flex_left">
              <span class="custom-input-box">
                <input class="js-gift-checkbox custom-input" id="input_gift" name="gift" type="checkbox" value="1" {if $gift.isGift}checked="checked"{/if}>
                <span for="input_gift" class="custom-input-item custom-input-checkbox"><i class="fto-ok-1 checkbox-checked"></i></span>
              </span>
                <label class="mb-0">{$gift.label}</label >
              </p>
              <div id="gift" class="collapse{if $gift.isGift} in{/if} mb-2">
