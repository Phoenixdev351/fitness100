...
<div class="product-add-to-cart  mb-3">
  {if !$configuration.is_catalog}
    ...
    {block name='product_availability'}
        <div id="product-availability" class="js-product-availability {if $product.availability == 'available'} product-available {elseif $product.availability == 'last_remaining_items'} product-last-items {else} product-unavailable {/if} mar_b6 fs_md">
        {if $product.availability == 'available'}{/if}
    ...
    {block name='product_quantity'}
      <div class="product-quantity flex_child {if !$product.add_to_cart_url} hide_main_cart_button {/if}">
        ...
        {if (isset($sttheme.buy_now) && $sttheme.buy_now) || (isset($steasybuilder) && $steasybuilder.is_editing)}
        <div class="add_buy_now mar_b6 {if $sttheme.product_buy_button} add_full_width {/if} {if !isset($sttheme.buy_now) || !$sttheme.buy_now} display_none {/if}">
          <button class="btn btn-default btn-large add-to-cart btn-full-width btn-spin benabled" data-button-action="add-to-cart" type="submit" {if !$product.add_to_cart_url} disabled {/if}>
            <i class="fto-glyph icon_btn"></i><span>{l s='Buy now' d='Shop.Theme.Actions'}</span>
          </button>
        </div>
        {/if}
      </div>
