...
<div class="product_info_tabs sttab_block mobile_tab {if $sttheme.product_tabs_style==0 || $sttheme.product_tabs_style==5} sttab_2 sttab_2_2 {elseif $sttheme.product_tabs_style==2 || $sttheme.product_tabs_style==6} sttab_2 sttab_2_3 {elseif $sttheme.product_tabs_style==3} sttab_3 sttab_3_2 flex_container flex_start {/if}">
  <ul class="nav nav-tabs {if !$sttheme.product_tabs} tab_lg {/if} {if $sttheme.product_tabs_style==2 || $sttheme.product_tabs_style==6} flex_box flex_center {/if}" role="tablist">
    ...
    {foreach from=$product.extraContent item=extra key=extraKey}
        ...
        {if $extra.moduleName == 'stthemeeditor'}
          {if $sttheme.display_pro_tags==1 && isset($extra.content.tags)}
            <li class="nav-item">
              <a class="nav-link {if !$st_active_pro_tab || $st_active_pro_tab=='porudct-tags-tab'} active{/if}" data-toggle="tab" role="tab" aria-controls="porudct-tags-tab" data-module="{$extra.moduleName}" href="#porudct-tags-tab">{l s='Tags' d='Shop.Theme.Transformer'}</a>
            </li>
    ...
    {/foreach}
    {hook h="displayProductTab" st_active_pro_tab=$st_active_pro_tab}
  </ul>
  ...
  <div class="tab-content {if $sttheme.product_tabs_style==3} flex_child {/if}">
           ...
           <div class="product_description_container style_content truncate_block st_showless_block_{if !empty($sttheme.showless_pro_desc)}1{else}0{/if} truncate_cate_desc_{$sttheme.truncate_pro_desc}">
            <div class="st_read_more_box">
              ...
              {foreach from=$product.extraContent item=extra key=extraKey}
                  ...
                  {/if}
                  {if $extra.moduleName == 'steasybuilder' && isset($extra.content.description) && $extra.content.description}
                      {$extra.content.description nofilter}
                  {/if}
              {/foreach}
            </div>
            <a href="javascript:;" title="{l s='Read more' d='Shop.Theme.Transformer'}" class="st_read_more" rel="nofollow"><span class="st_showmore_btn">{l s='Read more' d='Shop.Theme.Transformer'}</span><span class="st_showless_btn">{l s='Show less' d='Shop.Theme.Transformer'}</span></a>
          </div>
          </div>
         {/block}
        </div>
   ...
   {block name='product_attachments'}
     {if $product.attachments}
      <div role="tabpanel" class="tab-pane {if $st_active_pro_tab=='attachments'} active {if $sttheme.product_acc_style!=0 || (isset($steasybuilder) && $steasybuilder.is_editing)} st_open {/if} {elseif $sttheme.product_acc_style==2} st_open {/if}" id="attachments">
        <div class="mobile_tab_title">
            <a href="javascript:;" class="opener"><i class="fto-plus-2 plus_sign"></i><i class="fto-minus minus_sign"></i></a>
   ...
   {/foreach}
   {hook h="displayProductTabContent" st_active_pro_tab=$st_active_pro_tab}
</div>
</div>
{/block}