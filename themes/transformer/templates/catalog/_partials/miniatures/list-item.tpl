 ...
 *}
  ...
  <div class="products product_list {if isset($cssClass)} {$cssClass}{/if} {if $sttheme.infinite_scroll==1} waypoint {/if}{if $for_w=='category' && $sttheme.list_grid} list {else} row grid {/if} {if $sttheme.clear_list_view} clear_list_{$sttheme.clear_list_view} {/if} clear_list_align_{(int)$sttheme.list_view_align} clear_list_proportion_{(int)$sttheme.list_view_proportion}" data-classnames="{$smarty.capture.product_gide_class}">
    ...
    {if $sttheme.big_next && $for_w=="category" && !$sttheme.infinite_scroll && $listing.pagination.pages_count>1 && $listing.pagination.items_shown_to<$listing.pagination.total_items}
      ...
      {if $curr_iteration%$pro_per_lg == 0} last-in-line{elseif $curr_iteration%$pro_per_lg == 1} first-in-line{/if}{if $curr_iteration > ($nbLi - $totModulo)} last-line{/if}{if $curr_index < $pro_per_lg} first-line{/if}
      {if $curr_iteration%$pro_per_md == 0} last-item-of-tablet-line{elseif $curr_iteration%$pro_per_md == 1} first-item-of-tablet-line{/if}{if $curr_iteration > ($nbLi - $totModuloTablet)} last-tablet-line{/if}{if $curr_index < $pro_per_md} first-tablet-line{/if}
      {if $nbLiNext%$pro_per_sm == 0} last-item-of-mobile-line{elseif $nbLiNext%$pro_per_sm == 1} first-item-of-mobile-line{/if}{if $nbLiNext > ($nbLi - $totModuloMobile)} last-mobile-line{/if}{if $curr_index < pro_per_sm} first-mobile-line{/if}
      {if $nbLiNext%$pro_per_xs == 0} last-item-of-portrait-line{elseif $nbLiNext%$pro_per_xs == 1} first-item-of-portrait-line{/if}{if $nbLiNext > ($nbLi - $totModuloPortrait)} last-portrait-line{/if}{if $curr_index < $pro_per_xs} first-portrait-line{/if} big_page_next_wrap">
