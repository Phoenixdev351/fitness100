 ...
 *}
    ...
    || (isset($steasybuilder) && $steasybuilder.is_editing)}
    <div id="footer-bottom" class="{if $sttheme.f_info_center} footer_bottom_center {/if}">
        <div class="wide_container_box {if !$sttheme.f_info_fullwidth && $sttheme.responsive_max!=3}wide_container{/if}">
            <div id="footer-bottom-container" class="{if !$sttheme.f_info_fullwidth && $sttheme.responsive_max!=3}container{else}container-fluid{/if}">
                <div class="row">
                    <div class="col-12 col-sm-12 clearfix">      
                        <aside id="footer_bottom_right">
                        	{if isset($sttheme.footer_img_src) && $sttheme.footer_img_src}    
	                            <img data-src="{$sttheme.footer_img_src}" alt="{l s='Payment methods' d='Shop.Theme.Transformer'}" class="lazyload" width="242" height="21" />
	                        {/if}
                            {$smarty.capture.displayFooterBottomRight nofilter}
                        </aside>
                        <aside id="footer_bottom_left">
                        	{block name='copyright_link'}<div class="copyright_text">{if isset($sttheme.copyright_text)}{$sttheme.copyright_text nofilter}{/if}</div>{/block}
