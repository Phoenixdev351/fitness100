...
<div id="js-active-search-filters" class="active_filters_box flex_container flex_start {if !$filter_position && !$activeFilters|count} hidden-xs-up {/if}">
    ...
    <a class="toggle_btn active_filter_item {if $filter_position>1} st_show_on_mobile collapsed {/if}" data-toggle="collapse" href="#horizontal_filters" aria-expanded="{if $filter_position>1}false{else}true{/if}" aria-controls="horizontal_filters" title="{l s='Toggle filters' d='Shop.Theme.Transformer'}">
